---
title: Konfigurieren der HBase-Replikation zwischen zwei Datencentern | Microsoft Docs
...
---
# <a name="configure-hbase-geo-replication-in-hdinsight"></a>Konfigurieren von HBase-Georeplikation in HDInsight
...
* [Konfigurieren einer VPN-Verbindung zwischen zwei virtuellen Netzwerken][hdinsight-hbase-replication-vnet]
* [Konfigurieren von DNS für virtuelle Netzwerke][hdinsight-hbase-replication-dns]
* Konfigurieren von HBase-Georeplikation (dieses Tutorial)

Das folgende Diagramm veranschaulicht die beiden virtuellen Netzwerke und die Netzwerkverbindung, die Sie in [Konfigurieren einer VPN-Verbindung zwischen zwei virtuellen Netzwerken][hdinsight-hbase-geo-replication-vnet] und [Konfigurieren von DNS für virtuelle Netzwerke][hdinsight-hbase-replication-dns] erstellt haben:
...
* **Zwei virtuelle Netzwerke in Azure mit VPN-Konnektivität und konfiguriertem DNS**.  Anweisungen hierzu finden Sie unter [Konfigurieren einer VPN-Verbindung zwischen zwei virtuellen Azure-Netzwerken][hdinsight-hbase-replication-vnet] und [Konfigurieren von DNS zwischen zwei virtuellen Azure-Netzwerken][hdinsight-hbase-replication-dns].
...
## <a name="provision-hbase-clusters-in-hdinsight"></a>Bereitstellen von HBase-Clustern in HDInsight
In [Konfigurieren einer VPN-Verbindung zwischen zwei virtuellen Azure-Netzwerken][hdinsight-hbase-replication-vnet] haben Sie ein virtuelles Netzwerk in einem Rechenzentrum in Europa und ein virtuelles Netzwerks in einem US-Rechenzentrum erstellt. Die beiden virtuellen Netzwerke sind über VPN verbunden. In dieser Sitzung werden Sie einen HBase-Cluster in den einzelnen virtuellen Netzwerken bereitstellen. Später in diesem Lernprogramm erstellen Sie eines der HBase-Cluster, um das andere HBase-Cluster zu replizieren.

Das klassische Azure-Portal unterstützt die Bereitstellung von HDInsight-Clustern mit benutzerdefinierten Konfigurationsoptionen nicht. Stellen Sie z.B. *hbase.replication* auf *true* ein. Wenn Sie nach der Bereitstellung eines Clusters den Wert in der Konfigurationsdatei festlegen, verlieren Sie die Einstellung nach dem Durchführen des Reimagings des Clusters. Weitere Informationen finden Sie unter [Bereitstellen von Hadoop-Clustern in HDInsight][hdinsight-provision]. Eine der Optionen zum Bereitstellen von HDInsight-Clustern mit benutzerdefinierten Optionen ist die Verwendung von Azure PowerShell.
...
## <a name="configure-dns-conditional-forwarder"></a>Konfigurieren der bedingten DNS-Weiterleitungen
...
1. RDP in **Contoso-HBase-EU**.  Anweisungen finden Sie unter [Verwalten von Hadoop-Clustern in HDInsight mit dem klassischen Azure-Portal][hdinsight-manage-portal]. Tatsächlich handelt es sich um headnode0 des Clusters.
...
3. Klicken Sie auf **Verwaltungstools**.
4. Klicken Sie auf **DNS**.
5. Erweitern Sie im linken Fensterbereich **DSN**, **Contoso-DNS-EU**.
6. Klicken Sie mit der rechten Maustaste auf **Bedingte Weiterleitungen** und klicken Sie dann auf **Neue bedingte Weiterleitung**.
...
8. Drücken Sie die **EINGABETASTE**, und klicken Sie dann auf **OK**.  Sie können nun die IP-Adresse von Contoso-DNS-US von der von Contoso-DNS-EU auflösen.
9. Wiederholen Sie die Schritte zum Hinzufügen einer bedingten DNS-Weiterleitung an den DNS-Dienst auf dem virtuellen Computer von Contoso-DNS-US mit den folgenden Werten:
...
> [!IMPORTANT]
> DNS muss funktionieren, bevor Sie mit den nächsten Schritten fortfahren können.
...
3. Wechseln Sie in das HBase-Stammverzeichnis:
        ...
        cd %HBASE_HOME%\bin
4. Öffnen Sie die HBase-Befehlszeile:

        hbase shell
5. Erstellen einer HBase-Tabelle:

        create 'Contacts', 'Personal', 'Office'
6. Schließen Sie weder die RDP-Sitzung noch das Hadoop-Befehlszeilenfenster. Sie werden diese später noch im Lernprogramm benötigen.
...
1. Wechseln Sie zum RDP-Fenster **Contso-HBase_EU**.
...
1. Konfigurieren Sie aus dem HBase-Shell-Fenster der RDP-Sitzung **Contso-HBase-EU-** jede Spaltenfamilie, die repliziert werden soll:
...
* [Bereitstellen von HBase-Clustern in Azure Virtual Network][hdinsight-hbase-provision-vnet]
