---
title: Installieren von GNU R in Linux-basiertem HDInsight | Microsoft-Dokumentation
...
---
# <a name="install-and-use-gnu-r-on-linux-based-hdinsight-hadoop-clusters"></a>Installieren und Verwenden von GNU R in Linux-basierten HDInsight-Hadoop-Clustern

Sie können GNU R in einem beliebigen Clustertyp in Hadoop in der HDInsight-Umgebung mithilfe der **Skriptaktion**-Clusteranpassung installieren. Data Scientists und Analysten können mit R große Mengen von Daten in Hadoop-Clustern verarbeiten, die in HDInsight bereitgestellt werden.

> [!WARNING]
> Dieses Dokument enthält Anweisungen zum Installieren von GNU R in HDInsight als benutzerdefinierte Aktion. Dies bietet __nicht__ die gleichen Funktionen wie __Microsoft R Server in HDInsight__. Es wird empfohlen, R-Server in HDInsight einzusetzen, statt die Schritte in diesem Dokument auszuführen.
>
> Informationen zur Verwendung von R Server in HDInsight finden Sie unter [Erste Schritte mit R Server in HDInsight](hdinsight-hadoop-r-server-get-started.md). 
...
* [r-base-dev](http://packages.ubuntu.com/precise/r-base-dev): GNU R-Hilfspakete
...
## <a name="install-r-using-script-actions"></a>Installation von R mithilfe von Skriptaktionen
...
Dieser Abschnitt enthält Anweisungen zur Verwendung des Skripts während der Erstellung eines Clusters mithilfe des Azure-Portals. 
...
1. Beginnen Sie die Bereitstellung eines Clusters anhand der Schritte in [Bereitstellen Linux-basierter HDInsight-Cluster](hdinsight-hadoop-provision-linux-clusters.md), schließen Sie sie jedoch nicht ab.

2. Wählen Sie auf dem Blatt **Optionale Konfiguration** die Option **Skriptaktionen**, und geben Sie die folgenden Informationen an, um das Formular auszufüllen:
...
4. Setzen Sie die Bereitstellung des Clusters wie unter [Bereitstellen Linux-basierter HDInsight-Cluster](hdinsight-hadoop-provision-linux-clusters.md)beschrieben fort.
...
1. Stellen Sie mithilfe von SSH eine Verbindung mit dem HDInsight-Cluster her:
   ...
   * [Verwenden von SSH mit Linux-basiertem Hadoop in HDInsight unter Bash on Windows 10, Unix oder OS X](hdinsight-hadoop-linux-use-ssh-unix.md)
...
3. Geben Sie das folgende R-Programm ein. Mit diesem Code werden die Zahlen 1 bis 100 erzeugt und dann mit 2 multipliziert.
...
4. Als Nächstes verwenden Sie den folgenden Befehl, um temporären Pfad anzuzeigen, unter dem die MapReduce-Ausgabe gespeichert wurde:
    ...
    Der Befehl gibt einen Wert zurück, der `/tmp/file5f615d870ad2` ähnelt. Verwenden Sie den folgenden Befehl, um die tatsächliche Ausgabe anzuzeigen:
   
        print(from.dfs(calc))
   
    Der folgende Text ist ein Beispiel der Ausgabe:
...
5. Geben Sie zum Beenden von R folgenden Befehl ein:
