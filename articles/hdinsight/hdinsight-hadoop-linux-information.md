...
Intern verfügt jeder Knoten im Cluster über einen Namen, der während der Konfiguration des Clusters zugewiesen wird. Um die Clusternamen zu suchen, können Sie die Seite **Hosts** auf der Ambari-Webbenutzeroberfläche besuchen. Gehen Sie alternativ wie folgt vor, um eine Liste der Hosts über die Ambari REST-API zurückzugeben:

    curl -u admin:PASSWORD -G "https://CLUSTERNAME.azurehdinsight.net/api/v1/clusters/CLUSTERNAME/hosts" | jq '.items[].Hosts.host_name'

Ersetzen Sie dabei **PASSWORD** durch das Kennwort des Administratorkontos und **CLUSTERNAME** durch den Namen des Clusters. Hiermit wird ein JSON-Dokument mit einer Liste der Hosts im Cluster zurückgegeben, dann ruft jq den Wert des `host_name`-Elements für die einzelnen Hosts im Cluster ab.
...
Bei einigen Befehlen müssen Sie möglicherweise angeben, dass Sie Blobspeicher verwenden. Diesen Befehle können Sie die Zeichenfolge **wasb://** oder **wasbs://** voranstellen.
...
1. Verwenden Sie den folgenden Befehl, um HDFS-Konfigurationsinformationen über „curl“ abzurufen, und filtern Sie sie mit [jq](https://stedolan.github.io/jq/):
   ...
   > 
    ...
    Dadurch wird ein ähnlicher Wert wie der folgende zurückgegeben, wobei **CONTAINER** der Standardcontainer und **ACCOUNTNAME** der Name des Azure-Speicherkontos ist:
   
        wasbs://CONTAINER@ACCOUNTNAME.blob.core.windows.net
2. Rufen Sie die Ressourcengruppe für das Speicherkonto über die [Azure-Befehlszeilenschnittstelle](../xplat-cli-install.md) ab. Ersetzen Sie im folgenden Befehl **ACCOUNTNAME** durch den aus Ambari abgerufenen Namen des Speicherkontos:
   ...
   > [!NOTE]
   > Wenn von diesem Befehl nichts zurückgegeben wird, müssen Sie die Azure-Befehlszeilenschnittstelle eventuell in den Azure-Ressourcen-Manager-Modus ändern und den Befehl erneut ausführen. Wechseln Sie mit dem folgenden Befehl in den Azure-Ressourcen-Manager-Modus:
   > 
   > `azure config mode arm`
   > 
   > 
3. Rufen Sie den Schlüssel für das Speicherkonto ab. Ersetzen Sie **GROUPNAME** durch den Namen der Ressourcengruppe aus dem vorherigen Schritt. Ersetzen Sie **ACCOUNTNAME** durch den Namen des Speicherkontos:
...
* **Storm**: Sie sollten alle ausgeführten Storm-Topologien erneut ausgleichen, nachdem ein Skalierungsvorgang durchgeführt wurde. So kann die Topologie die Parallelitätseinstellungen an die neue Anzahl von Knoten im Cluster anpassen. Verwenden Sie eine der folgenden Optionen, um ausgeführte Topologien erneut auszugleichen:
  ...
  * **Storm-Benutzeroberfläche**: Führen Sie die folgenden Schritte aus, um eine Topologie mithilfe der Storm-Benutzeroberfläche erneut auszugleichen.
    ...
    1. Öffnen Sie \_\_https://CLUSTERNAME.azurehdinsight.net/stormui__ in Ihrem Webbrowser, wobei CLUSTERNAME der Name Ihres Storm-Clusters ist. Geben Sie bei entsprechender Aufforderung den Namen und das Kennwort des HDInsight-Clusteradministrators ein, die Sie beim Erstellen des Clusters festgelegt haben.
