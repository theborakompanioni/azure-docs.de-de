...
* Ein Apache Storm auf HDInsight-Clusterversion 3.4. Verwenden Sie einen der folgenden Artikel zu ersten Schritten, um einen Cluster zu erstellen:
...
* Ein Texteditor oder eine integrierte Entwicklungsumgebung (IDE) für Java.
  ...
  * Einen SSH-Client. Weitere Informationen zur Verwendung von SSH mit HDInsight finden Sie in den folgenden Artikeln:
    ...
    * [Verwenden von SSH mit Linux-basiertem Hadoop in HDInsight unter Linux, OS X oder Bash unter Windows 10](hdinsight-hadoop-linux-use-ssh-unix.md).
...
**com.microsoft.example.EventHubReader** liest Daten vom Event Hub (die von EventHubWriter geschriebenen Daten) und speichert diese im Standardspeicher des Clusters im Verzeichnis „/devicedata“.
...
* Umbenennen von Lizenzdateien für die Abhängigkeiten: Wenn das nicht erfolgt, kann das Laufzeitfehler zur Folge haben.
...
* **EventHubs.properties**: Enthält Informationen zum Herstellen einer Verbindung mit einem Azure Event Hub.

Sie müssen die Datei `EventHubs.properties` mit Informationen zu Ihrem Event Hub und HDInsight-Cluster füllen.
...
4. Geben Sie den Topologien ein oder zwei Minuten, um die Ereignisse vom Event Hub zu schreiben und lesen, und prüfen Sie dann mit dem folgenden Befehl, ob der EventHuabReader Daten in Ihrem HDInsight-Speicher speichert:
   ...
   > Möglicherweise wird für einige Dateien die Dateigröße 0 angezeigt, da sie vom EventHubReader erstellt wurden, aber keine Daten darin gespeichert wurden.
    ...
    Dieser gibt Daten zurück, die in etwa wie folgt aussehen:
...
### <a name="if-using-a-windows-based-hdinsight-cluster"></a>Bei Verwendung eines Windows-basierten HDInsight-Clusters
...
7. Wählen Sie in der **Abfragekonsole** die Option **Hive-Editor**, und ersetzen Sie den Standardwert `select * from hivesampletable` durch den folgenden:
    ...
    Klicken Sie auf **Auswählen** , um die Abfrage auszuführen. Dadurch werden 10 Zeilen der Daten ausgegeben, die vom EventHubReader in den Clusterspeicher geschrieben wurden. Nach Abschluss der Abfrage sollten Sie eine Ausgabe ähnlich der folgenden erhalten:
...
Die persistenten Prüfpunkte können an WASB (der von HDInsight-Clustern auf Windows-Basis verwendete Azure-Speicher) exportiert und importiert werden. Die dafür geeigneten Skripts befinden sich auf dem Storm-Cluster in HDInsight unter **c:\apps\dist\storm-0.9.3.2.2.1.0-2340\zkdatatool-1.0\bin**.
