...
- beteiligt sich an der [rollenbasierten Zugriffssteuerung (role-based access control, RBAC) in Azure](/active-directory/role-based-access-control-what-is.md) – Datenbanken, Pools für elastische Datenbanken und Data Warehouses innerhalb eines Servers erben dessen Zugriffsrechte
...
Um mithilfe des [Azure-Portals](https://portal.azure.com) einen Azure SQL-Datenbankserver (ohne eine Datenbank) zu erstellen, navigieren Sie zu einem leeren SQL-Server-Formular (logischer Server). Der folgende Screenshot zeigt eine Methode zum Öffnen eines Formulars, um einen leeren logischen SQL-Server zu erstellen. 
...
Wenn Sie über eine andere Methode zu diesem Formular gelangen, sind die Informationen auf dem Formular identisch.
...
Zum Verwalten eines vorhandenen Servers navigieren Sie mithilfe einer Reihe von Methoden zum Server, z.B. über eine bestimmte Seite der SQL-Datenbank, die Seite der **SQL-Server** oder die Seite **All resources** (Alle Ressourcen). Der folgende Screenshot zeigt, wie Sie beginnen, über die Seite **Übersicht** für den Server eine Firewall auf Serverebene festzulegen. 
...
> [!TIP]
> Ein Schnellstart-Tutorial zu PowerShell finden Sie unter [Erstellen einer einzelnen Azure SQL-Datenbank mithilfe von PowerShell](sql-database-get-started-portal.md).
>
...
> [!TIP]
> Ein Schnellstart-Tutorial zur Azure CLI finden Sie unter [Erstellen einer einzelnen Azure SQL-Datenbank mithilfe der Azure CLI](sql-database-get-started-cli.md).
>
...
> [!IMPORTANT]
> Sie können einen Server mithilfe von Transact-SQL nicht erstellen, aktualisieren oder löschen.
>
...
|[DROP DATABASE (Transact-SQL)](/sql/t-sql/statements/drop-database-transact-sql)|Entfernt eine Datenbank.|
