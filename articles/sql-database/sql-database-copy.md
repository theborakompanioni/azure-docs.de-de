...
Sie können mithilfe der [automatisierten Sicherungen von Azure SQL-Datenbank](sql-database-automated-backups.md) eine Kopie der SQL-Datenbank erstellen. Beim Kopiervorgang wird das Ende des Transaktionsprotokolls kopiert. Dann werden die vollständigen und differenziellen Sicherungen sowie die Transaktionsprotokollsicherungen, die Teil der automatisierten Sicherungen sind, zum Erstellen einer Kopie verwendet, die im Hinblick auf Transaktionen konsistent mit der Quelldatenbank zum Zeitpunkt der letzten Transaktionsprotokollsicherung ist. 
