...
- SQL Server 2016 Management Studio und SQL Server Data Tools für Visual Studio 2015 unterstützen Azure Active Directory-Authentifizierung. (Die Azure Active Directory-Authentifizierung wird vom **.NET Framework-Datenanbieter für SqlServer** unterstützt; mindestens .NET Framework 4.6.) Hierfür können die neuesten Versionen dieser Tools und Datenschichtanwendungen (DAC und .bacpac) Azure Active Directory-Authentifizierung verwenden, aber **sqlcmd.exe** und **bcp.exe** können keine Verbindung herstellen, da sie den ODBC-Anbieter verwenden.
- [Microsoft JDBC-Treiber 6.0 für SQL Server](https://blogs.technet.microsoft.com/dataplatforminsider/2016/04/04/preview-the-microsoft-jdbc-driver-6-0-for-sql-server/) unterstützt Azure Active Directory-Authentifizierung.
...
- Azure Active Directory-Authentifizierung wird von den Blättern **Datenbank importieren** und **Datenbank exportieren** des Azure-Portals unterstützt.
...
> [AZURE.NOTE] Datenbankbenutzer (mit Ausnahme von Administratoren) können nicht mithilfe des Portals erstellt werden, RBAC-Rollen werden nicht an SQL Server verteilt, und Azure RBAC-Rollen gelten nicht für Datenbankberechtigungen. Beispielsweise gewährt die Rolle **SQL Server-Mitwirkender** keinen Zugriff zur Verbindung mit der SQL-Datenbank. Die Zugriffsberechtigung muss direkt in der Datenbank mithilfe von Transact-SQL-Anweisungen erteilt werden.
