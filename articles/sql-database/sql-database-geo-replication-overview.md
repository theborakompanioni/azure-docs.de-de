---
title: "Failovergruppen und aktive geografische Replikation – Azure SQL-Datenbank | Microsoft-Dokumentation"
description: "Gruppen für automatisches Failover mit aktiver Georeplikation ermöglichen Ihnen die Einrichtung von 4 Replikaten Ihrer Datenbank in einem beliebigen Azure-Rechenzentrum und automatisches Failover bei einem Ausfall."
...
---
# <a name="overview-failover-groups-and-active-geo-replication"></a>Übersicht: Failovergruppen und aktive geografische Replikation
Mit der aktiven Georeplikation können Sie bis zu vier lesbare sekundäre Datenbanken in der gleichen oder in verschiedenen Standorten von Rechenzentren (Regionen) konfigurieren. Sekundäre Datenbanken stehen für Abfragen und Failover zur Verfügung, wenn ein Rechenzentrum ausgefallen ist oder keine Verbindung mit der primären Datenbank möglich ist. Das Failover muss durch die Anwendung des Benutzers manuell eingeleitet werden. Nach einem Failover hat die neue primäre Datenbank einen anderen Verbindungsendpunkt. 

> [!NOTE]
> Die aktive Georeplikation (lesbare sekundäre Datenbanken) ist für alle Datenbanken in allen Diensttarifen verfügbar.
>  

Gruppen für automatisches Failover in Azure SQL-Datenbank sind ein SQL-Datenbank-Feature für automatische Replikation, Konnektivität und automatisch skaliertes Failover. Damit erhalten Kunden die Möglichkeit, nach schwerwiegenden regionalen Ausfällen oder anderen ungeplanten Ereignissen, die zum vollständigen oder teilweisen Verlust der Verfügbarkeit der Dienste von SQL-Datenbank in der primären Region führen, automatisch mehrere verwandte Datenbanken in der sekundären Region wieder herzustellen. Außerdem können sie die lesbaren sekundären Datenbanken zur Auslagerung schreibgeschützter Workloads verwenden.  Da Gruppen für automatisches Failover mehrere Datenbanken beinhalten, müssen sie auf dem primären Server konfiguriert werden. Primärer und sekundärer Server müssen zu dem gleichen Abonnement gehören. Gruppen für automatisches Failover unterstützen die Replikation aller Datenbanken in der Gruppe auf nur einen sekundären Server in einer anderen Region. Die aktive Georeplikation erlaubt hingegen bis zu 4 sekundäre Server in beliebigen Regionen.
...
* **Toleranzperiode mit Datenverlust**: Da die primären und sekundären Datenbanken mithilfe der asynchronen Replikation synchronisiert werden, kann das Failover zu Datenverlust führen. Sie können die Richtlinie für automatisches Failover entsprechend der Toleranz Ihrer Anwendung gegenüber Datenverlust anpassen. Durch Konfiguration der Toleranzperiode mit Datenverlust können Sie steuern, wie lange das System wartet, bevor das Failover initiiert wird, das wahrscheinlich zu Datenverlust führt. 

   > [!NOTE]
   > Wenn das System erkennt, dass nach dem Ausfall die Datenbanken in der Gruppe noch immer online sind (wenn der Ausfall z.B. nur die Dienststeuerebene betroffen hat), aktiviert es sofort unabhängig von dem für die Toleranzperiode mit Datenverlust festgelegten Wert das Failover mit vollständiger Datensynchronisierung (freundliches Failover). Dies stellt sicher, dass während der Wiederherstellung kein Datenverlust auftritt. Die Toleranzperiode ist nur wirksam, wenn das freundliche Failover nicht möglich ist. Wenn der Ausfall vor Ablauf der Toleranzperiode behoben wird, wird das Failover nicht aktiviert.
   >
...
| [New-AzureRmSqlDatabaseFailoverGroup](https://docs.microsoft.com/powershell/module/azurerm.sql/set-azurermsqldatabasefailovergroup?view=azurermps-3.7.0) |    Dieser Befehl erstellt eine neue Failovergruppe und registriert sie auf primären und sekundären Servern.|
