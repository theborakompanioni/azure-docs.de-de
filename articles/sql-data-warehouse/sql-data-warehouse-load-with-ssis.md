<properties
   pageTitle="Laden von Daten mit SQL Server Integration Services (SSIS) | Microsoft Azure"
...
# Laden von Daten mit SQL Server Integration Services (SSIS)

In diesem Tutorial erfahren Sie, wie Sie ein SQL Server Integration Services (SSIS)-Paket erstellen, um Daten aus verschiedensten Datenquellen in SQL Data Warehouse zu verschieben. Im Rahmen des SSIS-Datenflusses können die Daten optional auch umstrukturiert, transformiert und bereinigt werden.
...
4. **Beispieldaten**. In diesem Tutorial werden als Quelldaten in SQL Server gespeicherte Beispieldaten aus der AdventureWorks-Beispieldatenbank verwendet, die in SQL Data Warehouse geladen werden sollen. Die AdventureWorks-Beispieldatenbank finden Sie unter [AdventureWorks 2014 Sample Databases][].
...
4. Wählen Sie **Integration Services-Projekt** aus. Geben Sie Werte für **Name** und **Speicherort** ein, und wählen Sie dann **OK** aus.
...
3. Ziehen Sie in der Toolbox aus der Liste mit anderen Quellen eine ADO.NET-Quelle auf die Entwurfsoberfläche. Vergewissern Sie sich, dass der Quelladapter immer noch ausgewählt ist, und ändern Sie den Namen im Bereich **Eigenschaften** in **SQL Server source**.
4. Ziehen Sie in der Toolbox aus der Liste mit anderen Quellen ein ADO.NET-Ziel auf die Entwurfsoberfläche, und platzieren Sie es unter der ADO.NET-Quelle. Vergewissern Sie sich, dass der Zieladapter immer noch ausgewählt ist, und ändern Sie den Namen im Bereich **Eigenschaften** in **SQL DW destination**.
...
4. Gehen Sie im Dialogfeld **Verbindungs-Manager** wie folgt vor:
    ...
    3. Wählen Sie im Abschnitt für die Serveranmeldung die Option für die SQL Server-Authentifizierung aus, und geben Sie die Authentifizierungsinformationen ein.
...
6. Klicken Sie im ADO.NET-Ziel-Editor neben der Liste zum Verwenden einer Tabelle oder Sicht auf **Neu**, um das Dialogfeld **Tabelle erstellen** zu öffnen und eine neue Zieltabelle mit einer der Quelltabelle entsprechenden Spaltenliste zu erstellen.
