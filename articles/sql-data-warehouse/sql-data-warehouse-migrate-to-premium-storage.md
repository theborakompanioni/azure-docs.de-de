---
title: Migrieren eines vorhandenen Azure SQL Data Warehouse in Storage Premium | Microsoft Docs
...
Für SQL Data Warehouse wurde vor Kurzem [Storage Premium eingeführt, um die Leistung besser vorhersagen zu können][Storage Premium eingeführt, um die Leistung besser vorhersagen zu können]. Wir sind jetzt bereit für das Migrieren vorhandener Data Warehouses, für die derzeit Storage Standard verwendet wird, zu Storage Premium. Im Folgenden erhalten Sie weitere Details dazu, wie und wann automatische Migrationen erfolgen und wie Sie die Migration selbst durchführen, um den Zeitpunkt der Ausfallzeiten zu steuern.

Wenn Sie mehr als ein Data Warehouse verwenden, ermitteln Sie anhand des unten angegebenen [Zeitplans für die automatische Migration][Zeitplans für die automatische Migration], wann die einzelnen Migrationen durchgeführt werden.

## Bestimmen des Speichertyps
Wenn Sie ein Data Warehouse vor den unten angegebenen Terminen erstellt haben, verwenden Sie derzeit Storage Standard. Für jedes Data Warehouse in Storage Standard, das der automatischen Migration unterliegt, wird außerdem im [Azure-Portal][Azure-Portal] oben im Blatt „Data Warehouse“ ein Hinweis mit folgendem Inhalt angezeigt: „*Ein bevorstehendes Upgrade auf Storage Premium ist mit Ausfallzeiten verbunden. Erfahren Sie mehr ->*.“
...
1. Benennen Sie „MyDW“ um, indem Sie den ALTER DATABASE-Befehl nach einer Zeichenfolge ähnlich dieser ausführen: MyDW\_BeforeMigration. Dieser Befehl löscht alle vorhandenen Transaktionen und muss in der Masterdatenbank ausgeführt werden, um erfolgreich zu sein.
   ...
   ```
2. [Anhalten][Anhalten]\: Halten Sie „MyDW\_BeforeMigration“ an, damit eine automatische Sicherung erstellt wird.
3. [Wiederherstellen][Wiederherstellen]\: Stellen Sie aus der jüngsten Momentaufnahme eine neue Datenbank mit dem vorherigen Namen wieder her (z.B. „MyDW“).
...
1. Data Warehouse sollte mit 1.000 DWUs oder mehr ausgeführt werden (siehe [Skalieren von Computeleistung][Skalieren von Computeleistung]).
2. Benutzer, die das Skript ausführen, sollten über die [Rolle „mediumrc“][Rolle „mediumrc“] oder höher verfügen.
...
Wenn Probleme mit Ihrem Data Warehouse auftreten, können Sie [ein Supportticket erstellen][ein Supportticket erstellen] und als möglichen Grund „Migration zu Storage Premium“ angeben.
