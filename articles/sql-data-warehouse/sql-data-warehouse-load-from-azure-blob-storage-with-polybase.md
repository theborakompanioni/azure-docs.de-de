...
Verwenden Sie zum Laden von Daten aus Azure Blob Storage, und zum Speichern der Daten in einer Tabelle innerhalb Ihrer Datenbank, die T-SQL-Anweisung [CREATE TABLE AS SELECT] [] (CTAS). Das Laden mit CTAS nutzt die stark typisierten externe Tabellen, die Sie gerade erstellt haben. Verwenden Sie eine [CTAS][]-Anweisung pro Tabelle, um die Daten in neue Tabellen zu laden.
...
Sie können den Ladefortschritt mithilfe der dynamischen Verwaltungsansicht (Dynamic Management View, DMV) `[sys].[dm_pdw_exec_requests]` verfolgen.
...
Standardmäßig speichert SQL Data Warehouse die Tabelle als gruppierten Columnstore-Index. Nach Abschluss eines Ladevorgangs sind einige der Datenzeilen möglicherweise nicht in den Columnstore-Index komprimiert. Es gibt zahlreiche Gründe, warum dies geschieht. Weitere Informationen finden Sie unter:
...
Weitere Informationen zum Verwalten von Columnstore-Indizes finden Sie im Artikel zum [Verwalten von Indizes] [].
...
Wenn Sie sich dazu entschließen, Statistiken für einzelne Spalten jeder Spalte jeder Tabelle zu erstellen, können Sie das Codebeispiel für die gespeicherte Prozedur `prc_sqldw_create_stats` im Artikel [Statistiken][] verwenden.
