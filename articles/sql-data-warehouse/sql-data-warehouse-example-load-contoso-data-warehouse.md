<properties
   pageTitle="Laden des Contoso Retail-Data Warehouse in SQL Data Warehouse | Microsoft Azure"
   description="Erfahren Sie, wie Sie Contoso Retail-Data Warehouse mit PolyBase in SQL Data Warehouse laden, um eine optimale Leistung zu erzielen."
...
# Laden des Contoso Retail-Data Warehouse

In diesem Artikel erfahren Sie, wie Sie Daten schnell in das Contoso Retail-Data Warehouse-Schema laden.

Dieses Dataset bietet Ihnen eine alternative Datenquelle, mit der Sie alles über Azure SQL Data Warehouse lernen können.

Das Contoso-Dataset ist ca. GB groß. Die unformatierten Daten werden in einem Azure-Blobspeicher gespeichert, auf den wir direkt zugreifen, um in SQL Data Warehouse zu laden.

In diesem Lernprogramm führen Sie folgende Schritte aus:

1. Konfigurieren von PolyBase zum Laden aus einem öffentlichen Blobspeichercontainer
2. Importieren der Daten mit [CTAS][]
3. [ERSTELLEN SIE DIE TABELLEN NEU][], um sicherzustellen, dass die Columnstore-Indizes optimiert werden.
4. Erstellen von Statistiken für Schlüsselspalten

## Konfigurieren von PolyBase

Der erste Schritt beim Laden von Daten ist das Konfigurieren der Skripts, die für PolyBase erforderlich sind:

### Erstellen der externen Datenquelle

Da die Daten in einem öffentlichen Container abgelegt werden, ist die erste Aufgabe das Erstellen der externen Datenquelle:
...
Es ist in diesem Fall nicht erforderlich, einen Hauptschlüssel oder datenbankweite Anmeldeinformationen zu erstellen, da der Blobspeichercontainer als „öffentlich“ konfiguriert ist, was bedeutet, dass jeder aus diesem Container lesen und auflisten kann.

> [AZURE.IMPORTANT] Wenn Sie Ihren Azure-Blobspeichercontainer öffentlich machen möchten, bedenken Sie, dass Ihnen Datenausgangsgebühren in Rechnung gestellt werden, wenn Daten das Rechenzentrum verlassen.

### Erstellen eines externen Dateiformatobjekts

Das Dateiformat bietet die erforderlichen Informationen zum Analysieren der Textdateien im Blobspeicher.
...
In unserem Fall sind die Daten nicht komprimiert und durch den senkrechten Strich getrennt.

## Erstellen von externen Tabellen

Da jetzt die Verbindung mit Azure-Blobspeicher konfiguriert ist, können wir nun die externen Tabellen erstellen.

Der Ordnung halber müssen wir zunächst ein Schema erstellen:
...
Nun erstellen wir die externen Tabellen. Beachten Sie, dass wir hier nur die Datentypenzuordnung bereitstellen – wird binden diese Definition an die Metadaten, um die Dateien zu suchen.

Warnung: Dieses Skript ist lang!
...
## Laden der Daten mit CTAS

Zum Laden der Daten sollten Sie die stark typisierten externen Tabellen nutzen, die wir gerade erstellt haben. Hierzu genügen einfache [CTAS][]-Anweisungen.

Allerdings sollten wir zunächst ein neues Schema zum Laden unsere Tabellen erstellen.
...
Zum Laden der Daten verwenden wir die [CTAS][]-Anweisung. Beachten Sie, dass es eine [CTAS][] pro Tabelle gibt.
...
Sie können den Fortschritt Ihres Ladens mithilfe der dynamischen Verwaltungsansicht (Dynamic Management View, DMV) `[sys].[dm_pdw_exec_requests]` verfolgen. Da die oben genannten CTAS-Anweisungen ein `label` haben, können Sie den Fortschritt auch mit `label` verfolgen. Weitere Informationen finden Sie im Artikel zu [Bezeichnungen][].

## Neuerstellen der Indizes

Um sicherzustellen, dass die Tabellen im komprimierten Columnstore-Format vorliegen, können wir schließlich eine Neuerstellung der Tabelle vornehmen. Damit werden alle offenen oder geschlossenen Zeilengruppen in das komprimierte Columnstore-Format konvertiert.
...
Weitere Informationen über die Überprüfung des Status Ihres gruppierten Columnstores finden Sie im Artikel zum [Verwalten von Columnstores][].

## Erstellen einiger Statistikobjekte

Um die Statistiken für alle Spalten zu erstellen, können Sie den Code der gespeicherten Prozedur nutzen `prc_sqldw_create_stats`, der im Artikel zu [Statistiken][] beschrieben wird.

Die Teilmenge der Ausgabe im Folgenden deckt die Dimensionen vollständig ab sowie alle verknüpften Spalten im Contoso Retail-Data Warehouse. Dies ist ein guter Ausgangspunkt.

Sie können später selbst anderen Faktenspalten weitere Statistiken hinzufügen.

Auch dieses Skript ist wieder recht lang.
...
Sie haben jetzt alle Daten in Azure SQL Data Warehouse geladen. Großartig!

Jetzt können Sie Abfragen wie die folgende auf die Tabellen anwenden:
...
<!--MSDN references-->
[sys.dm_pdw_exec_requests]: https://msdn.microsoft.com/de-DE/library/mt203887.aspx
[ERSTELLEN SIE DIE TABELLEN NEU]: https://msdn.microsoft.com/de-DE/library/ms188388.aspx
