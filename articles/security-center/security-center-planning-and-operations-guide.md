...
## Übersicht über Azure Security Center
...
## Planungshandbuch
Dieses Handbuch enthält eine Reihe von Schritten und Aufgaben, mit denen Sie die Verwendung von Azure Security Center auf der Grundlage der Sicherheitsanforderungen und des Cloudverwaltungsmodells Ihres Unternehmens optimieren können. Für eine optimale Nutzung von Azure Security Center ist es wichtig zu verstehen, wie der Dienst von den verschiedenen Personen oder Teams in Ihrer Organisation genutzt wird, um die Anforderungen in puncto sichere Entwicklung und sicherer Betrieb, Überwachung, Governance und Reaktionen auf Vorfälle zu erfüllen. Bei der Planung zur Verwendung von Azure Security Center sind insbesondere folgende Aspekte zu berücksichtigen:
...
Azure Security Center ermöglicht es den Personen, diese unterschiedlichen Aufgaben auszuführen. Beispiel:
...
Bei der Planung der Zugriffssteuerung mit RBAC für Azure Security Center müssen Sie zunächst ermitteln, wer in Ihrer Organisation Azure Security Center verwendet und welche Aufgaben diese Personen jeweils ausführen. Danach können Sie RBAC entsprechend konfigurieren.
...
**Virtuelles Netzwerk**: Azure Security Center überprüft anhand der Infrastruktur und Konfiguration Ihres [Azure Virtual Networks](https://azure.microsoft.com/documentation/services/virtual-network/), ob [Netzwerksicherheitsgruppen](../virtual-network/virtual-networks-nsg.md) angewendet wurden und ordnungsgemäß mit Regeln für eingehenden Datenverkehr konfiguriert sind. Überlegen Sie sich, welche Datenverkehrsregeln definiert werden müssen, und geben Sie diese Information an die Personen weiter, die die entsprechenden Sicherheitsempfehlungen umsetzen.
...
Nach Konfiguration und Anwendung der Azure Security Center-Empfehlungen müssen Sie sich Gedanken zu den Betriebsprozessen von Azure Security Center machen.

Klicken Sie im Azure-Portal auf **Durchsuchen**, und geben Sie im Feld **Filter** die Zeichenfolge **Security Center** ein, um Azure Security Center aufzurufen. Welche Ansichten dem Benutzer angezeigt werden, hängt von diesen angewendeten Filtern ab.
...
Wenn Sie Ihrer Azure-Umgebung neue Ressourcen (virtuelle Computer, SQL-Datenbanken) hinzufügen, erkennt Azure Security Center dies automatisch und beginnt mit der Überwachung ihrer Sicherheit. Wenn in der Sicherheitsrichtlinie die Datensammlung aktiviert ist, werden für Ihre virtuellen Computer automatisch zusätzliche Überwachungsfunktionen aktiviert.
...
3.	Auf virtuellen Computern, die der Umgebung neu hinzugefügt werden, ist zunächst häufig nur das Betriebssystem installiert. Der Ressourcenbesitzer benötigt unter Umständen etwas Zeit, um weitere Apps bereitzustellen, die von diesen virtuellen Computern verwendet werden. Im Idealfall sollten Sie den endgültigen Zweck der Workload kennen. Wird der Computer als Anwendungsserver verwendet? Auf der Grundlage des Zwecks der neuen Workload können Sie dann die passende **Sicherheitsrichtlinie** aktivieren. Das ist der dritte Schritt in diesem Workflow.
4.	Wenn der Azure-Umgebung neue Ressourcen hinzugefügt werden, werden auf der Kachel **Sicherheitswarnungen** unter Umständen neue Warnungen angezeigt. Prüfen Sie die Kachel immer auf neue Warnungen, und setzen Sie die Empfehlungen von Azure Security Center um.

Außerdem empfiehlt es sich, regelmäßig den Zustand vorhandener Ressourcen zu prüfen, um Konfigurationsänderungen zu identifizieren, die zu Sicherheitsrisiken geführt haben, nicht den empfohlenen Grundwerten entsprechen und Sicherheitswarnungen generiert haben. Navigieren Sie hierzu zunächst zum Azure Security Center-Dashboard. Hier gibt es drei Hauptbereiche, die Sie regelmäßig überprüfen sollten.
...
Azure Security Center erkennt Bedrohungen, wenn diese entstehen, und macht Sie darauf aufmerksam. Organisationen sollten eine Überwachung auf neue Sicherheitswarnungen implementieren und bei Bedarf geeignete Maßnahmen zur weiteren Untersuchung oder Abwehr des Angriffs ergreifen.

> [AZURE.NOTE] Dieser Artikel unterstützt Sie nicht bei der Erstellung eines Plans für Reaktionen auf Vorfälle. Zu diesem Zweck können Sie den [Computer Security Incident Handling Guide](http://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-61r2.pdf) des NIST (National Institute of Standards and Technology) verwenden.
...
| **Ebene** | **Angriffserkennung** | **Vorgeschlagene Lösung** |
|------------------|------------------------------------------|-----------------------------------------------------------------------------------------------|
| Netzwerk | Brute-Force-Erkennung (RDP) | Entfernen von nicht benötigten geöffneten Endpunkten zur Verringerung der Angriffsfläche, Konfigurieren von ACLs und Verwenden eines sicheren Kennworts |
| Virtuelle Computer | SVCHOST-Ausführung im falschen Verzeichnis | Verschieben des virtuellen Computers in ein anderes (isoliertes) Subnetz sowie Scannen und Neuerstellen |
| Anwendung | Einschleusung von SQL-Befehlen in die Azure-Datenbank | Erhöhen der Sicherheit der Datenbankkonfiguration |
