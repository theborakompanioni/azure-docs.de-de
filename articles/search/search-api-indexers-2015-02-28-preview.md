---
title: "Indexer-Vorgänge (REST-API für den Azure Search-Dienst: 2015-02-28-Preview) | Microsoft Docs"
...
## <a name="typical-usage-flow"></a>Typischer Verwendungsablauf
...
> [!NOTE]
> Die maximal zulässige Anzahl von Datenquellen variiert je nach Tarif. Der kostenlose Dienst kann bis zu 3 Datenquellen enthalten. Der Standard-Dienst kann 50 Datenquellen enthalten. In den [Einschränkungen für Dienste](search-limits-quotas-capacity.md) finden Sie weitere Informationen.
> 
> 
...
* `api-key`: Erforderlich. `api-key` wird zum Authentifizieren der Anforderung beim Search-Dienst verwendet. Es handelt sich um einen für Ihren Dienst eindeutigen Zeichenfolgewert. Die Anforderung **Datenquelle erstellen** muss einen `api-key`-Header enthalten, der auf Ihren Administratorschlüssel (im Gegensatz zum Abfrageschlüssel) festgelegt ist. 
...
**Richtlinien zur Erkennung von Datenänderungen**
...
**Richtlinien zur Erkennung von Datenlöschungen**
...
## <a name="update-data-source"></a>Datenquelle aktualisieren
...
`api-version` ist erforderlich. Die aktuelle Version ist `2015-02-28`. Unter [Azure Search-API-Versionen](https://msdn.microsoft.com/library/azure/dn864560.aspx) finden Sie nähere Angaben und weitere Informationen zu alternativen Versionen.

`api-key` muss ein Admin-Schlüssel sein (im Gegensatz zu einer Abfrageschlüssel). Weitere Informationen zu Schlüsseln finden Sie unter [REST-API für den Search-Dienst](https://msdn.microsoft.com/library/azure/dn798935.aspx) im Abschnitt "Authentifizierung". [Search-Dienst im Portal erstellen](search-create-service-portal.md) wird erläutert, wie Sie die Dienst-URL und Schlüsseleigenschaften, die in der Anforderung verwendet werden, abrufen können.
...
## <a name="list-data-sources"></a>Datenquellen auflisten
...
## <a name="get-data-source"></a>Datenquelle abrufen
...
> [!NOTE]
> Legen Sie beim Aufrufen dieser API den `Accept`-Anforderungsheader nicht auf `application/json;odata.metadata=none` fest. Andernfalls wird das `@odata.type`-Attribut nicht in der Antwort berücksichtigt, sodass Sie nicht zwischen den Richtlinien für die Erkennung geänderter und gelöschter Daten verschiedener Typen unterscheiden können. 
> 
> 

<a name="DeleteDataSource"></a>

## <a name="delete-data-source"></a>Datenquelle löschen
...
> [!NOTE]
> Wenn die zu löschende Datenquelle von einem Indexer referenziert wird, wird der Löschvorgang dennoch normal fortgesetzt. Der Indexer geht jedoch bei der nächsten Ausführung in den Fehlerstatus über.  
> 
> 
...
## <a name="create-indexer"></a>Indexer erstellen
...
## <a name="list-indexers"></a>Indexer auflisten
...
`api-version` ist erforderlich. Die Vorschauversion ist `2015-02-28-Preview`. [Versionsverwaltung für Azure Search](https://msdn.microsoft.com/library/azure/dn864560.aspx) finden Sie nähere Angaben und weitere Informationen zu alternativen Versionen.
...
## <a name="get-indexer"></a>Indexer abrufen
...
## <a name="get-indexer-status"></a>Indexer-Status abrufen
