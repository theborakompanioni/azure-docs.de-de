---
title: "Weiterleiten von Auftragsstatus und Auftragsdatenströmen von Automation an Log Analytics (OMS) | Microsoft Docs"
...
1. Ein OMS-Abonnement. Weitere Informationen finden Sie unter [Erste Schritte mit Log Analytics](../log-analytics/log-analytics-get-started.md).  
   ...
   > [!NOTE]
   > Damit diese Konfiguration ordnungsgemäß funktioniert, müssen der OMS-Arbeitsbereich und das Automation-Konto im selben Azure-Abonnement enthalten sein.
   >
   >
2. Ein [Azure Storage-Konto](../storage/storage-create-storage-account.md)  

   > [!NOTE]
   > Das Storage-Konto *muss* sich in derselben Region befinden wie das Automation-Konto.
   >
   >
...
## <a name="setup-integration-with-log-analytics"></a>Einrichten der Integration in Log Analytics
1. Starten Sie auf Ihrem Computer **Windows PowerShell** über den **Startbildschirm**.  
2. Navigieren Sie über die PowerShell-Befehlszeilenshell zu dem Ordner mit dem heruntergeladen Skript, und führen Sie es aus. Ändern Sie dabei die Werte für die Parameter *-AutomationAccountName* und *-LogAnalyticsWorkspaceName*.
...
3. Nach der Ausführung dieses Skripts sollten Sie etwa 30 Minuten, nachdem neue Diagnosedaten in den Speicher geschrieben worden sind, in Log Analytics Einträge sehen.  Wenn nach diesem Zeitraum keine Datensätze verfügbar sind, lesen Sie den Abschnitt zur Problembehandlung im Artikel [JSON-Dateien in Blob Storage](../log-analytics/log-analytics-azure-storage-json.md#troubleshooting-configuration-for-azure-diagnostic-logs), um weitere Informationen zu erhalten.

### <a name="verify-configuration"></a>Überprüfen der Konfiguration
Um zu bestätigen, dass das Skript Ihr Automation-Konto und Ihren OMS-Arbeitsbereich erfolgreich konfiguriert hat, können Sie in PowerShell folgende Schritte ausführen.  Bevor Sie mit diesen Schritten beginnen, navigieren Sie im Azure-Portal zu Log Analytics (OMS), um Ihren OMS-Arbeitsbereichsnamen und den Ressourcengruppennamen zu ermitteln. Notieren Sie sich die Werte für **Name** und **Ressourcengruppe** auf dem Blatt „Log Analytics (OMS)“.<br> ![OMS Log Analytics-Arbeitsbereichsliste](media/automation-manage-send-joblogs-log-analytics/oms-la-workspaces-list-blade.png): Diese beiden Werte werden verwendet, um die Konfiguration in Ihrem OMS-Arbeitsbereich mithilfe des PowerShell-Cmdlets [Get-AzureRmOperationalInsightsStorageInsight](https://msdn.microsoft.com/library/mt603567.aspx) zu überprüfen.

1. Navigieren Sie im Azure-Portal zu den Speicherkonten, und suchen Sie nach folgendem Speicherkonto, bei dem die Namenskonvention befolgt wird: *AutomationAccountNameomsstorage*.  Kurz nach Abschluss eines Runbookauftrags werden zwei Blobcontainer erstellt: **insights-logs-joblogs** und **insights-logs-jobstreams**.  
2. Führen Sie in PowerShell den folgenden PowerShell-Code aus, und ändern Sie dabei die Werte für die zuvor kopierten oder notierten Parameter **ResourceGroupName** und **WorkspaceName**.  
   ...
   Dieser Befehl gibt Speicherdetails für den angegebenen OMS-Arbeitsbereich zurück.  Es soll überprüft werden, ob die zuvor angegebenen Speicherdetails für das Automation-Konto vorhanden sind und ob der Wert des **State**-Objekts **OK** lautet.<br> ![Ergebnisse des Cmdlets „Get-AzureRmOperationalInsightsStorageInsights“](media/automation-manage-send-joblogs-log-analytics/automation-posh-getstorageinsights-results.png).
