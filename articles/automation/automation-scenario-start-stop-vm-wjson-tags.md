...
4. Wählen Sie auf dem Blatt **Zeitpläne** die Option **Zeitplan mit Runbook verknüpfen**, und wählen Sie auf dem Blatt **Zeitplan** die Option **Neuen Zeitplan erstellen**.
5.  Geben Sie auf dem Blatt **Neuer Zeitplan** den Namen dieses Zeitplans ein. Beispiel: HourlyExecution.
6. Legen Sie für den **Start**-Zeitplan die Startzeit auf eine Stunde gerundet fest.
7. Wählen Sie **Wiederholung** und für das **Erneut alle**-Intervall **1 Stunde**.
8. Stellen Sie sicher, dass **Ablaufdatum festlegen ** auf **Nein** festgelegt ist, und klicken Sie dann auf **Erstellen**, um den neuen Zeitplan zu speichern.
9. Wählen Sie auf dem Optionenblatt **Runbook planen** die Option **Parameter und Ausführungseinstellungen**, und geben Sie auf dem Test-ResourceSchedule-Blatt **Parameter** den Namen Ihres Abonnements in das Feld **SubscriptionName** ein. Dies ist der einzige für das Runbook erforderliche Parameter. Klicken Sie zum Abschluss auf **OK**.
   

Der Runbookzeitplan sollte nach Abschluss folgendermaßen aussehen:<br> ![Konfiguriertes Test-ResourceSchedule-Runbook](./media/automation-scenario-start-stop-vm-wjson-tags/automation-schedule-config.png)<br>
...
2. Wählen Sie eine VM oder eine Ressourcengruppe aus, um diesen Zeitplan durch Auswahl des Tag-Symbols anzuwenden.<br>![VM-Tagoption](./media/automation-scenario-start-stop-vm-wjson-tags/automation-vm-tag-option.png)
...
-  Weitere Informationen zur PowerShell-Skriptunterstützung finden Sie unter [Announcing Native PowerShell Script Support in Azure Automation](https://azure.microsoft.com/blog/announcing-powershell-script-support-azure-automation-2/) (Ankündigung der Native PowerShell-Skriptunterstützung in Azure Automation).
