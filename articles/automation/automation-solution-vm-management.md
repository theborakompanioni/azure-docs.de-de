...
Anzeigen des Auftragsstatus in Abhängigkeit der Zeit für die Runbooks „StartVM“ und „StopVM“ | Category=JobLogs RunbookName_s="StartByResourceGroup-MS-Mgmt-VM" OR "StopByResourceGroup-MS-Mgmt-VM" NOT(ResultType="started") | measure Count() by ResultType interval 1day|
...
1.  Wählen Sie in Ihrem Automation-Konto die Kachel **Lösungen** aus.  
2.  Wählen Sie auf dem Blatt **Lösungen** die Lösung **Start-Stop-VM[Workspace]** aus.  Klicken Sie auf dem Blatt **VMManagementSolution[Workspace]** im Menü auf **Löschen**.<br><br> ![Löschen der VM-Mgmt-Lösung](media/automation-solution-vm-management/vm-management-solution-delete.png)
3.  Bestätigen Sie im Fenster **Lösung löschen**, dass Sie die Lösung löschen möchten.
4.  Während die Informationen überprüft werden und die Lösung gelöscht wird, können Sie den Fortschritt im Menü unter **Benachrichtigungen** nachverfolgen.  Nachdem der Vorgang zum Löschen der Lösung begonnen hat, kehren Sie zum Blatt **VMManagementSolution[Workspace]** zurück.  

Das Automation-Konto und der OMS-Arbeitsbereich werden im Rahmen dieses Vorgangs nicht gelöscht.  Wenn Sie den OMS-Arbeitsbereich nicht beibehalten möchten, müssen Sie ihn manuell löschen.  Dies kann auch im Azure-Portal erfolgen.   Wählen Sie im Azure-Portal auf der Startseite **Log Analytics** und dann auf dem Blatt **Log Analytics** den Arbeitsbereich aus. Klicken Sie dann auf dem Blatt mit den Arbeitsbereichseinstellungen im Menü auf **Löschen**.  
