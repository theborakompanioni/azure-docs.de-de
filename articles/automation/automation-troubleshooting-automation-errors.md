...
**Fehler:** Für den Knoten wurde ein Bericht ausgegeben mit einem **Fehlerstatus** und der Fehlermeldung „Fehler beim Versuch, die Aktion vom Server https://``<url>``//accounts/``<konto-id>``/Nodes(AgentId=``<agent-id>``)/GetDscAction failed because a valid configuration ``<guid>`` wurde nicht gefunden.“
