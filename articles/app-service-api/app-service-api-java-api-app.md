---
title: Erstellen und Bereitstellen einer Java-API-App in Azure App Service
...
---
# <a name="build-and-deploy-a-java-api-app-in-azure-app-service"></a>Erstellen und Bereitstellen einer Java-API-App in Azure App Service
...
2. [Maven] auf dem Entwicklungscomputer
3. [Git] auf dem Entwicklungscomputer
4. Ein kostenpflichtiges Abonnement oder eine [kostenlose Testversion] von [Microsoft Azure]
5. Eine HTTP-Testanwendung, z.B. [Postman]

## <a name="scaffold-the-api-using-swaggerio"></a>Erstellen des Gerüsts für die API mit Swagger.IO
...
2. Navigieren Sie zum [Online-Swagger-Editor]. Klicken Sie dort auf das Menüelement **Datei -> JSON** einfügen.
   
    ![Menüelement „JSON einfügen“][paste-json]
3. Fügen Sie den zuvor kopierten Swagger-JSON-Code der Kontaktlisten-API ein. 
   
    ![JSON-Code in Swagger einfügen][pasted-swagger]
4. Zeigen Sie die im Editor gerenderten Dokumentationsseiten und die API-Zusammenfassung an. 
   
    ![Mit Swagger generierte Dokumente anzeigen][view-swagger-generated-docs]
5. Wählen Sie die Menüoption **Server erstellen -> JAX-RS**, um den serverseitigen Code zu erstellen, den Sie später zum Hinzufügen einer Modellimplementierung bearbeiten. 
...
## <a name="edit-the-code-to-add-api-implementation"></a>Bearbeiten des Codes zum Hinzufügen der API-Implementierung
...
3. Öffnen Sie die Dienstimplementierungsdatei *ContactsApiServiceImpl.java*, die im Ordner *src/main/java/io/swagger/api/imp*l enthalten ist, mit [Visual Studio Code] oder Ihrem bevorzugten Text-Editor.
   
    ![Datei mit Kontaktdienstcode öffnen][open-contact-service-code-file]
4. Überschreiben Sie den Code in der Datei mit diesem neuen Code, um dem Dienstcode eine Modellimplementierung hinzuzufügen. 
...
5. Öffnen Sie ein Eingabeaufforderungsfenster, und ändern Sie das Verzeichnis in den Stammordner Ihrer Anwendung.
6. Erstellen Sie den Code mit dem folgenden Maven-Befehl, und führen Sie ihn mit dem Jetty-App-Server lokal aus. 
   
        mvn package jetty:run
7. Daraufhin sollte im Befehlsfenster angezeigt werden, dass Jetty den Code an Port 8080 gestartet hat: 
   
    ![Datei mit Kontaktdienstcode öffnen][run-jetty-war]
8. Senden Sie mit [Postman] eine Anforderung an die API-Methode „get all contacts“ unter „http://localhost:8080/api/contacts“.
   
    ![Kontakte-API aufrufen][calling-contacts-api]
9. Senden Sie mit [Postman] eine Anforderung an die API-Methode „get specific contact“ unter „http://localhost:8080/api/contacts/2“.
   
    ![Kontakte-API aufrufen][calling-specific-contact-api]
10. Erstellen Sie abschließend die Java WAR-Datei (Web ARchive), indem Sie folgenden Maven-Befehl in der Konsole ausführen. 
    
         mvn package war:war
11. Die WAR-Datei wird nach ihrer Erstellung im Ordner **target** abgelegt. Navigieren Sie zum Ordner **target**, und benennen Sie die WAR-Datei in **ROOT.war** um. (Stellen Sie sicher, dass die Groß-/Kleinschreibung für dieses Format geeignet ist.)
    
          rename swagger-jaxrs-server-1.0.0.war ROOT.war
12. Führen Sie abschließend die folgenden Fehler im Stammordner Ihrer Anwendung aus, um den Ordner **deploy** zum Bereitstellen der WAR-Datei für Azure zu erstellen. 
...
## <a name="publish-the-output-to-azure-app-service"></a>Veröffentlichen der Ausgabe in Azure App Service
...
6. Übertragen Sie die WAR-Datei per Git Push an das Online-Repository. Navigieren Sie dazu zum zuvor erstellten Ordner **deploy**, damit Sie einfach ein Commit für den Code im Repository in App Service ausführen können. Navigieren Sie im Konsolenfenster zum Ordner, in dem sich der Ordner „webapps“ befindet. Geben Sie dort die folgenden Git-Befehle ein, um den Prozess zu starten und eine Bereitstellung auszulösen. 
...
Weitere Informationen zum Verwenden von Azure mit Java finden Sie unter [Azure for Java developers - Tutorials, API Reference](/java/azure) (Azure für Java-Entwickler – Tutorials, API-Referenz).
...
[Storage SDK für Java]:../storage/blobs/storage-java-how-to-use-blob-storage.md
