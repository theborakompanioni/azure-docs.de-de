---
title: "CORS-Unterstützung in App Service | Microsoft Docs"
...
---
# <a name="consume-an-api-app-from-javascript-using-cors"></a>Nutzen einer API-App aus JavaScript mit CORS
...
3. Suchen Sie auf dem Blatt **Einstellungen**, das rechts neben dem Blatt **API-App** angezeigt wird, den Abschnitt **API**, und klicken Sie dann auf **CORS**.
...
Sie können CORS für eine API-App auch konfigurieren, indem Sie [Azure Resource Manager-Vorlagen](../resource-group-authoring-templates.md) in Befehlszeilentools verwenden, z.B. [Azure PowerShell](/powershell/azureps-cmdlets-docs) und die [Azure-Befehlszeilenschnittstelle](../xplat-cli-install.md). 
...
## <a name="deploy-the-todolistangular-project-to-a-new-web-app"></a>Bereitstellen des Projekts ToDoListAngular für eine neue Web-App
...
4. Geben Sie im Dialogfeld **App Service erstellen** auf der Registerkarte **Hosten** einen **Web-App-Namen** ein, der in der Domäne *azurewebsites.net* eindeutig ist. 
...
7. Wählen Sie in der Dropdownliste **App Service-Plan** den Plan aus, den Sie bereits erstellt haben. 
...
### <a name="set-the-middle-tier-url-in-web-app-settings"></a>Festlegen der URL für die mittlere Ebene in den Web-App-Einstellungen
...
3. Fügen Sie im Abschnitt **App-Einstellungen** folgenden Schlüssel und Wert hinzu:
   ...
   | --- | --- | --- |
   | toDoListAPIURL |https://{Name Ihrer API-App der mittleren Ebene}.azurewebsites.net |https://todolistapi0121.azurewebsites.net |
...
### <a name="deploy-the-todolistangular-web-project-to-the-new-web-app"></a>Bereitstellen des Webprojekts ToDoListAngular für die neue Web-App
...
### <a name="test-the-application-without-cors-enabled"></a>Testen der Anwendung ohne Aktivierung von CORS
1. Öffnen Sie im Entwicklertools-Browser das Konsolenfenster.
2. Klicken Sie im Browserfenster, in dem die AngularJS-Benutzeroberfläche angezeigt wird, auf den Link **Aufgabenliste** .
...
## <a name="configure-cors-for-the-middle-tier-api-app"></a>Konfigurieren Sie CORS für die API-App der mittleren Ebene
...
4. Geben Sie im Textfeld die URL für die Web-App ToDoListAngular (Front-End) ein. Beispiel: Wenn Sie das Projekt ToDoListAngular für eine Web-App mit dem Namen „todolistangular0121“ bereitgestellt haben, können Sie Aufrufe über die URL `https://todolistangular0121.azurewebsites.net`zulassen.
...
### <a name="test-the-application-with-cors-enabled"></a>Testen der Anwendung mit Aktivierung von CORS
...
### <a name="how-to-enable-cors-in-web-api-code"></a>Gewusst wie: Aktivieren von CORS im Web-API-Code
...
2. Fügen Sie wie im folgenden Beispiel gezeigt eine `config.EnableCors()`-Codezeile in die **Register**-Methode der **WebApiConfig**-Klasse ein. 
...
3. Fügen Sie im Web-API-Controller eine `using`-Anweisung für den `System.Web.Http.Cors`-Namespace und der Controllerklasse oder einzelnen Aktionsmethoden das `EnableCors`-Attribut hinzu. Im folgenden Beispiel gilt die CORS-Unterstützung für den gesamten Controller.
...
* [Azure API Management Overview](https://azure.microsoft.com/documentation/videos/azure-api-management-overview/)
