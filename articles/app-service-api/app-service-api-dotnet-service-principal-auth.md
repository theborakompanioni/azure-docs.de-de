---
title: "Dienstprinzipalauthentifizierung für API-Apps in Azure App Service | Microsoft-Dokumentation"
...
---
# <a name="service-principal-authentication-for-api-apps-in-azure-app-service"></a>Dienstprinzipalauthentifizierung für API-Apps in Azure App Service
## <a name="overview"></a>Übersicht
In diesem Artikel wird erläutert, wie Sie die App Service-Authentifizierung für den *internen* Zugriff auf API-Apps verwenden. Ein Beispiel für ein internes Szenario ist die Verwendung einer API-App, die nur für Ihren eigenen Anwendungscode nutzbar sein soll. Die empfohlene Methode zum Implementieren dieses Szenarios in App Service ist die Verwendung von Azure AD zum Schützen der aufgerufenen API-App. Sie rufen die geschützte API-App mit einem Bearertoken auf, das Sie über Azure AD erhalten, indem Sie App-Identität-Anmeldeinformationen (Dienstprinzipal) bereitstellen. Alternativen zum Verwenden von Azure AD werden im Abschnitt **Dienst-zu-Dienst-Authentifizierung** der [Übersicht über die Authentifizierung in Azure App Service](../app-service/app-service-authentication-overview.md#service-to-service-authentication).

In diesem Artikel lernen Sie Folgendes:

* Verwenden von Azure Active Directory (Azure AD) zum Schützen einer API-App vor Zugriff ohne Authentifizierung
* Nutzen einer geschützten API-App aus einer API-App, Web-App oder mobilen App mit Azure AD-Dienstprinzipal-Anmeldeinformationen (App-Identität) Weitere Informationen dazu, wie Sie Daten einer Logik-App nutzen, finden Sie unter [Verwenden der in App Service gehosteten benutzerdefinierten API mit Logik-Apps](../logic-apps/logic-apps-custom-hosted-api.md).
...
4. Wählen Sie unter **Authentifizierungsanbieter** die Option **Azure Active Directory** aus.
   
    ![Blatt „Authentifizierung/Autorisierung“ im Azure-Portal](./media/app-service-api-dotnet-user-principal-auth/authblade.png)
5. Konfigurieren Sie das Blatt **Azure Active Directory-Einstellungen** , um eine neue Azure AD-Anwendung zu erstellen, oder verwenden Sie eine vorhandene Azure AD-Anwendung, falls Sie bereits über eine geeignete Anwendung verfügen.
...
6. Wenn Sie mit dem Blatt für die Konfiguration des Authentifizierungsanbieters fertig sind, klicken Sie auf **OK**.
...
#### <a name="how-to-consume-the-protected-api-app"></a>Nutzen der geschützten API-App
...
#### <a name="how-to-protect-the-api-app-from-access-by-users-in-the-same-tenant"></a>Schützen der API-App vor dem Zugriff von Benutzern in demselben Mandanten
...
## <a name="set-up-authentication-in-azure"></a>Einrichten der Authentifizierung in Azure
...
4. Wählen Sie in der Dropdownliste **Die auszuführende Aktion, wenn die Anforderung nicht authentifiziert ist** die Option **Mit Azure Active Directory anmelden**.
...
6. Klicken Sie auf dem Blatt **Azure Active Directory-Einstellungen** auf **Express**.
    ...
    Mit der Option **Express** kann von Azure automatisch eine AAD-Anwendung in Ihrem Azure AD-[Mandanten](https://msdn.microsoft.com/en-us/library/azure/jj573650.aspx#BKMK_WhatIsAnAzureADTenant) erstellt werden. 
...
### <a name="verify-that-the-api-app-is-protected"></a>Sicherstellen des Schutzes für die API-App
...
2. Melden Sie sich mit den Anmeldeinformationen eines Benutzers an Ihrem AAD-Mandanten an.
   
   Nachdem Sie sich angemeldet haben, wird im Browser die Seite mit dem Hinweis zur erfolgreichen Erstellung angezeigt.

## <a name="configure-the-todolistapi-project-to-acquire-and-send-the-azure-ad-token"></a>Konfigurieren des Projekts ToDoListAPI zum Abrufen und Senden des Azure AD-Tokens
...
### <a name="get-azure-ad-configuration-values"></a>Abrufen von Azure AD-Konfigurationswerten
...
4. Klicken Sie in der Liste mit den Anwendungen auf den Namen der Anwendung, die von Azure für Sie erstellt wurde, als Sie die Authentifizierung für die API-App ToDoListDataAPI (Datenebene) aktiviert haben.
5. Klicken Sie auf die Registerkarte **Konfigurieren** .
6. Kopieren Sie den Wert **Client-ID** , und speichern Sie ihn an einem Ort, an dem Sie später darauf zugreifen können. 
7. Kehren Sie im klassischen Azure-Portal zur Liste **Anwendungen im Besitz meines Unternehmens**zurück, und klicken Sie auf die AAD-Anwendung, die Sie im vorherigen Tutorial (nicht in diesem Tutorial) für die API-App ToDoListAPI der mittleren Ebene erstellt haben.
8. Klicken Sie auf die Registerkarte **Konfigurieren** .
9. Kopieren Sie den Wert **Client-ID** , und speichern Sie ihn an einem Ort, an dem Sie später darauf zugreifen können.
10. Wählen Sie unter **Schlüssel** in der Dropdownliste **Dauer auswählen** die Option **1 Jahr** aus.
11. Klicken Sie auf **Speichern**.
    
     ![App-Schlüssel generieren](./media/app-service-api-dotnet-service-principal-auth/genkey.png)
12. Kopieren Sie den Schlüsselwert, und speichern Sie ihn an einem Ort, an dem Sie später darauf zugreifen können.
    
     ![Neuen App-Schlüssel kopieren](./media/app-service-api-dotnet-service-principal-auth/genkeycopy.png)

### <a name="configure-azure-ad-settings-in-the-middle-tier-api-apps-runtime-environment"></a>Konfigurieren von Azure AD-Einstellungen in der Laufzeitumgebung der API-App für die mittlere Ebene
1. Navigieren Sie zum [Azure-Portal](https://portal.azure.com/)und dann zum Blatt **API-App** für die API-App, die das Projekt ToDoListAPI (mittlere Ebene) hostet.
...
3. Fügen Sie im Abschnitt **App-Einstellungen** die folgenden Schlüssel und Werte hinzu:
   ...
   | --- | --- |
   | **Wert** |https://login.microsoftonline.com/{Name Ihres Azure AD-Mandanten} |
...
3. Fügen Sie Aufgabenelemente hinzu, um zu überprüfen, ob die Anwendung funktioniert.
...
## <a name="protect-the-api-app-from-browser-access"></a>Schützen der API-App vor dem Browserzugriff
...
### <a name="disable-browser-access"></a>Deaktivieren des Browserzugriffs
...
### <a name="verify-browser-access-no-longer-works"></a>Prüfen, ob der Browserzugriff nicht mehr funktioniert
...
## <a name="restrict-access-to-a-particular-service-principal"></a>Einschränken des Zugriffs auf einen bestimmten Dienstprinzipal
...
1. Öffnen Sie die Datei *Controllers/TodoListController.cs* .
2. Heben Sie die Auskommentierung der Zeilen auf, mit denen `trustedCallerClientId` und `trustedCallerServicePrincipalId` festgelegt werden.
   
        private static string trustedCallerClientId = ConfigurationManager.AppSettings["todo:TrustedCallerClientId"];
        private static string trustedCallerServicePrincipalId = ConfigurationManager.AppSettings["todo:TrustedCallerServicePrincipalId"];
3. Heben Sie die Auskommentierung des Codes in der CheckCallerId-Methode auf. Diese Methode wird zu Beginn jeder Aktionsmethode im Controller aufgerufen. 
...
4. Stellen Sie das Projekt ToDoListDataAPI erneut in Azure App Service bereit.
5. Greifen Sie in Ihrem Browser auf die HTTPS-URL der AngularJS-Front-End-Web-App zu, und klicken Sie auf der Startseite auf die Registerkarte **Aufgabenliste** .
...
6. Mit der Azure AD PowerShell rufen Sie den Wert des Dienstprinzipals für die Azure AD-Anwendung ab, die Sie für das Projekt TodoListWebApp erstellt haben.
...
8. Klicken Sie auf **Einstellungen > Anwendungseinstellungen**.
9. Fügen Sie im Abschnitt **App-Einstellungen** die folgenden Schlüssel und Werte hinzu:
...
11. Kehren Sie im Browser zur URL der Web-App zurück, und klicken Sie auf der Startseite auf die Registerkarte **Aufgabenliste** .
