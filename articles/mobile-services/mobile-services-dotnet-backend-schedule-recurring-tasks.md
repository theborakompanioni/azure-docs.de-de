---
title: Planen von Back-End-Aufgaben in einen mobilen .NET-Back-End-Dienst | Microsoft Docs
...
2. Ersetzen Sie den Inhalt der "SampleJob.cs" durch den folgenden Code:
    ...
    Im oben angegebenen Code müssen Sie die Zeichenfolgen *todolistService* und *todolistContext* durch den Namespace und den DbContext Ihres heruntergeladenen Projekts ersetzen. Dabei handelt es sich in diesem Fall um den Namespace *mobile&#95;service&#95;name*Service bzw. um den Kontext *mobile&#95;service&#95;name*Context.
...
2. Klicken Sie auf **Testen Sie es**, und klicken Sie dann auf **POST jobs/{jobName}**.
    ...
    ![][8]
3. Klicken Sie auf **Ausprobieren**, geben Sie `Sample` als Parameterwert für **{jobName}** ein, und klicken Sie auf **Senden**.
...
2. Klicken Sie im [klassischen Azure-Portal] auf Mobile Services und dann auf Ihre App.
3. Klicken Sie auf der Registerkarte **Scheduler** auf **+Create**.
   
   > [!NOTE]
   > Wenn Sie Ihren mobilen Dienst auf der Ebene <em>Free</em> ausführen, können Sie zu einem gegebenen Zeitpunkt jeweils nur einen geplanten Auftrag ausführen. Bei bezahlten Ebenen können Sie bis zu zehn geplante Aufträge zur Zeit ausführen.
   > 
   > 
4. Geben Sie im Scheduler-Dialogfeld die Zeichenfolge *Sample* für **Auftragsname** ein. Legen Sie das Planintervall und die Planeinheiten fest, und klicken Sie anschließend auf das Häkchen.
...
5. Klicken Sie auf den gerade erstellten neuen Auftrag und danach auf **Einmal ausführen**, um das Skript zu testen.
   ...
   > [!NOTE]
   > Eine POST-Anforderung kann weiterhin verwendet werden, um den geplanten Auftrag zu starten. Die Autorisierung ist jedoch für den Benutzer voreingestellt. Dies bedeutet, dass die Anforderung den Anwendungsschlüssel im Header enthalten muss.
   > 
   > 
6. (Optional) Klicken Sie im [klassischen Azure-Portal] auf die Option zum Verwalten der Datenbank, die mit Ihrem mobilen Dienst verknüpft ist.
   
    ![][6]
7. Führen Sie im klassischen Azure-Portal eine Abfrage aus, um die von der App vorgenommenen Änderungen anzuzeigen. Ihre Abfrage ist mit der folgenden Abfrage vergleichbar, allerdings wird anstelle von `todolist` der Name Ihres mobilen Diensts als Schemaname verwendet.
