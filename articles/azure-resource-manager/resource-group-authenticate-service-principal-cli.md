---
title: Erstellen eines Dienstprinzipals mithilfe der Azure-Befehlszeilenschnittelle | Microsoft Docs
...
2. Ihnen stehen zwei Optionen zum Erstellen der AD-Anwendung zur Verfügung. Sie können die AD-Anwendung und den Dienstprinzipal entweder in einem Schritt oder separat erstellen. Erstellen Sie sie in einem Schritt, wenn Sie keine Startseite und Bezeichner-URIs für Ihre App angeben müssen. Erstellen Sie sie getrennt, wenn Sie diese Werte für eine Web-App festlegen müssen. Beide Optionen werden in diesem Schritt veranschaulicht.
   ...
   * Um die AD-Anwendung getrennt zu erstellen, geben Sie den Namen der App, einen Startseiten-URI, Bezeichner-URIs und ein Kennwort an (wie im folgenden Befehl gezeigt):
     ...
     Bei beiden Optionen wird der neue Dienstprinzipal zurückgegeben. Die **Objekt-ID** wird beim Erteilen von Berechtigungen benötigt. Die mit den **Dienstprinzipalnamen** aufgeführte GUID wird bei der Anmeldung benötigt. Diese GUID hat den gleichen Wert wie die App-ID. In den Beispielanwendungen wird dieser Wert als **Client-ID** bezeichnet. 
...
5. Ihnen stehen zwei Optionen zum Erstellen der AD-Anwendung zur Verfügung. Sie können die AD-Anwendung und den Dienstprinzipal entweder in einem Schritt oder separat erstellen. Erstellen Sie sie in einem Schritt, wenn Sie keine Startseite und Bezeichner-URIs für Ihre App angeben müssen. Erstellen Sie sie getrennt, wenn Sie diese Werte für eine Web-App festlegen müssen. Beide Optionen werden in diesem Schritt veranschaulicht.
   ...
   * Um die AD-Anwendung getrennt zu erstellen, geben Sie den Namen der App, einen Startseiten-URI, Bezeichner-URIs und die Zertifikatdaten an (wie im folgenden Befehl gezeigt):
     ...
     Bei beiden Optionen wird der neue Dienstprinzipal zurückgegeben. Die Objekt-ID wird beim Gewähren von Berechtigungen benötigt. Die mit den **Dienstprinzipalnamen** aufgeführte GUID wird bei der Anmeldung benötigt. Diese GUID hat den gleichen Wert wie die App-ID. In den Beispielanwendungen wird dieser Wert als **Client-ID** bezeichnet. 
