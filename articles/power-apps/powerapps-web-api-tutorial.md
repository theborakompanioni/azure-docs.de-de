<properties
	pageTitle="Tutorial: Erstellen einer benutzerdefinierten API mit einer ASP.Net-Web-App in PowerApps und Logic Flows | Microsoft Azure"
...
7. Extrahieren Sie den Swagger-JSON-Inhalt, indem Sie zu ***https://\<Azure-Webapp-URL>/swagger/docs/v1*** navigieren.
	...
	> [AZURE.IMPORTANT] Ein Swagger-Dokument mit doppelten Vorgangs-ID(s) ist ungültig. Wenn Sie die C#-Beispielvorlage verwenden, wird die Vorgangs-ID „Values\_Get“ zweimal wiederholt. Ändern Sie eine Instanz in „Value\_Get“.
...
3. Verwenden Sie für **webAPI** die folgende Konfiguration:  
  ...
  3. Antwort-URL: ***https://\<Ihre-Stamm-URL>/.auth/login/aad/callback***  
	...
	>[AZURE.IMPORTANT] Sie benötigen die Client-ID dieser App später, darum notieren Sie sie.
...
>[AZURE.IMPORTANT] Beide Apps müssen sich im gleichen Verzeichnis befinden.
...
2. Wählen Sie in **Einstellungen** die Option **„Authentifizierung/Autorisierung“**.
...
2. Wechseln Sie zum PowerApps-[Webportal][1], und fügen Sie eine benutzerdefinierte API hinzu. In [What are Custom APIs](powerapps-register-custom-api.md) (Was sind benutzerdefinierte APIs?) werden die Schritte aufgelistet.
