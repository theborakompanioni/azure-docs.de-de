<properties
   pageTitle="SAP NetWeaver auf virtuellen Linux-Computern (VMs) – Bereitstellungshandbuch | Microsoft Azure"
...
[deployment-guide-3.4]: virtual-machines-linux-sap-deployment-guide.md#a9a60133-a763-4de8-8986-ac0fa33aa8c1 "Szenario 3: Verschieben eines lokalen virtuellen Computers mithilfe einer nicht generalisierten VHD mit SAP"
...
[deployment-guide-4.4]: virtual-machines-linux-sap-deployment-guide.md#c7cbb0dc-52a4-49db-8e03-83e7edc2927d "Herunterladen, Installieren und Aktivieren des Azure VM-Agents"
[deployment-guide-4.4.2]: virtual-machines-linux-sap-deployment-guide.md#6889ff12-eaaf-4f3c-97e1-7c9edc7f7542 "Linux"
[deployment-guide-4.5]: virtual-machines-linux-sap-deployment-guide.md#d98edcd3-f2a1-49f7-b26a-07448ceb60ca "Konfigurieren der Azure-Erweiterung zur verbesserten Überwachung für SAP"
...
[deployment-guide-5.1]: virtual-machines-linux-sap-deployment-guide.md#bb61ce92-8c5c-461f-8c53-39f5e5ed91f2 "Bereitschaftsüberprüfung für die verbesserte Azure-Überwachung für SAP"
...
[planning-guide-5.2.1]: virtual-machines-linux-sap-planning-guide.md#1b287330-944b-495d-9ea7-94b83aff73ef "Vorbereitung des Verschiebens eines lokalen virtuellen Computers mit einem nicht generalisierten Datenträger in Azure"
...
[planning-guide-5.3.2]: virtual-machines-linux-sap-planning-guide.md#a43e40e6-1acc-4633-9816-8f095d5a7b6a "Hochladen einer virtuellen Festplatte (VHD) vom lokalen Standort in Azure"
...
#### Internetverbindung
Für das Herunterladen und Ausführen der erforderlichen Tools und Skripts ist ein Internetzugang erforderlich. Zudem benötigt auch der virtuelle Microsoft Azure-Computer, der die Azure-Erweiterung zur verbesserten Überwachung ausführt, Zugang zum Internet. Falls dieser virtuelle Azure-Computer Teil eines virtuellen Azure-Netzwerks oder einer lokalen Domäne ist, achten Sie auf eine korrekte Einrichtung der entsprechenden Proxyeinstellungen. Diese werden im Kapitel [Konfigurieren des Proxys][deployment-guide-configure-proxy] in diesem Dokuments beschrieben.
...
#### Ressourcengruppen
Ressourcengruppen sind ein neues Konzept. Sie enthalten jeweils alle Ressourcen mit dem gleichen Lebenszyklus, da sie z.B. zur gleichen Zeit erstellt und gelöscht werden. Weitere Informationen über Ressourcengruppen finden Sie [in diesem Artikel][resource-group-overview].
...
* SAP-spezifische Azure-CLI-Befehle, die Teil der [Azure-CLI][azure-cli] sind
...
## <a name="b3253ee3-d63b-4d74-a49b-185e76c4088e"></a>Bereitstellungsszenarios für virtuelle Computer für SAP in Microsoft Azure
...
### Bereitstellung von virtuellen Computern für SAP
...
#### Verschieben eines virtuellen lokalen Computers nach Microsoft Azure mit einem nicht generalisierten Datenträger
...
#### Erstellen eines virtuellen Computer über das Azure-Portal
Die einfachste Möglichkeit zum Erstellen eines neuen virtuellen Computers mithilfe eines Images aus dem Azure Marketplace ist über das Azure-Portal. Navigieren Sie zu <https://portal.azure.com/#create>. Geben Sie das Betriebssystem ein, das Sie bereitstellen möchten, z.B. Windows oder SUSE und wählen Sie die Version aus. Achten Sie darauf, das Bereitstellungsmodell „Azure Resource Manager“ auszuwählen, und klicken Sie auf „Erstellen“.
...
1. Einstellungen
    ...
    1. Überwachung: Sie können die Diagnoseeinstellung deaktivieren. Sie wird automatisch aktiviert, wenn die Befehle zum Erstellen der erweiterten Azure-Überwachung aktiviert werden. Dies wird im Kapitel [Konfigurieren der Überwachung][deployment-guide-configure-monitoring-scenario-1] beschrieben.
...
#### Erstellen virtueller Computer mithilfe einer Vorlage
Sie können eine Bereitstellung auch mithilfe der im GitHub-Repository [azure-quickstart-templates][azure-quickstart-templates-github] veröffentlichten SAP-Vorlagen erstellen. Oder Sie erstellen manuell einen virtuellen Computer mithilfe des [Azure-Portals][virtual-machines-windows-tutorial], der [PowerShell][virtual-machines-ps-create-preconfigure-windows-resource-manager-vms] oder über die [Azure-CLI][virtual-machines-linux-tutorial].

* [Vorlage für die Konfiguration mit 2 Ebenen (nur ein virtueller Computer)][sap-templates-2-tier-marketplace-image] Verwenden Sie diese Vorlage, wenn Sie ein System mit 2 Ebenen und nur einem virtuellen Computer erstellen möchten.
* [Vorlage für die Konfiguration mit 3 Ebenen (mehrere virtuelle Computer)][sap-templates-3-tier-marketplace-image] Verwenden Sie diese Vorlage, wenn Sie ein System mit 3 Ebenen und mehreren virtuellen Computern erstellen möchten.
...
* **osType:** Betriebssystem für die Bereitstellung, z.B. Windows Server 2012 R2 oder SLES 12
...
#### Beitreten zu einer Domäne (nur Windows)
Falls die Bereitstellung in Azure über Azure Standort-zu-Standort oder Express Route mit dem lokalen AD/DNS verbunden ist (im [Planungs- und Bereitstellungshandbuch][planning-guide] auch als standortübergreifend bezeichnet), wird davon ausgegangen, dass der virtuelle Computer einer lokalen Domäne beitritt. Überlegungen zu diesem Schritt finden Sie im Kapitel [Beitreten eines virtuellen Computers zu einer lokalen Domäne (nur Windows)][deployment-guide-4.3] in diesem Dokument.

#### <a name="ec323ac3-1de9-4c3a-b770-4ff701def65b"></a>Konfigurieren der Überwachung
Konfigurieren Sie die Azure-Erweiterung zur verbesserten Überwachung für SAP wie im Kapitel [Konfigurieren der Azure-Erweiterung zur verbesserten Überwachung für SAP][deployment-guide-4.5] in diesem Dokument beschrieben.

Überprüfen Sie die Voraussetzungen für die SAP-Überwachung im Hinblick auf die erforderlichen Mindestversionen des SAP-Kernels und des SAP-Host-Agents in den Ressourcen, die im Kapitel [SAP-Ressourcen][deployment-guide-2.2] in diesem Dokument aufgeführt sind.
...
Verwenden Sie zum Erstellen einer Bereitstellung über ein privates Betriebssystemimage im Azure-Portal eine der im GitHub-Repository [azure-quickstart-templates][azure-quickstart-templates-github] veröffentlichten SAP-Vorlagen. Sie können auch mithilfe von [PowerShell][virtual-machines-upload-image-windows-resource-manager] einen virtuellen Computer erstellen.

* [Vorlage für die Konfiguration mit 2 Ebenen (nur ein virtueller Computer)][sap-templates-2-tier-user-image] Verwenden Sie diese Vorlage, wenn Sie ein System mit 2 Ebenen und nur einem virtuellen Computer und eigenem Betriebssystemimage erstellen möchten.
* [Vorlage für die Konfiguration mit 3 Ebenen (mehrere virtuelle Computer)][sap-templates-3-tier-user-image] Verwenden Sie diese Vorlage, wenn Sie ein System mit 3 Ebenen und mehreren virtuellen Computern und eigenem Betriebssystemimage erstellen möchten.
...
#### Installieren des VM-Agents (nur Linux)
Der Linux-Agent muss bereits im Benutzerimage installiert sein, wenn die oben genannten Vorlagen verwendet werden sollen. Andernfalls schlägt die Bereitstellung fehl. Laden Sie den VM-Agent herunter, und installieren Sie ihn. Eine Beschreibung finden Sie im Kapitel [Herunterladen, Installieren und Aktivieren von Azure VM-Agent][deployment-guide-4.4] in diesem Dokument. Wenn Sie die oben genannten Vorlagen nicht verwenden, können Sie den VM-Agent auch nachträglich installieren.
...
Konfigurieren Sie die Azure-Erweiterung zur Überwachung für SAP, wie im Kapitel [Konfigurieren der Azure-Erweiterung zur verbesserten Überwachung für SAP][deployment-guide-4.5] in diesem Dokument beschrieben. Überprüfen Sie die Voraussetzungen für die SAP-Überwachung im Hinblick auf die erforderlichen Mindestversionen des SAP-Kernels und des SAP-Host-Agents in den Ressourcen, die im Kapitel [SAP-Ressourcen][deployment-guide-2.2] in diesem Dokument aufgeführt sind.

#### Überprüfen der Überwachung
Überprüfen Sie, ob die Überwachung wie im Kapitel [Überprüfungen und Problembehandlung für die Einrichtung der Ende-zu-Ende-Überwachung für SAP in Azure][deployment-guide-troubleshooting-chapter] beschrieben funktioniert.

### <a name="a9a60133-a763-4de8-8986-ac0fa33aa8c1"></a>Szenario 3: Verschieben eines virtuellen lokalen Computers mit einer nicht generalisierten virtuellen Azure-Festplatte mit SAP
...
> ![Linux][Logo_Linux] Linux
>
> <https://azure.microsoft.com/de-DE/documentation/articles/virtual-machines-linux-agent-user-guide/>
...
* **storageType:** (nur Vorlage für 2 Ebenen) Zu verwendender Speichertyp 
    * Für größere Systeme wird dringend die Verwendung von Storage Premium empfohlen. Weitere Informationen zu den anderen Speichertypen finden Sie unter 
        ...
        * [Premium-Speicher: Hochleistungsspeicher für Workloads in Azure Virtual Machine][storage-premium-storage-preview-portal]
        * [Einführung in Microsoft Azure Storage][storage-introduction]
* **osDiskVhdUri:** URI des privaten Betriebssystem-Datanträgers, z.B. https://`<Kontoname`>.blob.core.windows.net/vhds/osdisk.vhd
* **newOrExistingSubnet:** Bestimmt, ob ein neues virtuelles Netzwerk und Subnetz erstellt oder ein bestehendes Subnetz verwendet werden soll. Wenn Sie bereits über ein virtuelles Netzwerk verfügen, das mit dem lokalen Netzwerk verbunden ist, wählen Sie hier „vorhanden“ aus.
* **subnetId:** Die ID des Subnetzes, mit dem die virtuellen Computer eine Verbindung herstellen sollen. Wählen Sie das Subnetz des virtuellen VPN- oder Express Route-Netzwerks aus, um den virtuellen Computer mit dem lokalen Netzwerk zu verbinden. In der Regel folgt die ID dem Muster /subscriptions/`<subscription id`>/resourceGroups/`<resource group name`>/providers/Microsoft.Network/virtualNetworks/`<virtual network name`>/subnets/`<subnet name`>
...
Der Linux-Agent muss bereits im Betriebssystem-Datenträger installiert sein, wenn die oben genannten Vorlagen verwendet werden sollen. Andernfalls schlägt die Bereitstellung fehl. Laden Sie den VM-Agent herunter, und installieren Sie ihn auf dem virtuellen Computer, wie im Kapitel [Herunterladen, Installieren und Aktivieren von Azure VM-Agent][deployment-guide-4.4] in diesem Dokument beschrieben.
...
### Szenario 4: Aktualisieren der Überwachungskonfiguration für SAP
...
![Ergebnis der Überprüfung der Azure PS-Cmdlet-Version][deployment-guide-figure-600] <a name="figure-6"></a>
...
![Ergebnis der Überprüfung der Azure-CLI-Version][deployment-guide-figure-760] <a name="0ad010e6-f9b5-4c21-9c09-bb2e5efb3fda"></a>
...
* Laden Sie den Azure-VM-Agent herunter:
	* Sie können das Installationspakets für den Azure-VM-Agent hier herunterladen: <https://go.microsoft.com/fwlink/?LinkId=394789>
...
Zum Installieren und Konfigurieren der Azure-Erweiterung zur verbesserten Überwachung für SAP können Sie Azure PowerShell oder Azure-CLI verwenden. Lesen Sie das Kapitel [Azure PowerShell][deployment-guide-4.5.1], falls Sie die Erweiterung mithilfe eines Windows-Computers auf einem virtuellen Windows- oder Linux-Computer installieren möchten. Lesen Sie hingegen das Kapitel [Azure-CLI][deployment-guide-4.5.2], falls Sie die Erweiterung mithilfe eines Linux-Desktops auf einem virtuellen Linux-Computer installieren möchten.
...
* Um zu überprüfen, ob alle Schritte für die verbesserte Azure-Überwachung erfolgreich ausgeführt wurden und die Azure-Infrastruktur die notwendigen Daten bereitstellt, fahren Sie mit dem Bereitschaftstest für die Azure-Erweiterung zur verbesserten Überwachung für SAP fort, wie im Kapitel [Bereitschaftsüberprüfung für die verbesserte Azure-Überwachung für SAP][deployment-guide-5.1] in diesem Dokument beschrieben. 
...
Führen Sie die Schritte in [diesem Artikel][install-extension-cli] durch, um die Azure-Erweiterung zur verbesserten Überwachung für SAP von einem Linux-Laptop/-Desktopcomputer aus auf einem virtuellen Linux-Computer zu installieren.

 [comment]: <> (MSSedusch TODO check if link is still valid)

## <a name="564adb4f-5c95-4041-9616-6635e83a810b"></a>Überprüfungen und Problembehandlung für die Einrichtung der Ende-zu-Ende-Überwachung für SAP in Azure
...
Führen Sie hierzu den Bereitschaftstest für die Azure-Erweiterung zur verbesserten Überwachung für SAP aus, wie im Kapitel [Bereitschaftsüberprüfung für die verbesserte Azure-Überwachung für SAP][deployment-guide-5.1] beschrieben. Wenn das Ergebnis dieser Überprüfung positiv ist und alle relevanten Leistungsindikatoren vorliegen, wurde die Azure-Überwachung erfolgreich eingerichtet. Fahren Sie in diesem Fall mit der Installation des SAP-Host-Agents fort, wie in den SAP-Hinweisen im Kapitel [SAP-Ressourcen][deployment-guide-2.2] in diesem Dokument beschrieben. Wenn beim Ergebnis des Bereitschaftstests fehlende Leistungsindikatoren auftreten, fahren Sie mit dem Integritätstest für die Azure-Überwachungsinfrastruktur fort, wie im Kapitel [Integritätsprüfung der Konfiguration für die Azure-Überwachungsinfrastruktur][deployment-guide-5.2] beschrieben. Lesen Sie im Falle eines Problems mit der Azure-Überwachungskonfiguration das Kapitel [Weitere Problembehandlung für die Azure-Überwachungsinfrastruktur für SAP][deployment-guide-5.3], die weiterer Hilfestellung für die Problembehandlung bietet.

### <a name="bb61ce92-8c5c-461f-8c53-39f5e5ed91f2"></a>Bereitschaftsüberprüfung für die verbesserte Azure-Überwachung für SAP
...
#### Ausführen des Bereitschaftstests auf einem virtuellen Windows-Computer
...
| ------------------------------|----------------------------------- |
| **Indikatoren gesamt: leer** | Die folgenden beiden Azure Storage-Indikatoren dürfen leer sein: <ul><li>Storage Read Op Latency Server msec</li><li>Storage Read Op Latenz E2E msec</li></ul>Alle anderen Indikatoren müssen Werte enthalten. |
...
#### Ausführen des Bereitschaftstests auf einem virtuellen Linux-Computer
...
###### Lösung
Die Konfiguration ist fehlerhaft. Aktivieren Sie die Überwachungserweiterung für den virtuellen Computer erneut, wie im Kapitel [Konfigurieren der Azure-Erweiterung zur verbesserten Überwachung für SAP][deployment-guide-4.5] in diesem Dokument beschrieben.

#### ![Windows][Logo_Windows] Einige Azure-Leistungsindikatoren fehlen.
