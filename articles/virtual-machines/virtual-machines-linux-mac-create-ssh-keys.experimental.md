...
Ihre öffentliche SSH-Schlüsseldatei wird standardmäßig unter `~/.ssh/id_rsa.pub` erstellt. Wenn Sie mit dem folgenden Befehl aufgefordert werden, sollten Sie eine „Passphrase“ zum Sichern Ihres privaten Schlüssels erstellen. (Die Passphrase ist ein Kennwort zum Verschlüsseln Ihres privaten Schlüssels.)
...
In diesem Artikel werden öffentlich-private SSH-Schlüsseldateien (Protokollversion 2 RSA) erstellt, die für Bereitstellungen im Resource Manager empfohlen werden.  *ssh-rsa*-Schlüssel werden im [Portal](https://portal.azure.com) sowohl für klassische Bereitstellungen als auch für Resource Manager-Bereitstellungen benötigt.
...
`ssh-keygen` bezieht sich auf ein Kennwort zum Verschlüsseln des privaten Schlüssels als „Passphrase“.  Es wird *dringend* empfohlen, Ihren Schlüsselpaaren eine Passphrase hinzuzufügen. Wenn der private Schlüssel nicht mit einer Passphrase geschützt ist, kann er von Personen, die über die Datei mit dem Schlüssel verfügen, zum Anmelden an allen Servern mit dem entsprechenden öffentlichen Schlüssel verwendet werden. Das Hinzufügen einer Passphrase sorgt für mehr Schutz. Wenn sich eine Person Zugang zu Ihrer Datei mit dem privaten Schlüssel verschafft, haben Sie so mehr Zeit, die Schlüssel für die Authentifizierung zu ändern.
...
Sie können `ssh-agent` zum Zwischenspeichern des Kennworts für die private Schlüsseldatei verwenden, damit Sie die Passphrase nicht bei jeder SSH-Anmeldung eingeben müssen. Bei Verwendung eines Macintosh-Computers werden die Kennwörter für private Schlüssel sicher im „Schlüsselbund“ von OSX gespeichert, wenn Sie `ssh-agent`aufrufen.
...
Wenn Sie bereits einen virtuellen Computer erstellt haben, können Sie den neuen öffentlichen SSH-Schlüssel für den virtuellen Linux-Computer mit dem folgenden Befehl installieren, indem Sie den VM-Benutzernamen und die Serveradresse durch Ihre eigenen Werte ersetzen:
...
Eine bewährte Methode ist das Erstellen und Konfigurieren einer `~/.ssh/config`-Datei, um Anmeldungen zu beschleunigen und das Verhalten Ihres SSH-Clients zu optimieren.
