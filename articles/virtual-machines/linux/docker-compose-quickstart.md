...
Im Azure Marketplace stehen Ihnen verschieden Azure-Verfahren und Images oder Resource Manager-Vorlagen zur Verfügung, um einen virtuellen Linux-Computer zu erstellen und als Docker-Host einzurichten. So finden Sie etwa unter [Verwenden der Docker-VM-Erweiterung zum Bereitstellen Ihrer Umgebung](dockerextension.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json) ein schnelles Verfahren zum Erstellen eines virtuellen Ubuntu-Computers mit der Azure Docker-VM-Erweiterung unter Verwendung einer [Schnellstartvorlage](https://github.com/Azure/azure-quickstart-templates/tree/master/docker-simple-on-ubuntu). 
...
- [Azure CLI 2.0:](#azure-cli-20) Unsere CLI der nächsten Generation für das Resource Manager-Bereitstellungsmodell
...
Stellen Sie nach Abschluss der Bereitstellung eine SSH-Verbindung mit dem neuen Docker-Host her. Verwenden Sie dazu den DNS-Namen, den Sie während der Bereitstellung angegeben haben. Sie können `azure vm show -g myResourceGroup -n myDockerVM` (Azure CLI 1.0) oder `az vm show -g myResourceGroup -n myDockerVM -d --query [fqdns] -o tsv` (Azure CLI 2.0) verwenden, um die Details Ihres virtuellen Computers anzuzeigen, einschließlich des DNS-Namens.
