...
> [!NOTE]
> In diesem Artikel werden Befehle der Azure-Befehlszeilenschnittstelle (auch Azure CLI 1.0) im Resource Manager-Modus vorgestellt. Um im Resource Manager-Modell zu arbeiten, können Sie auch die [Azure-Befehlszeilenschnittstelle 2.0 (Vorschau)](/cli/azure/install-az-cli2) ausprobieren, unsere Multi-Plattform-Befehlszeilenschnittstelle der nächsten Generation.
...
Dank eines Back-End-Adresspoolbereichs weiß ein Lastenausgleich, welche Ressourcen zum Weiterleiten von eingehendem Netzwerkdatenverkehr von seinem Endpunkt mithilfe von Azure Resource Manager verwendet werden. Nachdem Sie den Back-End-Adresspoolbereich (siehe Befehl „azure network lb address-pool create“) erstellt und ihm einen Namen gegeben haben, müssen Sie die Endpunkte hinzufügen, die jetzt von einer Ressource mit dem Namen „Netzwerkschnittstellen“ definiert werden.

Zum Konfigurieren des Back-End-Adressbereichs benötigen Sie mindestens eine „Netzwerkschnittstelle“ (weitere Informationen finden Sie unter der Befehlszeile „azure network lb nic“).

Im folgenden Beispiel wurde die zuvor erstellte Netzwerkschnittstelle „nic1“ verwendet, um den Back-End-Adresspoolbereich zu erstellen.
