...
- Bereitstellungsregion von Azure („USA, Westen“ oder „USA, Osten“ ab April 2017)
...
- Daten für jedes HANA (große Instanzen)-System:
  ...
  - Die gewünschte IP-Adresse für die HANA (große Instanz)-Einheit aus dem Adressbereich des Server-IP-Pools; beachten Sie, dass die ersten 30 IP-Adressen im Adressbereich des Server-IP-Pools für die interne Verwendung in HANA (große Instanzen) reserviert sind
  ...
  - Die Benutzer-ID (userid), die der Benutzer „hana-sidadm“ in Linux hat, ist erforderlich, um die notwendigen SAP HANA-bezogenen Datenträgervolumes zu erstellen.
- Die ID des Azure-Abonnements, mit dem SAP HANA in Azure (große Instanzen) direkt verbunden wird
...
- Die Definition eines Azure-VNets, das verwendet wird, um in ihm die virtuellen Computer der SAP-Anwendungsebene bereitzustellen.
...
- Das Azure-VNet, das erstellt wird, muss mindestens ein VM-Subnetz und ein ExpressRoute-Gatewaysubnetz haben. Diesen Subnetzen müssen die IP-Adressbereiche zugewiesen werden, die weiter unten angegeben und erläutert sind.
...
>[!Note]
>Das Azure-VNet für HANA (großen Instanzen) muss entsprechend dem Azure Resource Manager-Bereitstellungsmodells erstellt werden. Das ältere Azure-Bereitstellungsmodell, üblicherweise als ASM bezeichnet, wird für die Lösung HANA (großen Instanzen) nicht unterstützt.

Das VNet kann mithilfe des Azure-Portals, von PowerShell, der Azure-Vorlage oder Azure CLI erstellt werden (siehe [Erstellen eines virtuellen Netzwerks im Azure-Portal](../../../virtual-network/virtual-networks-create-vnet-arm-pportal.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)). Im folgenden Beispiel wird ein VNet erläutert, das über das Azure-Portal erstellt wurde.

Bei Betrachtung der Definitionen eines Azure-VNets über das Azure-Portal sollen einige Definitionen untersucht und soll ermittelt werden, welche Beziehung sie zu den nachstehend aufgelisteten Punkten haben. Wenn von **Adressraum** die Rede ist, ist der Adressraum gemeint, der für das Azure-VNet verwenden werden kann. Dies ist auch der Adressbereich, der im VNet für die BGP-Routenweitergabe verwendet wird. Dieser **Adressraum** ist hier zu sehen:
...
In anderen Fällen, in denen Sie IP-Adressbereiche sparsam oder sehr speziell verwenden müssen, können Sie den **VNet-Adressraum** eines VNets auf die speziellen Bereiche einschränken, die für jedes Subnetz verwendet werden. Im vorliegenden Fall können Sie den **VNet-Adressraum** eines VNets als mehrere bestimmte Bereiche definieren:

![Azure-VNet-Adressraum mit zwei Bereichen](./media/hana-overview-connectivity/image3-azure-vnet-address-space_alternate.png)

In diesem Fall hat der **VNet-Adressraum** zwei definierte Bereiche. Diese sind mit den IP-Adressbereichen identisch, die für den **IP-Adressbereich des Azure-VM-Subnetzes** und den **IP-Adressbereich des VNet-Gatewaysubnetzes** definiert sind.

Für diese Mandantensubnetze (VM-Subnetze) können Sie einen beliebigen Benennungsstandard verwenden. Allerdings **darf es für jedes VNet stets nur ein Gatewaysubnetz geben**, das mit der ExpressRoute-Verbindung von SAP HANA in Azure (große Instanzen) verknüpft wird. **Dieses Subnetz muss zudem stets den Namen „GatewaySubnet“ haben**, um eine ordnungsgemäße Platzierung des ExpressRoute-Gateways sicherzustellen.
...
- Sie müssen den **VNet-Adressraum** an Microsoft übermitteln, wenn Sie eine Erstbereitstellung von HANA (große Instanzen) ausführen. 
...
Einige der IP-Adressbereiche, die erforderlich sind, um HANA (große Instanzen) bereitzustellen, wurden bereits eingeführt. Es gibt einige weitere IP-Adressbereiche, die wichtig sind. Dazu folgen nun weitere Details. Die folgenden IP-Adressen, von denen nicht alle an Microsoft zu übermitteln sind, müssen definiert sein, bevor eine Anforderung zur Erstbereitstellung gesendet wird:

- **VNet-Adressraum:** Dies ist, wie bereits oben vorgestellt, der IP-Adressbereich, den Sie dem Adressraum-Parameter in den Azure Virtual Networks (VNet) zugewiesen haben (oder später zuweisen möchten), die mit der SAP HANA (große Instanzen)-Umgebung verbunden werden. Dieser Adressraum-Parameter sollte ein mehrzeiliger Wert sein, der aus den Azure-VM-Subnetzbereichen und dem Azure-Gatewaysubnetzbereich besteht (siehe Abbildungen weiter oben). Dieser Bereich darf weder mit dem Adressbereich Ihres lokalen oder Server-IP-Pools noch mit ER P2P-Adressbereichen überlappen. Wie lässt sich dies sicherstellen? Ihr Unternehmensnetzwerkteam oder Dienstanbieter sollte einen IP-Adressbereich bereitstellen, der in Ihrem Netzwerk nicht verwendet wird. Beispiel: Wenn Ihr Azure-VM-Subnetz (siehe oben) als „10.0.1.0/24“ und Ihr Azure-Gatewaysubnetz (siehe unten) als „10.0.2.0/28“ angegeben ist, sollte Ihr Azure-VNet-Adressraum in zwei Zeilen stehen: „10.0.1.0/24“ und „10.0.2.0/28“. Obwohl die Adressraumwerte aggregiert werden können, wird empfohlen, sie an die Subnetzbereiche anzupassen, um eine zukünftige versehentliche Wiederverwendung an anderer Stelle Ihres Netzwerks zu vermeiden. **Dies ist ein IP-Adressbereich, der an Microsoft übermittelt werden muss, wenn eine Erstbereitstellung angefordert wird.**

- **IP-Adressbereich des Azure-VM-Subnetzes:** Dieser IP-Adressbereich ist, wie bereits weiter oben erläutert, derjenige, den Sie dem Azure-VNet-Subnetzparameter für das Azure-VNet zugewiesen haben (oder später zuweisen möchten), das mit der SAP HANA (große Instanzen)-Umgebung verbunden wird. Dieser IP-Adressbereich wird dazu verwendet, Ihren Azure-VMs IP-Adressen zuzuweisen. Für diesen Bereich wird zugelassen, Verbindungen mit Ihren SAP HANA (große Instanzen)-Servern herzustellen. Bei Bedarf können mehrere Azure-VM-Subnetze verwendet werden. Microsoft empfiehlt für jedes Azure-VM-Subnetz einen /24-CIDR-Block. Dieser Adressbereich muss Bestandteil der Werte sein, die im Azure-VNet-Adressraum verwendet werden. Wie lässt sich dies sicherstellen? Ihr Unternehmensnetzwerkteam oder Dienstanbieter sollte einen IP-Adressbereich bereitstellen, der in Ihrem Netzwerk derzeit nicht verwendet wird.
...
- **IP-Adressbereich des VNet-Gatewaysubnetzes:** Je nachdem, welche Features Sie verwenden möchten, wird die folgende Größe empfohlen:
   - Höchstleistungs-ExpressRoute-Gateway: /26-Adressblock
   - Koexistenz mit VPN und ExpressRoute über ein Hochleistungs-ExpressRoute-Gateway (oder kleiner): /27-Adressblock
   - Alle anderen Fälle: /28-Adressblock. Dieser Adressbereich muss Bestandteil der Werte sein, die in den Werten von „Azure-VNet-Adressraum“ verwendet werden. Dieser Adressbereich muss Bestandteil der Werte sein, die in dem Azure-VNet-Adressraum verwendet werden, den Sie an Microsoft übermitteln müssen. Wie lässt sich dies sicherstellen? Ihr Unternehmensnetzwerkteam oder Dienstanbieter sollte einen IP-Adressbereich bereitstellen, der in Ihrem Netzwerk derzeit nicht verwendet wird. 

- **Adressbereich für ER-P2P-Konnektivität:** Dies ist der IP-Adressbereich für die SAP HANA (große Instanzen)-ExpressRoute-P2P-Verbindung (ExpressRoute = ER). Dieser Bereich von IP-Adressen muss ein /29-CIDR-IP-Adressbereich sein. Dieser Bereich darf weder mit dem lokalen noch mit anderen Azure-IP-Adressbereichen überlappen. Der Bereich wird verwendet, um die ER Konnektivität von Ihrem Azure-VNet ExpressRoute-Gateway mit den SAP HANA (große Instanzen)-Servern einzurichten. Wie lässt sich dies sicherstellen? Ihr Unternehmensnetzwerkteam oder Dienstanbieter sollte einen IP-Adressbereich bereitstellen, der in Ihrem Netzwerk derzeit nicht verwendet wird. **Dies ist ein IP-Adressbereich, der an Microsoft übermittelt werden muss, wenn eine Erstbereitstellung angefordert wird.**
  
- **Adressbereich des Server-IP-Pools:** Dieser IP-Adressbereich wird verwendet, um jedem SAP HANA (große Instanzen)-Server seine eigene IP-Adresse zuzuweisen. Als Subnetzgröße wird ein /24-CIDR-Block empfohlen – bei Bedarf kann aber auch ein kleineres Subnetz verwendet werden, mindestens 64 IP-Adressen müssen jedoch bereitgestellt werden. Aus diesem Bereich werden die ersten 30 IP-Adressen für die Verwendung durch Microsoft reserviert. Dies ist zu berücksichtigen, wenn Sie die Größe des Bereichs festlegen. Dieser Bereich darf weder mit den lokalen noch mit anderen Azure-IP-Adressen überlappen. Wie lässt sich dies sicherstellen? Ihr Unternehmensnetzwerkteam oder Dienstanbieter sollte einen IP-Adressbereich bereitstellen, der in Ihrem Netzwerk derzeit nicht verwendet wird. Ein eindeutiger /24-CIDR-Block (empfohlen) für die Zuweisung bestimmter IP-Adressen, die für SAP HANA in Azure (große Instanzen) erforderlich sind. **Dies ist ein IP-Adressbereich, der an Microsoft übermittelt werden muss, wenn eine Erstbereitstellung angefordert wird.**
...
Es folgen ein Beispiel der verschiedenen Bereiche sowie einige Beispielbereiche so, wie die Sie diese konfigurieren und schließlich an Microsoft übermitteln müssen. Wie Sie sehen können, sind die Werte für den Azure-VNet-Adressraum im ersten Beispiel nicht aggregiert, sondern aus den Bereichen des ersten IP-Adressbereichs des Azure-VM-Subnetzes und des IP-Adressbereichs des VNet-Gatewaysubnetzes definiert. Ein Verwenden mehrerer VM-Subnetze im Azure-VNet würde entsprechend funktionieren, indem die weiteren IP-Adressbereiche der zusätzlichen VM-Subnetze als Bestandteil des Azure-VNet-Adressraums konfiguriert und übermittelt werden.
...
Sie können die Daten, die Sie an Microsoft übermitteln, auch aggregieren. In diesem Fall umfasst der Adressraum des Azure-VNets nur einen Beeich. Wird der IP-Adressbereich aus dem obigen Beispiel verwendet, sieht dies wie folgt aus:
...
Nun da das Azure-VNet ein ExpressRoute-Gateway hat, verwenden Sie die von Microsoft bereitgestellten Autorisierungsinformationen zum Verknüpfen des ExpressRoute-Gateways mit der ExpressRoute-Verbindung von SAP HANA in Azure (große Instanzen), die für diesen Zweck erstellt wurde. Dieser Schritt kann über das Azure-Portal oder mit PowerShell ausgeführt werden. Das Portal wird empfohlen, aber die PowerShell-Anweisungen sehen folgt aus. 

- Sie führen das folgende Skript für jedes VNet-Gateway aus, wobei Sie für jede Verbindung eine andere AuthGUID verwenden. Die ersten beiden unten gezeigten Einträge stammen aus den Informationen, die von Microsoft bereitgestellt werden. Zudem ist die AuthGUID spezifisch für jedes VNet und sein Gateway. Das heißt, Sie benötigen eine weitere AuthID für Ihre ExpressRoute-Leitung, die SAP HANA (große Instanzen) mit Azure verbindet, wenn Sie ein weiteres Azure-VNet hinzufügen möchten. 
...
Sie müssen diesen Schritt mehrmals ausführen, wenn Sie das Gateway mit mehreren ExpressRoute-Leitungen verbinden möchten, die Ihrem Abonnement zugeordnet sind. Beispiel: Sie müssen wahrscheinlich das VNet-Gateway, über das das VNet mit Ihrem lokalen Netzwerk verbunden wird, mit der ExpressRoute-Leitung verbinden.
...
In diesem Fall empfiehlt es sich, den neuen IP-Adressbereichs als neuen Bereich zum VNet-Adressraum hinzuzufügen, statt einen neuen aggregierten Bereich zu erstellen. In beiden Fällen müssen Sie diese Änderung an Microsoft übermitteln, damit Verbindungen aus diesem neuen IP-Adressbereich mit den SAP HANA (große Instanzen)-Einheiten in Ihrem Client zugelassen werden. Sie können eine Azure-Supportanfrage öffnen, um zu veranlassen, dass der IP-Adressbereich hinzugefügt wird. Führen Sie nach Erhalt einer Bestätigung die nächsten Schritte aus.
