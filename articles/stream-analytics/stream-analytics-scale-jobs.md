...
## <a name="configuring-streaming-units"></a>Konfigurieren von Streamingeinheiten
Streamingeinheiten (Streaming Units, SUs) sind eine abstrakte Repräsentation für die Ressourcen und Computeleistung, die zum Ausführen eines Azure Stream Analytics-Auftrags benötigt werden. SUs bieten anhand eines kombinierten Maßes aus CPU, Arbeitsspeicher und Schreib- und Leseraten eine Möglichkeit zur Beschreibung der relativen Ereignisverarbeitungskapazität. Jede Streaming-Einheit entspricht etwa einem Durchsatz von 1 MB/s. 

Die benötigte SU-Anzahl für ein bestimmtes Projekt hängt von der Partitionskonfiguration für die Eingaben und der für den Auftrag definierten Abfrage ab. Über das klassische Azure-Portal können Sie für einen Auftrag bis zu Ihrem Kontingent Streamingeinheiten einrichten. Jedes Azure-Abonnement verfügt standardmäßig über ein Kontingent von höchstens 50 Streamingeinheiten für alle Analyseaufträge in einer bestimmten Region. Wenn Sie die Streamingeinheiten für Ihr Abonnement erhöhen möchten, wenden Sie sich an den [Microsoft Support](http://support.microsoft.com).
...
## <a name="embarrassingly-parallel-job"></a>Hochgradig paralleler Auftrag
...
1. Wenn Ihre Abfragelogik davon abhängig ist, dass der gleiche Schlüssel durch die gleiche Abfrageinstanz verarbeitet wird, müssen Sie sicherstellen, dass die Ereignisse in die gleiche Partition Ihrer Eingabe aufgenommen werden. Für Event Hubs bedeutet dies, dass für die Ereignisdaten ein **PartitionKey** (Partitionsschlüssel) festgelegt werden muss. Alternativ können Sie auch partitionierte Absender verwenden. Für Blob bedeutet das, dass die Ereignisse an den gleichen Partitionsordner gesendet werden. Wenn es für Ihre Abfragelogik nicht erforderlich ist, dass der gleiche Schlüssel von der gleichen Abfrageinstanz verarbeitet wird, können Sie diese Anforderung ignorieren. Ein Beispiel hierfür wäre eine einfache Auswahl-, Projekt- oder Filterabfrage.  
...
3. Derzeit unterstützen nur Event Hubs und Blob eine partitionierte Ausgabe. Für die Event Hubs-Ausgabe müssen Sie für das Feld **PartitionKey** den Wert **PartitionId** festlegen. Für Blob müssen Sie nichts tun.  
...
Bei der Abfrage handelt es sich um einen einfachen Filter, und daher brauchen wir uns um die Partitionierung der an Event Hubs gesendeten Eingabe nicht zu kümmern.  Sie sehen, dass für **Partition By** in der Abfrage der Wert **PartitionId** festgelegt ist. Die zweite oben genannte Anforderung ist also erfüllt. Für die Ausgabe muss die Event Hubs-Ausgabe in dem Auftrag so konfiguriert werden, dass das Feld **PartitionKey** auf **PartitionId** festgelegt ist. Und als letzte Prüfung: Die Anzahl von Eingabepartitionen ist gleich der Anzahl von Ausgabepartitionen. Diese Topologie ist hochgradig parallel.
...
### <a name="not-using-event-hubs-or-blobs-as-output"></a>Keine Verwendung von Event Hubs oder Blobs als Ausgabe
...
### <a name="multi-step-query-with-different-partition-by-values"></a>Mehrschrittige Abfrage mit unterschiedlichen Werten für „Partition By“
Eingabe: Event Hub mit 8 Partitionen. Ausgabe: Event Hub mit 8 Partitionen
...
> [!NOTE]
> Diese Beispielabfrage wird im Laufe des Artikels erläutert.
> 
> 
...
> [!NOTE]
> Wenn Sie zwei Datenströmen verknüpfen, stellen Sie sicher, dass die Ströme anhand des Partitionsschlüssels der Spalte partitioniert werden, in der Sie die Verknüpfungen vornehmen, und in beiden Strömen dieselbe Anzahl an Partitionen vorhanden ist.
> 
> 

## <a name="configure-stream-analytics-job-partition"></a>Konfigurieren der Stream Analytics-Auftragspartition
...
## <a name="stream-analytics-throughput-at-scale---raspberry-pi-scenario"></a>Skalierter Stream Analytics-Durchsatz – Raspberry Pi-Szenario
