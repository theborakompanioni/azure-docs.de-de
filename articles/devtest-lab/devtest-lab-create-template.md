---
title: Verwalten benutzerdefinierter Azure DevTest Labs-Images zum Erstellen virtueller Computer | Microsoft Docs
...
---
# <a name="manage-azure-devtest-labs-custom-images-to-create-vms"></a>Verwalten benutzerdefinierter Azure DevTest Labs-Images zum Erstellen virtueller Computer
...
In diesem Abschnitt erfahren Sie, wie Sie ein benutzerdefiniertes Image aus einer VHD-Datei erstellen.
Sie benötigen Zugriff auf eine gültige VHD-Datei, um alle in diesem Abschnitt beschriebenen Schritte auszuführen.   
...
5. Wählen Sie auf dem Blatt **Konfiguration** für das Lab die Option **Benutzerdefinierte Images** aus.
6. Wählen Sie auf dem Blatt **Benutzerdefinierte Images** **+ Benutzerdefiniertes Image**.
   
    ![Hinzufügen des benutzerdefinierten Images](./media/devtest-lab-create-template/add-custom-image.png)
7. Geben Sie den Namen des benutzerdefinierten Images ein. Dieser Name wird in der Liste der Basisimages angezeigt, wenn Sie einen virtuellen Computer erstellen.
8. Geben Sie die Beschreibung des benutzerdefinierten Images ein. Diese Beschreibung wird in der Liste der Basisimages angezeigt, wenn Sie einen virtuellen Computer erstellen.
9. Wählen Sie **VHD-Datei**.
10. Wenn Sie Zugriff auf eine VHD-Datei haben, die nicht aufgelistet ist, fügen Sie sie gemäß der Anweisungen im Abschnitt [Hochladen einer VHD-Datei](#upload-a-vhd-file) hinzu, und kehren Sie nach Abschluss zu diesem Punkt zurück.
11. Wählen Sie die gewünschte VHD-Datei aus.
12. Wählen Sie **OK**, um das Blatt **VHD-Datei** zu schließen.
13. Wählen Sie **Betriebssystemkonfiguration**.
14. Wählen Sie auf der Registerkarte **Betriebssystemkonfiguration** eine der Optionen **Windows** oder **Linux** aus.
15. Wenn Sie **Windows** ausgewählt haben, geben Sie über das Kontrollkästchen an, ob *Sysprep* auf dem Computer ausgeführt wurde.
16. Wählen Sie **OK**, um das Blatt **Betriebssystemkonfiguration** zu schließen.
17. Wählen Sie **OK** , um das benutzerdefinierte Image zu erstellen.
18. Gehen Sie zum Abschnitt [Nächste Schritte](#next-steps) .

### <a name="upload-a-vhd-file"></a>Hochladen einer VHD-Datei
...
2. Auf dem nächsten Blatt wird eine Anleitung zum Ändern und Ausführen eines PowerShell-Skripts angezeigt, das eine VHD-Datei in Ihr Azure-Abonnement hochlädt. 
   **Hinweis** : Dieser Vorgang kann abhängig von der Größe der VHD-Datei und der Geschwindigkeit Ihrer Verbindung lange dauern.
...
2. Wählen Sie **Weitere Dienste** und dann in der Liste **DevTest Labs**.
3. Wählen Sie in der Liste der Labs das gewünschte Lab aus.  
4. Wählen Sie auf dem Blatt des Labs die Option **My virtual machines**(Meine virtuellen Computer) aus.
5. Wählen Sie auf dem Blatt **My virtual machines** (Meine virtuellen Computer) den virtuellen Computer aus, von dem Sie das benutzerdefinierte Image erstellen möchten.
6. Wählen Sie auf dem Blatt des virtuellen Computers **Create custom image (VHD)**(Benutzerdefiniertes Image erstellen (VHD)).
   
    ![Menüelement: Benutzerdefiniertes Bild erstellen](./media/devtest-lab-create-template/create-custom-image.png)
7. Geben Sie auf dem Blatt **Create image** (Image erstellen) einen Namen und eine Beschreibung für Ihr benutzerdefiniertes Image ein. Diese Informationen werden in der Liste der Basisimages angezeigt, wenn Sie einen virtuellen Computer erstellen.
   
    ![Blatt: Benutzerdefiniertes Bild erstellen](./media/devtest-lab-create-template/create-custom-image-blade.png)
8. Wählen Sie aus, ob „sysprep“ auf dem virtuellen Computer ausgeführt wurde. Wenn „sysprep“ auf dem virtuellen Computer nicht ausgeführt wurde, geben Sie an, ob „sysprep“ ausgeführt werden soll, wenn Sie einen virtuellen Computer anhand dieses benutzerdefinierten Images erstellen.
9. Wählen Sie **OK** , wenn Sie mit der Erstellung des benutzerdefinierten Images fertig sind.
