---
title: Behandeln des Inhaltstyps von Logik-Apps | Microsoft-Dokumentation
...
---
# <a name="logic-apps-content-type-handling"></a>Behandeln des Inhaltstyps von Logik-Apps
...
Der Anforderungstrigger ermöglicht Ihnen, ein JSON-Schema für die Nutzlast eingeben, deren Empfang Sie erwarten. Dies ermöglicht dem Designer, Token zu generieren, die Sie bei der Nutzung des Inhalts der Anforderung unterstützen. Wenn Sie nicht bereits über ein Schema verfügen, wählen Sie `Use sample payload to generate schema`, um ein JSON-Schema aus einer Beispielnutzlast zu generieren.

![Schema](./media/logic-apps-http-endpoint/manualtrigger.png)

### <a name="use-parse-json-action"></a>Verwenden der JSON-Analyseaktion
Mit der `Parse JSON`-Aktion können Sie einen JSON-Inhalt analysieren, um anzeigbare Token zur Nutzung in der Logik-App zu erhalten. Ähnlich wie beim Anforderungstrigger können Sie ein JSON-Schema für den Inhalt, den Sie analysieren möchten, eingeben oder generieren. Dieses Tool ist ideal, um die Nutzung von Daten von Service Bus, DocumentDb usw. extrem zu erleichtern.
