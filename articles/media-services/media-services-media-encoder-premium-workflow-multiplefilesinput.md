...
- Überlagern des Eingabevideos mit einem Logobild bei der Codierung
...
Der Code in C# zum Auslesen der XML-Konfiguration aus einer Datei und deren Übergabe an den Task in einem Auftrag lautet wie folgt:
...
In einigen Fällen ist es sinnvoll, eine Komponenteneigenschaft zusammen mit der Workflowdatei anzupassen, die im Media Encoder Premium Workflow ausgeführt wird. Angenommen, Sie haben einen Workflow entworfen, der Ihre Videos mit Text überlagert, und der Text (z.B. das aktuelle Datum) soll zur Laufzeit festgelegt werden. Senden Sie hierzu im Codierungstask den Text als neuen Wert für die Texteigenschaft der Überlagerungskomponente. Sie können diesen Mechanismus verwenden, um andere Eigenschaften einer Komponente im Workflow zu ändern (z.B. Position oder Farbe der Überlagerung, Bitrate des AVC-Encoders usw.).**setRuntimeProperties** dient zum Überschreiben einer Eigenschaft in den Komponenten des Workflows.
...
Um den Pfad und die Eigenschaft zu überprüfen, klicken Sie auf die Aktionsschaltfläche unmittelbar neben jeder Eigenschaft. Sie können auf diese Aktionsschaltfläche klicken und „Bearbeiten“ auswählen. Dadurch erfahren Sie den tatsächlichen Namen der Eigenschaft und unmittelbar darüber den Namespace.
...
Jeder Task, die Sie an den **Media Encoder Premium Workflow** übermitteln, erfordert zwei Objekte:
...
- das zweite ist ein „Medienobjekt“, das die Mediendateien enthält, die Sie codieren möchten.
...
Verbindungen im Workflow
  - Wenn Sie eine oder mehrere „Media File Input“-Komponenten verwenden und vorhaben, **setRuntimeProperties** zum Angeben des Dateinamens zu nutzen, dürfen Sie diese nicht mit dem Pin der primären Dateikomponente verbinden. Stellen Sie sicher, dass es keine Verbindung zwischen dem primären Dateiobjekt und „Media File Input“-Komponenten gibt.
...
*Keine Verbindung zwischen der primären Datei und „Media File Input“-Komponenten, wenn Sie „setRuntimeProperties“ verwenden, um den Dateinamen festzulegen*
...
Wenn Sie angeben möchten, dass „/primarySourceFile“ diese Eigenschaft zum Benennen der Ausgabedateien mithilfe von „Ausdrücken“ verwendet, wird empfohlen, den Cliplisten-XML-Code als Eigenschaft *hinter* der „/primarySourceFile“-Eigenschaft anzugeben. Dies soll verhindern, dass die Clipliste von der Einstellung von „/primarySourceFile“ überschrieben wird.
...
Nehmen wir ein Beispiel, bei dem Sie das Eingabevideo bei der Codierung mit einem Logobild überlagern möchten. In diesem Beispiel heißen das Eingabevideo „MyInputVideo.mp4“ und das Logo „MyLogo.png“. Die folgenden Schritte müssen erfolgen:

- Erstellen eines Workflowobjekts mit der Workflowdatei (siehe das nachfolgende Beispiel).
...
Bei diesem Beispiel wird der Name der Videodatei an die „Media File Input“-Komponente und die „primarySourceFile“-Eigenschaft übermittelt. Der Name der Logodatei wird an eine andere „Media File Input“-Komponente übermittelt, die mit der Komponente für die grafische Überlagerung verbunden ist.
...
###Schritte zum Erstellen eines Workflows zum Überlagern von Video mit einem Logo     

Es folgen die Schritte zum Erstellen eines Workflows, der zwei Eingabedateien verwendet: ein Video und ein Bild. Das Bild soll das Video überlagern.
...
Nachdem dieser Vorgang abgeschlossen ist, untersucht die Komponente „Mediendateieingabe“ die Datei und füllt ihre Ausgabeeinstellungen auf, um die untersuchte Datei widerzuspiegeln.
...
*Layouttyp = Konfigurierbar planar*
...
Fügen Sie eine andere Mediendateieingabe (zum Laden der Logodatei) hinzu, klicken Sie auf diese Komponente, und benennen Sie sie in „Mediendateieingabe-Logo“ um. Wählen Sie über die Option „Datei“ ein Bild (z.B. eine PNG-Datei) aus. Verbinden Sie den Pin „Nicht komprimiertes Bild“ mit dem Pin „Nicht komprimiertes Bild“ der Überlagerung.
...
Sobald dies der Fall ist, können Sie ihn in Azure Media Services ausführen.

Bereiten Sie zuerst in Azure Media Services ein Medienobjekt vor, das die Videodatei und das Logo enthält. Dies ist mithilfe der .NET- oder REST-API möglich. Sie können dies auch im Azure-Portal durchführen oder den [Azure Media Services-Explorer](https://github.com/Azure/Azure-Media-Services-Explorer) (AMSE) nutzen.

Dieses Tutorial zeigt, wie Medienobjekte mit dem AMSE verwaltet werden. Es gibt zwei Möglichkeiten, einem Medienobjekt Dateien hinzuzufügen.

1. Erstellen Sie einen lokalen Ordner, in den Sie die beiden Dateien kopieren, und legen Sie ihn dann per Drag&Drop auf der Registerkarte „Medienobjekt“ ab.
1. Laden Sie die Datei als Medienobjekt hoch. Zeigen Sie anschließend die Informationen zum Medienobjekt an, wechseln Sie zur Registerkarte „Dateien“, und laden Sie eine zusätzliche Datei (mit dem Logo) hoch.
...
Sie können den Beispielworkflow [hier](https://github.com/Azure/azure-media-services-samples/tree/master/Encoding%20Presets/VoD/MediaEncoderPremiumWorkfows/) herunterladen.
