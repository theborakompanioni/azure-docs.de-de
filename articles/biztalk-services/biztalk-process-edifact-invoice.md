---
title: 'Tutorial: Verarbeiten von EDIFACT-Rechnungen mit Azure BizTalk Services | Microsoft Docs'
...
## <a name="what-does-the-solution-do"></a>Was ist die Aufgabe der Lösung?
...
> [!NOTE]
> Optional veranschaulicht diese Lösung auch das Verwenden der Batchverarbeitung zum Senden von Rechnungen in Batches anstatt einzelner getrennter Rechnungen.  
> 
> 
...
5. Führen Sie den Tutorialclient weiter aus. Öffnen Sie das Portal, klicken Sie auf **Service Bus** > ***Ihr Service Bus-Namespace*** > **Warteschlangen**, und überprüfen Sie, ob die drei Warteschlangen erstellt wurden.  

## <a name="step-2-create-and-deploy-trading-partner-agreement"></a>Schritt 2: Erstellen und Bereitstellen der Handelspartnervereinbarung
...
### <a name="create-the-trading-partners"></a>Erstellen der Handelspartner
...
2. Geben Sie auf der Seite **Neuer Partner** „Contoso“ als Namen des Partners ein, und klicken Sie anschließend auf **Speichern**.
3. Wiederholen Sie diesen Schritt, um den zweiten Partner, **Northwind**, zu erstellen.  

### <a name="create-the-agreement"></a>Erstellen der Vereinbarung
...
2. Geben Sie auf der Seite **Allgemeine Einstellungen** der Vereinbarung die Werte wie in der Abbildung unten dargestellt ein, und klicken Sie anschließen auf **Weiter**.
...
3. Erstellen Sie die Sendevereinbarung zwischen Contoso und Northwind. Diese Vereinbarung legt fest, wie Contoso die EDIFACT-Rechnung an Northwind sendet.
   ...
   2. Übernehmen Sie die Standardwerte der Registerkarten **Eingangs-URL**, **Transformation** und **Batchverarbeitung**.
   3. Laden Sie auf der Registerkarte **Protokoll** im Abschnitt **Schemas** das Schema **EFACT_D93A_INVOIC.xsd** hoch. Dieses Schema steht im Beispielpaket zur Verfügung.
...
4. Erstellen Sie die Empfangsvereinbarung zwischen Contoso und Northwind. Diese Vereinbarung legt fest, wie Contoso die Bestätigung von Northwind empfängt.
   ...
   3. Laden Sie auf der Registerkarte **Protokoll** im Abschnitt **Schemas** das Schema **EFACT_4.1_CONTRL.xsd** hoch. Dieses Schema steht im Beispielpaket zur Verfügung.
...
## <a name="step-3-create-and-deploy-the-biztalk-services-project"></a>Schritt 3: Erstellen und Bereitstellen des BizTalk Services-Projekts
...
* **EFACT_D93A_INVOIC. XSD**: Schema der EDIFACT-Standardrechnung
* **EFACT_4.1_CONTRL. XSD**: Schema der EDIFACT-Bestätigung, die Northwind an Contoso sendet
* **INHOUSEINVOICE_to_D93AINVOIC. TRFM**: Die Transformation, mit deren Hilfe das interne Rechnungsschema dem EDIFACT-Standardrechnungsschema zugeordnet wird  

### <a name="create-the-biztalk-services-project"></a>Erstellen des BizTalk Services-Projekts
...
4. Klicken Sie im Feld **Nachrichtentypen** auf das Pluszeichen (**+**), um das Schema der eingehenden Nachricht anzugeben. Da die eingehende Rechnung für die EAI-Brücke immer die interne Rechnung ist, legen Sie diese auf **INHOUSEINVOICE**fest.
   ...
   ![][8]  
5. Klicken Sie auf das Shape **XML-Transformation**, und klicken Sie im Eigenschaftenfeld für die Eigenschaft **Zuordnung** auf die Auslassungsschaltfläche (**...**). Wählen Sie im Dialogfeld **Zuordnungsauswahl** die Transformationsdatei **INHOUSEINVOICE_to_D93AINVOIC** aus, und klicken Sie anschließend auf **OK**.
...
8. Aktualisieren der Datei „EDIBridge.config“ mit Konfigurationsdetails
   ...
   * Stellen Sie unter *<behaviors>* den ACS-Namespace und den dem BizTalk Services-Abonnement zugeordneten Schlüssel bereit.
   * Stellen Sie unter *<client>* den Endpunkt bereit, an dem der EDI-Sendevertrag bereitgestellt ist.
...
1. Laden Sie auf dem Computer, auf dem das BizTalk Services-Projekt erstellt wurde, das SSL-Zertifikat für Ihr BizTalk Services-Abonnement herunter, und installieren Sie es. Klicken Sie im unter BizTalk Services auf **Dashboard** und anschließend auf **SSL-Zertifikat herunterladen**. Doppelklicken Sie auf das Zertifikat, und folgen Sie der Aufforderung zum Abschließen der Installation. Achten Sie darauf, dass Sie das Zertifikat unter dem Zertifikatspeicher **Vertrauenswürdige Stammzertifizierungsstellen** installieren.
2. Klicken Sie in Visual Studio im Projektmappen-Explorer mit der rechten Maustaste auf das Projekt **InvoiceProcessingBridge**, und klicken Sie anschließend im Kontextmenü auf **Bereitstellen**.
...
## <a name="step-5-optional-send-edifact-invoice-in-batches"></a>Schritt 5 (optional): Batchversand von EDIFACT-Rechnungen
