---
title: Anleitungen zum Content Delivery Network (CDN) | Microsoft-Dokumentation
...
---
# <a name="content-delivery-network-cdn-guidance"></a>Anleitungen zum Content Delivery Network (CDN)
...
* **Versionierung und Cachesteuerung**. Sie müssen berücksichtigen, wie Sie statische Inhalte aktualisieren und neue Versionen bereitstellen möchten. Der Inhalt des CDN wird beim Verwenden des Azure-Portals unter Umständen [bereinigt](cdn/cdn-purge-endpoint.md), wenn neue Versionen Ihrer Inhalte verfügbar sind. Dies ist eine ähnliche Herausforderung wie das Verwalten der clientseitigen Zwischenspeicherung, wie sie z. B. in einem Webbrowser auftritt.
...
Erwägen Sie, den Zugriff auf den CDN-Inhalt nach Land zu beschränken. Mit Azure CDN können Sie Anforderungen auf Basis des Ursprungslands filtern und den bereitgestellten Inhalt einschränken. Weitere Informationen finden Sie unter [Einschränken des Zugriffs auf Inhalte nach Ländern](cdn/cdn-restrict-access-by-country.md).
...
Das Azure CDN bietet keine Möglichkeiten zur Zugriffssteuerung, um den Zugriff auf Inhalte abzusichern. Sie können keine Shared Access Signatures (SAS) mit dem CDN verwenden.

Wenn Sie clientseitige Skripts mit dem CDN bereitstellen, stoßen Sie unter Umständen auf Probleme, wenn diese Skripts einen *XMLHttpRequest*-Aufruf verwenden, um HTTP-Anforderungen für andere Ressourcen auszuführen, z.B. Daten, Bilder oder Schriftarten in einer anderen Domäne. Viele Webbrowser verhindern CORS (Cross-Origin Resource Sharing, ursprungsübergreifende Ressourcenfreigabe), sofern der Webserver nicht so konfiguriert wurde, dass die entsprechenden Antwortheader festgelegt werden. Sie können das CDN für die Unterstützung von CORS konfigurieren:

* Wenn es sich bei dem Ursprung, aus dem Sie Inhalte bereitstellen, um Azure Blob Storage handelt, können Sie den Diensteigenschaften eine *CorsRule* hinzufügen. Die Regel kann die zulässigen Ursprünge für CORS-Anforderungen, die zulässigen Methoden wie z. B. GET und das maximale Alter in Sekunden für die Regel angeben (der Zeitraum, in dem der Client die verknüpften Ressourcen nach dem Laden des ursprünglichen Inhalts anfordern muss). Weitere Informationen finden Sie unter [Cross-Origin Resource Sharing (CORS)-Support für die Azure Storage-Dienste](http://msdn.microsoft.com/library/azure/dn535601.aspx).
* Falls der Ursprung, aus dem Sie Inhalte bereitstellen, ein Ordner innerhalb der Anwendung ist, z.B. der *cdn*-Stammordner, können Sie ausgehende Regeln in der Anwendungskonfigurationsdatei konfigurieren, um einen *Access-Control-Allow-Origin-Header* für alle Antworten festzulegen. Weitere Informationen zum Verwenden der Regeln für das Umschreiben finden Sie im Thema zum [URL-Umschreibemodul](http://www.iis.net/learn/extensions/url-rewrite-module).
...
### <a name="search-engine-optimisation"></a>Suchmaschinenoptimierung
...
### <a name="url-rewriting"></a>URL-Umschreibung
...
* Die nächsten vier Regeln zeigen das Umleiten von Anforderungen, wenn Sie die Ressourcen nicht in einem Ordner namens *cdn** im Stammverzeichnis der Webrolle speichern möchten. Die Regeln ordnen die URLs *cdn/Images*, *cdn/Content*, *cdn/Scripts*und *cdn/bundles* ihren jeweiligen Stammordnern in der Webrolle zu.
