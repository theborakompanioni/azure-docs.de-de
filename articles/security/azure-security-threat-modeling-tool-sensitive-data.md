...
## <a name="a-idbinaries-infoaensure-that-binaries-are-obfuscated-if-they-contain-sensitive-information"></a><a id="binaries-info"></a>Sicherstellen, dass Binärdateien verschleiert werden, wenn sie sensible Informationen enthalten
...
## <a name="a-idfilesystemaensure-that-sensitive-data-stored-by-the-application-on-the-file-system-is-encrypted"></a><a id="filesystem"></a>Sicherstellen, dass von der Anwendung im Dateisystem gespeicherte sensible Daten verschlüsselt sind
...
## <a name="a-idcache-browseraensure-that-sensitive-content-is-not-cached-on-the-browser"></a><a id="cache-browser"></a>Sicherstellen, dass sensible Inhalte nicht im Browser zwischengespeichert werden
...
## <a name="a-idencrypt-dataaencrypt-sections-of-web-apps-configuration-files-that-contain-sensitive-data"></a><a id="encrypt-data"></a>Verschlüsseln von Abschnitten der Web-App-Konfigurationsdateien, die sensible Daten enthalten
...
## <a name="a-idautocomplete-inputaexplicitly-disable-the-autocomplete-html-attribute-in-sensitive-forms-and-inputs"></a><a id="autocomplete-input"></a>Explizites Deaktivieren des autocomplete-HTML-Attributs in sensiblen Formularen und Eingabeumgebungen
...
| Zutreffende Technologien | Allgemein |
| Attribute              | N/V  |
| Referenzen              | [MSDN: autocomplete attribute](http://msdn.microsoft.com/library/ms533486(VS.85).aspx) (MSDN: autocomplete-Attribut), [Using AutoComplete in HTML Forms](http://msdn.microsoft.com/library/ms533032.aspx) (Verwenden von AutoComplete in HTML-Formularen), [Sicherheitsanfälligkeit bezüglich HTML-Bereinigung](http://technet.microsoft.com/security/bulletin/MS10-071), [Autocomplete..again?!](http://blog.mindedsecurity.com/2011/10/autocompleteagain.html) (Autocomplete – Schon wieder?) |
...
## <a name="a-iddata-maskaensure-that-sensitive-data-displayed-on-the-user-screen-is-masked"></a><a id="data-mask"></a>Sicherstellen, dass auf dem Benutzerbildschirm angezeigte sensible Daten maskiert sind
...
## <a name="a-iddynamic-usersaimplement-dynamic-data-masking-to-limit-sensitive-data-exposure-non-privileged-users"></a><a id="dynamic-users"></a>Implementieren der dynamischen Datenmaskierung zum Beschränken der Offenlegung von sensiblen Daten für Benutzer ohne Berechtigungen
...
## <a name="a-idsalted-hashaensure-that-passwords-are-stored-in-salted-hash-format"></a><a id="salted-hash"></a>Sicherstellen, dass Kennwörter im Salt-Hashformat gespeichert werden
...
## <a name="a-iddb-encryptedaensure-that-sensitive-data-in-database-columns-is-encrypted"></a><a id="db-encrypted"></a>Sicherstellen, dass sensible Daten in Datenbankspalten verschlüsselt sind
...
## <a name="a-idtde-enabledaensure-that-database-level-encryption-tde-is-enabled"></a><a id="tde-enabled"></a>Sicherstellen, dass die Verschlüsselung auf Datenbankebene (TDE) aktiviert ist
...
## <a name="a-idbackupaensure-that-database-backups-are-encrypted"></a><a id="backup"></a>Sicherstellen, dass Datenbanksicherungen verschlüsselt sind
...
## <a name="a-idapi-browseraensure-that-sensitive-data-relevant-to-web-api-is-not-stored-in-browsers-storage"></a><a id="api-browser"></a>Sicherstellen, dass für die Web-API relevante sensible Daten nicht im Speicher des Browsers gespeichert werden
...
## <a name="a-idencrypt-docdbaencrypt-sensitive-data-stored-in-documentdb"></a><a id="encrypt-docdb"></a>Verschlüsseln von sensiblen Daten, die in DocumentDB gespeichert werden
...
## <a name="a-iddisk-vmause-azure-disk-encryption-to-encrypt-disks-used-by-virtual-machines"></a><a id="disk-vm"></a>Verwenden von Azure Disk Encryption zum Verschlüsseln von Datenträgern, die von virtuellen Computern verwendet werden
...
## <a name="a-idfabric-appsaencrypt-secrets-in-service-fabric-applications"></a><a id="fabric-apps"></a>Verschlüsseln von Geheimnissen in Service Fabric-Anwendungen
...
## <a name="a-idmodeling-teamsaperform-security-modeling-and-use-business-unitsteams-where-required"></a><a id="modeling-teams"></a>Durchführen der Sicherheitsmodellierung und Verwenden von Geschäftseinheiten/Teams nach Bedarf
...
## <a name="a-identitiesaminimize-access-to-share-feature-on-critical-entities"></a><a id="entities"></a>Einschränken des Zugriffs auf die Funktion „Freigeben“ für kritische Entitäten
...
## <a name="a-idgood-practicesatrain-users-on-the-risks-associated-with-the-dynamics-crm-share-feature-and-good-security-practices"></a><a id="good-practices"></a>Schulen von Benutzern in Bezug auf die Risiken der Funktion „Freigeben“ von Dynamics CRM und bewährte Sicherheitsmethoden
...
## <a name="a-idexception-mgmtainclude-a-development-standards-rule-proscribing-showing-config-details-in-exception-management"></a><a id="exception-mgmt"></a>Einbinden einer Entwicklungsstandardregel, die das Anzeigen von Konfigurationsdetails bei der Ausnahmeverwaltung vorschreibt
...
## <a name="a-idsse-previewause-azure-storage-service-encryption-sse-for-data-at-rest-preview"></a><a id="sse-preview"></a>Verwenden von Azure Storage Service Encryption (SSE) für ruhende Daten (Vorschau)
...
## <a name="a-idclient-storageause-client-side-encryption-to-store-sensitive-data-in-azure-storage"></a><a id="client-storage"></a>Verwenden der clientseitigen Verschlüsselung zum Speichern von sensiblen Daten in Azure Storage
...
## <a name="a-idpii-phonesaencrypt-sensitive-or-pii-data-written-to-phones-local-storage"></a><a id="pii-phones"></a>Verschlüsseln von sensiblen Daten oder personenbezogenen Daten, die in den lokalen Speicher von Smartphones geschrieben werden
...
## <a name="a-idbinaries-endaobfuscate-generated-binaries-before-distributing-to-end-users"></a><a id="binaries-end"></a>Verschleiern von generierten Binärdateien vor der Verteilung an Endbenutzer
...
| Zutreffende Technologien | Allgemein |
| Attribute              | N/V  |
| Referenzen              | [Crypto Obfuscator For .Net](http://www.ssware.com/cryptoobfuscator/obfuscator-net.htm) (Crypto Obfuscator für .NET) |
| Schritte | Generierte Binärdateien (Assemblys in apk) sollten verschleiert werden, um das Reverse Engineering von Assemblys zu verhindern. Hierfür können Tools wie Crypto Obfuscator verwendet werden. |

## <a name="a-idcertaset-clientcredentialtype-to-certificate-or-windows"></a><a id="cert"></a>Festlegen von „clientCredentialType“ auf „Certificate“ oder „Windows“
...
## <a name="a-idsecurityawcf-security-mode-is-not-enabled"></a><a id="security"></a>WCF-Sicherheitsmodus ist nicht aktiviert
