...
- Jeder Windows-Computer muss per HTTPS eine Verbindung mit dem Internet herstellen können. Diese Verbindung kann direkt, über einen Proxy oder über den OMS Log Analytics Forwarder hergestellt werden.
...
8. Nach Abschluss wird der **Microsoft Monitoring Agent** in der **Systemsteuerung** angezeigt. Dort können Sie Ihre Konfiguration überprüfen und sicherstellen, dass der Agent mit Operational Insights (OMS) verbunden ist. Wenn die Verbindung mit OMS hergestellt ist, zeigt der Agent die folgende Meldung an: **The Microsoft Monitoring Agent has successfully connected to the Microsoft Operations Management Suite service** (Microsoft Monitoring Agent wurde erfolgreich mit dem Microsoft Operations Management Suite-Dienst verbunden).
...
- Ändern Sie das folgende Beispiel, und verwenden Sie es dann zur Installation des Agents über die Befehlszeile.
    ...
    >[AZURE.NOTE] Wenn Sie einen Agent aktualisieren möchten, müssen Sie die Skripting-API für Log Analytics verwenden. Im nächsten Abschnitt finden Sie Informationen zum Aktualisieren eines Agents.

    ```
    MMASetup-AMD64.exe /Q:A /R:N /C:"setup.exe /qn ADD_OPINSIGHTS_WORKSPACE=1 OPINSIGHTS_WORKSPACE_ID=<your workspace id> OPINSIGHTS_WORKSPACE_KEY=<your workspace key> AcceptEndUserLicenseAgreement=1"
    ```

## Aktualisieren des Agents und Hinzufügen eines Arbeitsbereichs mithilfe eines Skripts
...
>[AZURE.NOTE] Wenn Sie bereits die Befehlszeile oder Skripts zum Installieren oder Konfigurieren des Agents verwendet haben, sollten Sie wissen, dass `EnableAzureOperationalInsights` durch `AddCloudWorkspace` ersetzt wurde.

## Installieren des Agents mithilfe von DSC in Azure Automation

>[AZURE.NOTE] Mit diesem Beispielverfahren und -skript wird kein vorhandener Agent aktualisiert.
...
2.	Erstellen Sie Variablenassets in Azure Automation für *OPSINSIGHTS\_WS\_ID* und *OPSINSIGHTS\_WS\_KEY*. Legen Sie für *OPSINSIGHTS\_WS\_ID* Ihre OMS Log Analytics-Arbeitsbereichs-ID und für *OPSINSIGHTS\_WS\_KEY* den Primärschlüssel Ihres Arbeitsbereichs fest.
...
## Manuelles Konfigurieren eines Agents oder Hinzufügen zusätzlicher Arbeitsbereiche
...
3. Klicken Sie auf **Hinzufügen**, um das Feld **Add a Log Analytics Workspace** (Log Analytics-Arbeitsbereich hinzufügen) zu öffnen.
4. Fügen Sie die **Arbeitsbereichs-ID** und den **Arbeitsbereichsschlüssel (Primärschlüssel)** des Arbeitsbereichs ein, die Sie hinzufügen möchten. (Sie hatten diese Informationen zuvor in Editor kopiert.) Klicken Sie anschließend auf **OK**. ![Operational Insights konfigurieren](./media/log-analytics-windows-agents/add-workspace.png)

Nachdem die Daten von Computern erfasst wurden, die vom Agent überwacht werden, wird die Anzahl der von OMS überwachten Computer im OMS-Portal auf der Registerkarte **Verbundene Datenquellen** in den **Einstellungen** unter **Servers Connected** (Verbundene Server) angezeigt.
...
2.	Öffnen Sie **Microsoft Monitoring Agent**, und klicken Sie dann auf die Registerkarte **Operations Manager**. ![Microsoft Monitoring Agent – Registerkarte „Operations Manager“](./media/log-analytics-windows-agents/om-mg01.png)
