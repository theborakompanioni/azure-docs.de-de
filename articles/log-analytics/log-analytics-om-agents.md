...
## Umstellen einer Operations Manager-Gruppe auf einen neuen OMS-Arbeitsbereich
...
5. Führen Sie die Schritte unter **Operations Management Suite Onboarding Wizard** (Assistent zum Integrieren von Operations Management Suite) aus, und geben Sie die E-Mail-Adresse oder Telefonnummer und das Kennwort des mit dem neuen OMS-Arbeitsbereich verknüpften Administratorkontos ein.
    ...
    > [AZURE.NOTE] Auf der Seite **Operations Management Suite Onboarding Wizard: Select Workspace** (Assistent zum Integrieren von Operations Management Suite: Arbeitsbereich auswählen) wird der vorhandene Arbeitsbereich angezeigt, der verwendet wird.
...
2.  Wählen Sie die Option **Connected Sources** (Verbundene Quellen).
...
1.	Öffnen Sie die Operations Manager-Konsole, und wählen Sie den Arbeitsbereich **Administration** aus.
2.	Wählen Sie **Management Packs**, und geben Sie im Textfeld für die Suche die Zeichenfolge **Advisor** oder **Intelligence** ein.
...
## Entfernen der Integration für OMS
...
1.  Klicken Sie im OMS-Portal auf die Kachel **Einstellungen**.
2.	Wählen Sie die Option **Connected Sources** (Verbundene Quellen).
3.	In der Tabelle sollte im Abschnitt „System Center Operations Manager“ der Name der Verwaltungsgruppe angezeigt werden, die Sie aus dem Arbeitsbereich entfernen möchten. Klicken Sie unter der Spalte **Last Data** (Letzte Daten) auf **Entfernen**.  
4.	In einem Fenster werden Sie dazu aufgefordert zu bestätigen, dass Sie die Entfernung durchführen möchten. Klicken Sie auf **Ja**, um den Vorgang fortzusetzen.  
5.	Öffnen Sie die Operations Manager-Befehlsshell mit einem Konto, das Mitglied der Administratorrolle von Operations Manager ist.

    >[AZURE.WARNING] Vergewissern Sie sich, dass die Namen Ihrer benutzerdefinierten Management Packs nicht das Wort „Advisor“ oder „IntelligencePack“ enthalten, bevor Sie den Vorgang fortsetzen. Andernfalls werden diese mit den folgenden Schritten aus der Verwaltungsgruppe gelöscht.

6.	Geben Sie an der Eingabeaufforderung der Befehlsshell `Get-SCOMManagementPack -name "*advisor*" | Remove-SCOMManagementPack` ein.

7.	Geben Sie anschließend `Get-SCOMManagementPack -name “*IntelligencePack*” | Remove-SCOMManagementPack` ein.

8.	Öffnen Sie die Operations Manager-Betriebskonsole mit einem Konto, das Mitglied der Administratorrolle von Operations Manager ist.
9.	Wählen Sie unter **Verwaltung** den Knoten **Management Packs** aus, und geben Sie im Suchfeld den Text **Advisor** ein. Überprüfen Sie dann, ob die folgenden Management Packs noch in die Verwaltungsgruppe importiert werden:
...
>[AZURE.NOTE] Wenn der Computer, auf dem Sie dieses Skript ausführen, kein Verwaltungsserver ist, sollte darauf die Operations Manager 2012 SP1- oder R2-Befehlsshell installiert sein. Dies hängt von der Version Ihrer Verwaltungsgruppe ab.
