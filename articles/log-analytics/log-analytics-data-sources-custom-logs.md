<properties 
   pageTitle="Benutzerdefinierte Protokolle in Log Analytics | Microsoft Azure"
...
2.	Klicken Sie auf **Daten** und anschließend auf **Custom logs** (Benutzerdefinierte Protokolle).
...
>[AZURE.NOTE]Log Analytics behandelt Datums-/Uhrzeitangaben, die in einem Protokoll mithilfe des Zeitstempeltrennzeichens gesammelt werden, momentan als UTC-Werte. Dies wird jedoch bald geändert, sodass die Zeitzone für den Agent verwendet wird.
...
>[AZURE.NOTE] Sollte die RawData-Eigenschaft bei der Suche nicht vorhanden sein, müssen Sie unter Umständen Ihren Browser schließen und wieder öffnen.
...
| ManagementGroupName | Bei SCOM-Agents: Der Name der Verwaltungsgruppe. Bei anderen Agents: AOI-<Arbeitsbereich-ID>. |


## Protokollsuchvorgänge mit benutzerdefinierten Protokolleinträgen
...
|:--|:--|
| Type=MyApp\_CL | Alle Ereignisse aus einem benutzerdefinierten Protokoll namens „MyApp\_CL“. |
| Type=MyApp\_CL Severity\_CF=error | Alle Ereignisse aus einem benutzerdefinierten Protokoll namens „MyApp\_CL“ mit dem Wert *error* in einem benutzerdefinierten Feld namens *Severity\_CF*. |
...
Wir verwenden den Namen *MyApp\_CL* und geben eine Beschreibung ein.
...
Wir verwenden die Abfrage *Type=MyApp\_CL*, um alle Datensätze aus dem gesammelten Protokoll zurückzugeben.
