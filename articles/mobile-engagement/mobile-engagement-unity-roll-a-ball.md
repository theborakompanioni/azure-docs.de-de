...
1. Öffnen Sie den **Unity-Editor**, und klicken Sie auf **New** (Neu). 
   
    ![][51] 
2. Stellen Sie unter **Project name** (Projektname) & **Location** (Speicherort) einen Projektnamen und Speicherort bereit, wählen Sie **3D** aus, und klicken Sie auf **Create project** (Projekt erstellen).
   
    ![][52]
3. Speichern Sie die gerade erstellte Standardszene unter dem Namen **MiniGame** innerhalb eines neuen Ordners **\_Scenes** (_Szenen) unter **Assets** (Bestand) als Teil des neuen Projekts:
   
    ![][53]
4. Erstellen Sie ein 3D-Objekt über **3D Object > Plane** (Ebene) als Spielfeld, und geben Sie diesem Ebenenobjekt den Namen **Ground** (Boden).
   
    ![][1]
5. Setzen Sie die Transformationskomponente für dieses **Ground** -Objekt zurück, sodass sie wieder am Ursprung ist. 
   
    ![][3]
6. Deaktivieren Sie im **Gizmos**-Menü die Option **Show Grid** (Raster anzeigen) für das **Ground**-Objekt.
   
    ![][4]
7. Setzen Sie die Komponente **Scale** (Skalierung) für das **Ground**-Objekt auf die folgenden Werte: X = 2, Y = 1, Z = 2. 
   
    ![][5]
8. Fügen Sie dem Projekt ein neues 3D-Objekt über **3D Object > Sphere** (Kugel) hinzu, und geben Sie diesem Kugelobjekt den Namen **Player** (Spieler). 
   
    ![][6]
9. Wählen Sie das **Player**-Objekt aus, und klicken Sie auf **Reset Transform** (Transformation zurücksetzen) ähnlich wie beim Plane-Objekt. 
10. Setzen Sie die Komponente **Transform > Position > Y Coordinate** (y-Koordinate) für Player Y auf 0,5.  
...
12. Erstellen Sie in diesem Ordner ein neues **Material** namens **Background** (Hintergrund). 
...
14. Ziehen Sie dieses Material in die Szenenansicht, um die Farbe auf das **Ground** -Objekt anzuwenden. 
    ...
    ![][10]
15. Setzen Sie für mehr Klarheit abschließend **Transform > Rotation > Y** des Directional Light-Objekts (diffuses Licht) auf 60. 
    
    ![][12]

### <a name="moving-the-player"></a>Bewegen des Players
...
3. Klicken Sie auf **Add Component (Komponente hinzufügen) > New Script (Neues Skript) > C# Script**. Geben Sie den Namen **PlayerContoller** ein, und klicken Sie auf **Create and Add** (Erstellen und Hinzufügen). Damit wird ein Skript erstellt und dem Player-Objekt hinzugefügt.  
   
    ![][14]
4. Verschieben Sie dieses Skript im Projekt in den Ordner **Scripts** . 
5. Öffnen Sie das Skript zur Bearbeitung in Ihrem bevorzugten Skripteditor. Aktualisieren Sie den Skriptcode durch den folgenden Code, und speichern Sie ihn. 
...
6. Beachten Sie, dass das obige Skript eine **Speed** -Eigenschaft verwendet. Setzen Sie im Unity-Editor die Speed-Eigenschaft für die Geschwindigkeit auf 10.  
   
    ![][15]
7. Klicken Sie im Unity-Editor auf **Play** (Spielen). Jetzt sollten Sie die Kugel mit der Tastatur steuern können. Die Kugel sollte sich drehen und bewegen. 

### <a name="moving-the-camera"></a>Bewegen der Kamera
Die folgenden Schritte stammen aus dem [Unity-Tutorial](https://unity3d.com/learn/tutorials/projects/roll-a-ball/moving-the-camera?playlist=17141). Mit diesen Schritten wird die **Main Camera** (Hauptkamera) an das **Player**-Objekt gebunden. 

1. Setzen Sie **Transform.Position** auf X = 0, Y = 10,5, Z = -10.  
2. Setzen Sie **Transform.Rotation** auf X = 45, Y = 0, Z = 0.  
   
    ![][16]
3. Fügen Sie ein neues Skript namens **CameraController** der **MainCamera** hinzu, und verschieben Sie es in den Ordner „Scripts“. 
   
    ![][17]
4. Öffnen Sie das Skript zur Bearbeitung, und fügen Sie den folgenden Code ein:
...
### <a name="setting-up-the-play-area"></a>Einrichten des Spielbereichs
...
1. Klicken Sie auf **Create (Erstellen) > Create Empty (Leer erstellen) > Game Object (Spielobjekt)**, und geben Sie ihm den Namen **Walls** (Wände).
   
    ![][19]
2. Erstellen Sie unter diesem Walls-Objekt ein neues 3D-Objekt über **3D Object > Cube** (Würfel) mit dem Namen „West wall“ (Westwand). 
...
4. Duplizieren Sie die „West Wall“, um eine **East wall** mit der aktualisierten Transformationsposition und -skalierung zu erstellen. 
    ...
    ![][22]
5. Duplizieren Sie die „East Wall“ (Ostwand), um eine **North wall** (Nordwand) mit der aktualisierten Transformationsposition und -skalierung zu erstellen. 
   
    ![][23]
6. Duplizieren Sie die „North Wall“, um eine **South wall** (Südwand) mit der aktualisierten Transformationsposition und -skalierung zu erstellen. 
   
    ![][24]

### <a name="creating-collectible-objects"></a>Erstellen von sammelbaren Objekten
...
2. Passen Sie die Werte für **Transform > Rotation** & **Transform > Scale** des Pickup-Objekts (Mitnehmobjekt) an. 
...
6. Erstellen Sie im Projekt einen neuen Ordner namens **Prefabs** 
...
### <a name="updating-the-game-for-mobile-play"></a>Aktualisieren des Spiels für mobile Geräte
Mit den obigen Abschnitten ist das Tutorial von Unity abgeschlossen. Jetzt nehmen wir ein paar Änderungen vor, um das Spiel auch auf mobilen Geräten spielen zu können. Beachten Sie, dass wir zu Testzwecken bislang das Spiel mit Tastatureingaben gesteuert haben. Das Spiel wird nun so geändert, dass der Player durch Bewegen des Telefons bewegt wird, d.h., es wird ein Beschleunigungsmesser als Eingabe verwendet. 
