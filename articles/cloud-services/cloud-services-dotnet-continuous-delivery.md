...
## 1: Konfigurieren des Buildservers
...
Visual Studio muss nicht auf dem Buildserver installiert werden. Wenn Sie Team Foundation-Builddienst zum Verwalten des Buildservers verwenden möchten, folgen Sie den Anweisungen in der Dokumentation [Einrichten des Team Foundation-Builddiensts][Einrichten des Team Foundation-Builddiensts].
...
## 2: Erstellen eines Pakets mithilfe von MSBuild-Befehlen
...
3. Führen Sie MSBuild wie im folgenden Beispiel mit der Option „/target:Publish“ aus:
   ...
   Den Projektnamen können Sie optional als MSBuild-Parameter angeben. Wird er nicht angegeben, wird das aktuelle Verzeichnis verwendet. Weitere Informationen über MSBuild-Befehlszeilenoptionen finden Sie unter [MSBuild-Befehlszeilenreferenz](1.md).
...
## 3: Erstellen eines Pakets mit TFS Team Build
Wenn Sie Team Foundation Server (TFS) als Buildcontroller und den Buildserver als TFS-Buildcomputer eingerichtet haben, können Sie optional für Ihr Azure-Paket einen automatischen Build einrichten. Informationen zum Einrichten und Verwenden von Team Foundation Server als Buildsystem finden Sie unter [Skalieren des Buildsystems][Skalieren des Buildsystems]. Beim folgenden Verfahren wird insbesondere vorausgesetzt, dass Sie Ihren Buildserver, wie unter [Bereitstellen und Konfigurieren eines Buildservers][Bereitstellen und Konfigurieren eines Buildservers] beschrieben, konfiguriert und ein Teamprojekt sowie ein Clouddienstprojekt in diesem Teamprojekt erstellt haben.
...
## 4: Ein Paket mit einem PowerShell-Skript veröffentlichen
...
1. Installieren Sie die [Azure PowerShell-Cmdlets][Azure PowerShell-Cmdlets] \(Version 0.6.1 oder höher). Wählen Sie während der Cmdlet-Einrichtung die Installation als Snap-In. Diese offiziell unterstützte Version ersetzt die ältere von CodePlex angebotene Version, auch wenn diese die Versionsnummer 2.x.x haben.
...
## 5: Veröffentlichen eines Pakets mit TFS Team Build
