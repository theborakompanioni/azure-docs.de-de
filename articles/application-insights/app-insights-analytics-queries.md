...
* *Typ:* `bool`, `int`, `long`, `double`, `string`, `timespan`, `datetime`, `guid`, [`dynamic`](app-insights-analytics-scalars.md#dynamic-type)
...
Für die Anzahl von Zeilen, die an den Client zurückgegeben werden, besteht eine implizite Begrenzung, auch wenn Sie nicht `take` verwenden. Zum Aufheben dieser Begrenzung verwenden Sie die Client-Anforderungsoption `notruncation`.
...
Die Argumente müssen numerische Werte, Datums- oder TimeSpan-Werte sein. Sie können nicht auf die Spalten einer Tabelle verweisen. (Wenn Sie den Bereich basierend auf einer Eingabetabelle berechnen möchten, verwenden Sie die [range*-Funktion*](app-insights-analytics-scalars.md#range), vielleicht mit dem [mvexpand-Operator](#mvexpand-operator).)
...
[Weitere Beispiele](app-insights-analytics-aggregations.md).
...
* *Aggregation:* Ein Aufruf einer [Aggregationsfunktion](app-insights-analytics-aggregations.md) wie z. B. `count()` oder `avg()` mit Spaltennamen als Argumente. Siehe die [Liste der Aggregationsfunktionen](app-insights-analytics-aggregations.md).
...
* *Predicate:* Ein `boolean`-[Ausdruck](app-insights-analytics-scalars.md#boolean) über Spalten von *T*. Er wird für jede Zeile in *T* ausgewertet.
