...
Suchen Sie die Anzahl von Anfragen eines jeden Namens innerhalb der letzten Stunde:

    requests | where timestamp > ago(1h)
    | summarize req_count = sum(itemCount) by name

> [AZURE.NOTE] Verwenden Sie die Eigenschaft „itemCount“ statt einfach die Anfragedatenpunkte zu zählen. Dies kompensiert jedes [Sampling](app-insights-sampling.md), das möglicherweise gerade ausgeführt wird. Wenn das Sampling beispielsweise auf 33 % festgelegt wird, entspricht itemCount in jedem Datenpunkt 3.

#### Min und Max

Suchen Sie die minimalen und maximalen Zeitstempel aller Datensätze im Anforderungsdatenstrom. Es gibt keine group-by-Klausel, daher gibt es nur eine Zeile in der Ausgabe:
...
* *MaxListSize* ist eine optionale Integer-Beschränkung für die maximale Anzahl von zurückgegebenen Elementen (Standardwert: *128*).

## makeset

    makeset(Expression [ , MaxSetSize ] )

Gibt ein `dynamic` (JSON)-Array des Satzes von unterschiedlichen Werten zurück, die *Expr* in der Gruppe annimmt. (Tipp: Verwenden Sie [`dcount`](#dcount), um die unterschiedlichen Werte lediglich zu zählen.)
  
*  *MaxSetSize* ist eine optionale Integer-Beschränkung für die maximale Anzahl von zurückgegebenen Elementen (Standardwert: *128*).
...
Siehe auch den [`mvexpand`-Operator](app-insights-analytics-queries.md#mvexpand-operator) für die umgekehrte Funktion.
