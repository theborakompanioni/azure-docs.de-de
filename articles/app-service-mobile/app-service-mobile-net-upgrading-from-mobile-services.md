---
title: Upgrade von Mobile Services auf Azure App Service
...
> [!TIP]
> Es wird empfohlen, vor einem Upgrade eine [Migration auszuführen](app-service-mobile-migrating-from-mobile-services.md) . Auf diese Weise können Sie beide Versionen der Anwendung in den gleichen App Service-Plan übernehmen, ohne dass zusätzliche Kosten anfallen.
>
>

### <a name="improvements-in-mobile-apps-net-server-sdk"></a>Verbesserungen im Mobile Apps-.NET-Server-SDK
...
* Mehr Flexibilität im mobilen SDK. Sie können explizit steuern, welche Features und Routen eingerichtet werden, z. B. Authentifizierung, Tabellen-APIs und der Endpunkt für die Pushregistrierung. Weitere Informationen finden Sie unter [Verwenden des .NET-Server-SDK für Azure Mobile Apps](app-service-mobile-dotnet-backend-how-to-use-server-sdk.md).
...
> [!TIP]
> Es wird empfohlen, die restlichen Abschnitte in diesem Thema vollständig zu lesen, bevor Sie mit einem Upgrade beginnen. Notieren Sie sich alle Features, die Sie verwenden möchten und die nachfolgend behandelt werden.
>
>
...
## <a name="updating-the-server-project"></a>Aktualisieren des Serverprojekts
...
Wenn Sie den oben genannten Schritt ausführen, stellen Sie sicher, dass „MS_MobileServiceName“ festgelegt ist. Sie können einen anderen Schemanamen angeben, falls dieser in der Anwendung zuvor angepasst wurde.
...
#### <a name="local-store"></a>Lokaler Speicher
...
### <a name="controller-action-authorization"></a>Controlleraktionsautorisierung
Alle Instanzen des `[AuthorizeLevel(AuthorizationLevel.User)]`-Attributs müssen jetzt so geändert werden, dass das ASP.NET-Standardattribut `[Authorize]` verwendet wird. Darüber hinaus sind Controller jetzt, wie in anderen ASP.NET-Anwendungen, standardmäßig anonym.
Wenn Sie bisher eine der anderen AuthorizeLevel-Optionen, wie z. B. „Admin“ oder „Application“ verwendet haben, beachten Sie, dass diese nicht mehr vorhanden sind. Stattdessen können Sie AuthorizationFilters für gemeinsam genutzte geheime Schlüssel einrichten oder einen AAD-Dienstprinzipal konfigurieren, um sichere Dienst-zu-Dienst-Aufrufe zu ermöglichen.

### <a name="getting-additional-user-information"></a>Abrufen weiterer Benutzerinformationen
...
[Ich verwende bereits Websites und mobile Dienste – welche Vorteile bietet App Service?]: /en-us/documentation/articles/app-service-mobile-value-prop-migration-from-mobile-services
...
[Hinzufügen von Authentifizierung zu Ihrer mobilen App]: app-service-mobile-xamarin-ios-get-started-users.md
...
[Migrieren von Mobile Services zu einer mobilen App Service-App]: app-service-mobile-migrating-from-mobile-services.md
[Migrieren des vorhandenen mobilen Diensts zu App Service]: app-service-mobile-migrating-from-mobile-services.md
