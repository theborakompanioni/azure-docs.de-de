...
## <a name="1----register-an-application-with-azure-ad"></a>*1.    Registrieren einer Anwendung unter Azure AD*
...
5. Folgen Sie den Eingabeaufforderungen, und erstellen Sie eine neue **native Clientanwendung** (Trotz der Tatsache, dass Cordova-Apps HTML-basiert sind, erstellen wir hier eine native Clientanwendung. Daher muss die Option `Native Client Application` ausgewählt werden, andernfalls funktioniert die Anwendung nicht). Geben Sie einen Anzeigenamen für die Anwendung ein, und wählen Sie als Anwendungstyp „Nativ“. Beim **Umleitungs-URI** handelt es sich um den URI, mit dem Token an Ihre App zurückgegeben werden, beispielsweise `http://MyDirectorySearcherApp`. Klicken Sie zum Erstellen der Anwendung auf **Erstellen**.
...
8. Konfigurieren der Berechtigungen für Ihre Anwendung: Wählen Sie im Menü „Einstellungen“ den Abschnitt „Erforderliche Berechtigungen“, klicken Sie auf **Hinzufügen**, und **wählen Sie eine API** und anschließend „Windows Azure Active Directory“ (die AADGraph-API) aus. Klicken Sie auf **Berechtigungen auswählen**, und wählen Sie „Access the directory as the signed-in user“ (Als angemeldeter Benutzer auf das Verzeichnis zugreifen).

## <a name="2-clone-the-sample-app-repository-required-for-the-tutorial"></a>*2. Klonen des Beispiel-App-Repositorys, das für das Lernprogramm erforderlich ist*
...
## <a name="3-add-code-to-authenticate-users-and-obtain-tokens-from-aad"></a>*3. Hinzufügen von Code zum Authentifizieren von Benutzern und Abrufen von Token aus AAD*
