...
> [!WARNING]
> In diesem Tutorial wird aktuell eine veraltete, nicht mehr unterstützte Clientbibliothek verwendet, `Microsoft.Experimental.IdentityModel.Clients.ActiveDirectory` (ADAL, experimentell).  Wir arbeiten daran, dieses Tutorial auf die `Microsoft.Identity.Client` -Vorschaubibliothek (MSAL) zu aktualisieren. In der Zwischenzeit wird empfohlen, die Verwendung von ADAL (experimentell) in diesem Tutorial durch MSAL zu ersetzen.  Weitere Informationen zu Ihren Optionen bei der Auswahl einer Clientbibliothek finden Sie im Artikel zu den [Beschränkungen](active-directory-v2-limitations.md).
> 
> 

Dieses Tutorial konzentriert sich hauptsächlich auf das Abrufen und Verwenden von Zugriffstoken mithilfe von ADAL in einer Web-App. Eine vollständige Beschreibung finden Sie [hier](active-directory-v2-flows.md#web-apps).  Vorab möchten Sie vielleicht erfahren, wie eine [grundlegende Anmeldefunktion zu einer Web-App hinzugefügt wird](active-directory-v2-devquickstarts-dotnet-web.md) oder wie [eine Web-API ausreichend geschützt wird](active-directory-v2-devquickstarts-dotnet-api.md).
...
## <a name="use-adal-to-get-access-tokens"></a>Verwenden von ADAL zum Abrufen von Zugriffstoken
...
## <a name="4.-call-the-web-api"></a>4. Aufrufen der Web-API
