...
# Verwenden von Rollenzuweisungen zum Verwalten Ihrer Azure-Abonnementressourcen

> [AZURE.SELECTOR]
- [Azure-Portal](role-based-access-control-azure-portal.md)
- [Klassisches Azure-Portal](role-based-access-control-configure.md)

Die rollenbasierte Access Control in Azure (RBAC) ermöglicht eine präzise Zugriffsverwaltung für Azure. Mit RBAC können Sie den Benutzern nur die Zugriffsrechte gewähren, die diese zum Ausführen ihrer Aufgaben benötigen. Dieser Artikel hilft Ihnen bei der Einrichtung von RBAC im Azure-Portal. Weitere Informationen dazu, wie RBAC Sie bei der Zugriffsverwaltung unterstützt, finden Sie unter [Erste Schritte mit der Zugriffsverwaltung im Azure-Portal](role-based-access-control-what-is.md).

## Zugriff anzeigen
Auf dem Hauptblatt im [Azure-Portal](https://portal.azure.com) können Sie ermitteln, wer Zugriff auf eine Ressource, Ressourcengruppe oder ein Abonnement hat. Wir möchten beispielsweise sehen, wer Zugriff auf eine unserer Ressourcengruppen hat:

1. Wählen Sie auf der Navigationsleiste im linken Bereich die Option **Ressourcengruppen** aus. ![Ressourcengruppen – Symbole](./media/role-based-access-control-configure/resourcegroups_icon.png)
2. Wählen Sie auf dem Blatt **Ressourcengruppen** den Namen der Ressourcengruppe aus.
3. Wählen Sie rechts oben auf dem Blatt mit der Ressourcengruppe die Option **Benutzer** aus. ![Benutzer – Symbol](./media/role-based-access-control-configure/users_icon.png)
...
2. Wählen Sie auf dem Blatt **Rolle auswählen** die Rolle aus, die Sie zuweisen möchten.
...
4. Wählen Sie **OK** aus, um die Zuweisung zu erstellen. Den Status können Sie im Popupfenster **Benutzer wird hinzugefügt** verfolgen. ![Statusanzeige „Der Benutzer wird hinzugefügt“ – Screenshot](./media/role-based-access-control-configure/addinguser_popup.png)
...
2. Wählen Sie auf dem Blatt mit den Zuweisungsdetails die Option **Entfernen** aus. ![Entfernen – Symbol](./media/role-based-access-control-configure/remove_icon.png)
3. Wählen Sie **Ja** aus, um die Entfernung zu bestätigen. ![Blatt „Benutzer“ – Aus Rolle entfernen – Screenshot](./media/role-based-access-control-configure/remove-access1.png)

Geerbte Zuweisungen können nicht entfernt werden. In der folgenden Abbildung sehen Sie, dass die „Entfernen“-Schaltfläche ausgegraut ist. Sehen Sie sich stattdessen **Zugewiesen zu** an. Sie können die Rollenzuweisung entfernen, indem Sie zu der dort aufgelisteten Ressource gehen.
