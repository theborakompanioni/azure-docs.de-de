...
Durch Verwendung der gleichen Anmeldeinformationen (Benutzername und Kennwort) für den Zugriff auf Ihre Unternehmensressourcen und Ihre cloudbasierten Dienste können Sie dafür sorgen, dass Ihre Benutzer sich nicht mehrere Anmeldeinformationen merken müssen. So sinkt die Wahrscheinlichkeit, dass ein Benutzer seine Anmeldedaten vergisst. Gleichzeitig wird der Aufwand des Helpdesks für Kennwortzurücksetzungen reduziert.

In vielen Organisationen ist es ausreichend, die Azure AD-[Kennwortsynchronisierung](active-directory-aadconnectsync-implement-password-synchronization.md) zu verwenden, um Benutzern einen einzigen Satz Anmeldeinformationen für den Zugriff auf lokale und cloudbasierte Ressourcen zu bieten. In anderen Organisationen dagegen dürfen Kennwörter selbst in Hashformat die internen Organisationsgrenzen nicht verlassen.

Die Azure AD-Passthrough-Authentifizierung bietet diesen Kunden eine einfache Lösung. Sie stellt sicher, dass die Kennwortüberprüfung für Azure AD-Dienste im Abgleich mit ihrem lokalen Active Directory erfolgt. Kennwörter können überprüft werden, ohne dass eine komplexe Netzwerkinfrastruktur notwendig ist oder lokale Kennwörter in irgendeiner Form in der Cloud vorhanden sein müssen.

Bei Kombination mit der Option [Einmaliges Anmelden](active-directory-aadconnect-sso.md) müssen Benutzer nicht ihr Kennwort eingeben, um sich bei Azure AD oder anderen Clouddiensten anzumelden. Dieses Feature bietet diesen Kunden eine wirklich integrierte Umgebung auf den Computern ihres Unternehmens.

![Passthrough-Authentifizierung](./media/active-directory-aadconnect-pass-through-authentication/pta1.png)

Die Passthrough-Authentifizierung kann über Azure AD Connect konfiguriert werden und nutzt einen einfachen lokalen Agent, der auf Anforderungen einer Kennwortüberprüfung lauscht. Der Agent lässt sich problemlos auf mehreren Computern bereitstellen, um für hohe Verfügbarkeit und Lastenausgleich zu sorgen. Da die gesamte Kommunikation ausgehend ist, ist weder eine DMZ erforderlich, noch muss der Connector in einer DMZ installiert werden. Folgende Computeranforderungen gelten für den Connector:
...
>[!NOTE]
>Während der Vorschauphase ist die Kennwortsynchronisierung standardmäßig aktiviert, wenn die Passthrough-Authentifizierung als Anmeldeoption in Azure AD Connect ausgewählt wird. Diese Einstellung kann auf der Seite „Optionen“ von Azure AD Connect deaktiviert werden.
...
Der lokale Domänencontroller wertet die Anforderung aus und gibt eine Antwort an den Connector zurück, der diese wiederum an Azure AD zurückgibt. Azure AD wertet die Antwort aus und antwortet dem Benutzer entsprechend, z.B. durch Ausgeben eines Tokens oder Anfordern einer Multi-Factor Authentication. Dieses Diagramm zeigt die verschiedenen Schritte:
...
- Ein Server unter Windows Server 2012 R2 oder höher, auf dem Azure AD Connect ausgeführt wird. Dieser Computer muss Mitglied der gleichen Gesamtstruktur sein wie die Benutzer, die überprüft werden sollen.
- Wenn Sie über mehrere Gesamtstrukturen mit Benutzern verfügen, die mithilfe von Azure AD überprüft werden sollen, müssen Vertrauensstellungen zwischen den Gesamtstrukturen bestehen.
...
Bei Fehlern in Zusammenhang mit dem Connector können Sie das Ereignisprotokoll des Connectorcomputers unter **Application and Service Logs\Microsoft\AadApplicationProxy\Connector\Admin** prüfen. Bei Bedarf sind ausführlichere Protokolle verfügbar, indem Sie die Analyse- und Debugprotokolle anzeigen und das Sitzungsprotokoll des Connectors aktivieren. Es wird nicht empfohlen, dieses Protokoll standardmäßig zu aktivieren, und dass der Inhalt erst sichtbar wird, nachdem das Protokoll deaktiviert wurde.
...
Das Ergebnis sieht etwa wie die folgende Antwort aus:

![Passthrough-Authentifizierung](./media/active-directory-aadconnect-pass-through-authentication/pta3.png)

Sofern die Überwachungsprotokollierung aktiviert ist, finden Sie weitere Informationen auch in den Sicherheitsprotokollen der Domänencontroller. Eine einfache Abfrage nach Authentifizierungsanforderungen durch den Connector sieht etwa folgendermaßen aus:
