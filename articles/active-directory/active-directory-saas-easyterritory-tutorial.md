...
- Sie können es Benutzern ermöglichen, sich mit ihren Azure AD-Konten automatisch bei EasyTerritory anzumelden (einmaliges Anmelden, SSO).
...
- Ein EasyTerritory-Abonnement, für das einmaliges Anmelden (SSO) aktiviert ist
...
In diesem Abschnitt konfigurieren und testen Sie das einmalige Anmelden (SSO) von Azure AD bei EasyTerritory basierend auf einem Testbenutzer mit dem Namen Britta Simon.

Damit einmaliges Anmelden (SSO) funktioniert, muss Azure AD wissen, welcher Benutzer in EasyTerritory als Entsprechung zu einem Benutzer in Azure AD fungiert. Anders ausgedrückt: Zwischen einem Azure AD-Benutzer und dem entsprechenden Benutzer in EasyTerritory muss eine Linkbeziehung eingerichtet werden.
...
Zum Konfigurieren und Testen des einmaligen Anmeldens (SSO) in Azure AD bei EasyTerritory sind die folgenden Bausteine erforderlich:
...
In diesem Abschnitt aktivieren Sie das einmalige Anmelden (SSO) von Azure AD im Azure-Verwaltungsportal und konfigurieren das einmalige Anmelden in Ihrer EasyTerritory-Anwendung.

**Führen Sie zum Konfigurieren des einmaligen Anmeldens (SSO) von Azure AD bei EasyTerritory die folgenden Schritte aus:**
...
2. Wählen Sie im Dialogfeld **Einmaliges Anmelden** als **Modus** die Option **SAML-basierte Anmeldung** aus, um einmaliges Anmelden (SSO) zu aktivieren.
...
3. Führen Sie im Abschnitt **Domäne und URLs für EasyTerritory** die folgenden Schritte aus, wenn Sie die Anwendung im **IDP-initiierten Modus** konfigurieren möchten:
   ...
   1. Geben Sie im Textfeld **Bezeichner** eine URL nach folgendem Muster ein: `https://apps.easyterritory.com/<tenant id>/DEV/`
   2. Geben Sie im Textfeld **Antwort-URL** eine URL nach folgendem Muster ein: `https://apps.easyterritory.com/<tenant id>/DEV/AuthServices/Acs`
...
4. Führen Sie im Abschnitt **Domäne und URLs für EasyTerritory** die folgenden Schritte aus, wenn Sie die Anwendung im **SP-initiierten Modus** konfigurieren möchten:
  ...
  1. Klicken Sie auf die Option **Erweiterte URL-Einstellungen anzeigen**.
  2. Geben Sie im Textfeld **Anmelde-URL** eine URL im folgenden Format ein: `https://<company name>.easyterritory.com/`

    >[!NOTE] 
    >Hierbei handelt es sich um Beispielwerte. Sie müssen diese Werte durch die tatsächliche Anmelde-URL, den tatsächlichen Bezeichner und die tatsächliche Antwort-URL ersetzen. Wenden Sie sich an das [EasyTerritory-Supportteam](mailto:sales@easyterritory.com), um diese Werte zu erhalten.
...
### <a name="create-an-easyterritory-test-user"></a>Erstellen eines EasyTerritory-Testbenutzers
...
In diesem Abschnitt ermöglichen Sie Britta Simon die Verwendung des einmaligen Anmeldens (SSO) von Azure, indem Sie ihr Zugriff auf EasyTerritory gewähren.
