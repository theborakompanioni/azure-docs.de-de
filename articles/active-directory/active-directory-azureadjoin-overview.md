---
title: "Erweitern von Cloudfunktionen auf Windows 10-Geräten über Azure Active Directory Join| Microsoft Docs"
...
---
# <a name="extending-cloud-capabilities-to-windows-10-devices-through-azure-active-directory-join"></a>Erweitern von Cloudfunktionen auf Windows 10-Geräte über Azure Active Directory Join
## <a name="what-is-azure-active-directory-join"></a>Was ist Azure Active Directory Join?
...
## <a name="why-should-enterprises-adopt-azure-ad-join"></a>Warum sollten Unternehmen Azure AD Join einführen?
...
## <a name="what-capabilities-does-azure-ad-join-offer"></a>Welche Funktionen bietet Azure AD Join?
...
* **Einmaliges Anmelden auf Unternehmensressourcen:** Benutzer profitieren vom einmaligen Anmelden vom Windows-Desktop bei Apps und Ressourcen in der Cloud, z.B. Office 365 und Tausenden von Geschäftsanwendungen, die zur Authentifizierung über [Azure AD Connect](active-directory-azureadjoin-deployment-aadjoindirect.md) auf Azure AD zurückgreifen. Unternehmenseigene Geräte, die mit Azure AD verknüpft sind, profitieren auch vom einmaligen Anmelden bei lokalen Ressourcen, wenn sich das Gerät im Unternehmensnetzwerk befindet, und von einer ortsunabhängigen Anmeldung, wenn diese Ressourcen über den [Azure AD-Anwendungsproxy](https://msdn.microsoft.com/library/azure/Dn768219.aspx) verfügbar gemacht werden.
...
## <a name="how-do-different-devices-work-with-azure-ad-join"></a>Wie funktionieren verschiedene Geräte mit Azure AD Join?
| Unternehmensgerät (mit lokaler Domäne verknüpft) | Unternehmensgerät (mit der Cloud verknüpft) | Persönliches Gerät |
| --- | --- | --- |
| Benutzer können sich bei Windows mit den geschäftlichen Anmeldeinformationen anmelden (wie dies aktuell der Fall ist). |Benutzer können sich bei Windows mit den Anmeldeinformationen Ihrer Arbeit anmelden, die in Azure AD verwaltet werden. Dies ist für Unternehmensgeräte in drei Situationen von Bedeutung: <ol><li>Die Organisation verfügt nicht über eine lokale Active Directory-Umgebung (z.B. ein kleines Unternehmen).</li><li>Die Organisation erstellt nicht alle Benutzerkonten in Active Directory (Konten für Studenten, Berater oder Saisonkräfte werden beispielsweise nicht in Active Directory erstellt).</li><li>Die Organisation verfügt über unternehmenseigene Geräte, für die der Beitritt zu einer (lokalen) Domäne nicht durchgeführt werden kann, z.B. Smartphones oder Tablets, auf denen eine Mobile SKU ausgeführt wird (beispielsweise ein Zweitgerät, das in einen anderen Bereich eines Werks oder eines Einzelhandelsgeschäfts mitgenommen wird).</li></ol> Azure AD Join unterstützt den Beitritt von unternehmenseigenen Geräten sowohl für verwaltete Unternehmen als auch für Unternehmen mit Partnerverbund. |Benutzer melden sich an Windows mit den Anmeldeinformationen ihres persönlichen Microsoft-Kontos an (keine Änderung). |
