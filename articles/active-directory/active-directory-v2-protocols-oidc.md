...
> [AZURE.TIP] Klicken Sie auf den Link unten, um diese Anforderung auszuführen. Nach der Anmeldung sollte der Browser mit einem `id_token` in der Adressleiste zu `https://localhost/myapp/` umgeleitet werden.  Im Rahmen dieses Tutorials wird bei dieser Anforderung `response_mode=query` verwendet.  Empfohlen wird die Verwendung von `response_mode=form_post`.
...
> [AZURE.TIP] Klicken Sie auf den Link unten, um diese Anforderung auszuführen. Nach der Anmeldung sollte der Browser mit einem `id_token` und einem `code` in der Adressleiste zu `https://localhost/myapp/` umgeleitet werden.  Im Rahmen dieses Tutorials wird bei dieser Anforderung `response_mode=query` verwendet.  Empfohlen wird die Verwendung von `response_mode=form_post`.
