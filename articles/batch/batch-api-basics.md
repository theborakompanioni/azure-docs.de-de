...
- **Start**: Dieser Speicherort wird von einem Starttask als Arbeitsverzeichnis verwendet. Alle Dateien, die vom Batch-Dienst heruntergeladen werden, um die Startaufgabe zu starten, werden ebenfalls unter diesem Verzeichnis gespeichert. Auf dem Knoten steht das Startverzeichnis über die `%AZ_BATCH_NODE_STARTUP_DIR%`-Umgebungsvariable zur Verfügung. Der Starttask kann Dateien in diesem Verzeichnis erstellen, lesen, aktualisieren und löschen, und dieses Verzeichnis kann von Starttasks zum Konfigurieren des Betriebssystems verwendet werden.
