...
Zu diesem Zweck wird im App Service-Plan die Eigenschaft für die **Skalierung pro Standort** auf TRUE festgelegt (`"perSiteScaling": true`), und für die App wird die **Anzahl der zu verwendeten Worker** auf 1 festgelegt `"properties": { "numberOfWorkers": "1" }`.
...
Die **Skalierung pro App** ist ein Feature, das sowohl in den öffentlichen Azure-Regionen als auch in App Service-Umgebungen aktiviert ist. Es wird jedoch empfohlen, die App Service-Umgebungen mit ihren hoch entwickelten Features und größeren Kapazitätspools zu nutzen.
