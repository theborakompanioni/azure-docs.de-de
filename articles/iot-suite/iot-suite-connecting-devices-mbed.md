...
### <a name="walk-through-the-code"></a>Einführung in den Code
...
#### <a name="connecting-the-model-to-the-library"></a>Herstellen einer Verbindung zwischen Modell und Bibliothek
Die Funktionen **sendMessage** und **IoTHubMessage** sind Codebausteine zum Senden von Telemetriedaten vom Gerät und zum Verbinden der Nachrichten von IoT Hub mit den Befehlshandlern.

#### <a name="the-remotemonitoringrun-function"></a>Funktion „remote_monitoring_run“
Die Funktion **main** des Programms ruft die Funktion **remote_monitoring_run** auf, wenn die Anwendung gestartet wird, um das Verhalten des Geräts als IoT Hub-Geräteclient auszuführen. Die Funktion **remote_monitoring_run** besteht hauptsächlich aus geschachtelten Funktionspaaren:

* Mit **platform\_init** und **platform\_deinit** werden plattformspezifische Vorgänge zum Initialisieren und Herunterfahren durchgeführt.
...
Im Hauptteil der Funktion **remote_monitoring_run** führt das Programm die folgenden Vorgänge mit dem Handle **iotHubClientHandle** durch:
...
* Startet eine **while** -Schleife, mit der jede Sekunde Werte zu Temperatur, Außentemperatur und Luftfeuchtigkeit an IoT Hub gesendet werden.
...
3. Stellen Sie mit einer SSH-Clientanwendung eine Verbindung mit dem Gerät her, z. B. PuTTY. Sie können ermitteln, welcher serielle Port von Ihrem Gerät verwendet wird, indem Sie dies im Windows-Geräte-Manager überprüfen.
    ...
    ![][11]
4. Klicken Sie in PuTTY auf den Verbindungstyp **Seriell**. Das Gerät stellt in der Regel eine Verbindung mit 9600 Baud her. Geben Sie also in das Feld **Geschwindigkeit** den Wert „9600“ ein. Klicken Sie dann auf **Öffnen**.
5. Das Programm wird gestartet. Unter Umständen müssen Sie das Board zurücksetzen (drücken Sie STRG+UNTBR oder die Reset-Taste des Boards), falls das Programm nach dem Herstellen der Verbindung nicht automatisch gestartet wird.
