...
In diesem Artikel erfahren Sie, wie Sie ein Peering virtueller Netzwerke erstellen, ändern oder löschen. Das Peering virtueller Netzwerke ist ein Mechanismus, mit dem zwei virtuelle Netzwerke am selben Azure-Standort über das Azure-Backbonenetzwerk miteinander verbunden werden können. Nach dem Peering werden die beiden virtuellen Netzwerke weiterhin als separate Ressourcen verwaltet. Ressourcen, die mit einem der beiden virtuellen Netzwerke verbunden werden, kommunizieren mit derselben Latenz und Bandbreite, als ob sie mit demselben virtuellen Netzwerk verbunden wären. Wenn Sie nicht mit dem Peering virtueller Netzwerke vertraut sind, sollten Sie vor der Durchführung der Aufgaben in diesem Artikel den Artikel [Peering virtueller Netzwerke – Überblick](virtual-network-peering-overview.md) lesen und das Tutorial unter [Erstellen eines Peerings virtueller Netzwerke](virtual-network-create-peering.md) absolvieren.
...
- Falls Sie noch nicht mit dem Peering virtueller Netzwerke in Azure vertraut sind, empfiehlt es sich, vor der Lektüre dieses Artikels zunächst das Tutorial unter [Erstellen eines Peerings virtueller Netzwerke](virtual-network-create-peering.md) zu absolvieren.
...
## <a name="about-peering"></a>Informationen zum Peering 

Sie sollten vor dem Peering virtueller Netzwerke folgende Einschränkungen berücksichtigen:
...
- Sie können zwei über Resource Manager bereitgestellte virtuelle Netzwerke oder ein über Resource Manager bereitgestelltes virtuelles Netzwerk per Peering mit einem virtuellen Netzwerk verknüpfen, das über das klassische Bereitstellungsmodell bereitgestellt wurde. Sie können nicht zwei virtuelle Netzwerke, die mit dem klassischen Bereitstellungsmodell erstellt wurden, mittels Peering miteinander verknüpfen. Wenn Sie mit den Azure-Bereitstellungsmodellen nicht vertraut sind, lesen Sie den Artikel [Grundlegendes zu Azure-Bereitstellungsmodellen](../azure-resource-manager/resource-manager-deployment-model.md?toc=%2fazure%2fvirtual-network%2ftoc.json).
...
- Ein Peering wird zwischen zwei virtuellen Netzwerken eingerichtet. Peerings sind nicht transitiv. Wenn Sie Peerings zwischen Folgendem erstellen:
    - Virtuelles Netzwerk 1 und virtuelles Netzwerk 2
    - Virtuelles Netzwerk 2 und virtuelles Netzwerk 3

  Es erfolgt kein Peering zwischen dem virtuellen Netzwerk 1 und dem virtuellen Netzwerk 3 über das virtuelle Netzwerk 2. Wenn Sie ein Peering zwischen dem virtuellen Netzwerk 1 und dem virtuellen Netzwerk 3 einrichten möchten, müssen Sie ein Peering zwischen dem virtuellen Netzwerk 1 und dem virtuellen Netzwerk 3 erstellen.
...
- Sie können virtuelle Netzwerke, die über andere Bereitstellungsmodelle bereitgestellt werden, mittels Peering miteinander verbinden, die dasselbe Abonnement aufweisen. Das Peering von virtuellen Netzwerken, die über verschiedene Bereitstellungsmodelle erstellt wurden und zu unterschiedlichen Abonnements gehören, ist mit der Vorschauversion möglich.
- Die Abonnements, denen beide virtuelle Netzwerke zugewiesen sind, müssen demselben Azure Active Directory-Mandanten zugeordnet sein. Wenn Sie noch keinen AD-Mandanten besitzen, [erstellen Sie schnell einen](../active-directory/develop/active-directory-howto-tenant.md?toc=%2fazure%2fvirtual-network%2ftoc.json#start-from-scratch). 
- <a name="roles-permissions"></a>Bei Ihrem Konto muss es sich um die erforderliche Rolle bzw. die erforderlichen Berechtigungen zum Erstellen eines Peerings handeln. Wenn Sie beispielsweise ein Peering zwischen den virtuellen Netzwerken *VNet1* und *VNet2* erstellen, muss Ihr Konto mindestens die folgende Rolle oder die folgenden Berechtigungen für jedes virtuelle Netzwerk haben:
...
## <a name="create-peering"></a>Erstellen eines Peerings

>[!NOTE]
>Es gibt verschiedene Anforderungen, Einschränkungen und Überlegungen zur erfolgreichen Erstellung eines Peerings zu berücksichtigen. Lesen Sie vor der Erstellung eines Peerings auf jeden Fall die Liste der Überlegungen im Abschnitt [Informationen zum Peering](#about-peering) dieses Artikels.
>

1. Melden Sie beim [Portal](https://portal.azure.com) mit einem Konto an, dem die erforderlichen [Rollen und Berechtigungen](#roles-permissions) zugewiesen sind.
...
## <a name="change-subnet"></a>Anzeigen oder Ändern von Peeringeinstellungen
...
## <a name="delete-subnet"></a>Löschen eines Peerings
...
- Erstellen einer Hub-and-Spoke-Netzwerktopologie|[Mehrere](/azure/architecture/reference-architectures/hybrid-networking/hub-spoke?toc=%2fazure%2fvirtual-network%2ftoc.json#vnet-peering)|
