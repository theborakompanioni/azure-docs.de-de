---
layout: LandingPage
+toc_rel: breadcrumb.json
translationtype: Human Translation
ms.sourcegitcommit: 87b9d22ec066108adb282606305315919b901ea0
ms.openlocfilehash: f03b3e68dd62cdf429162d3fe7d2342bbf254b52

---
#<a name="virtual-network-documentation"></a>Dokumentation zu Virtual Network

Bei einer Azure Virtual Network-Instanz (VNet) handelt es sich um eine logische Darstellung Ihres eigenen Netzwerks in der Cloud.  Bei einem VNet haben Sie die vollständige Kontrolle über Elemente des Netzwerks. Außerdem können Sie das VNet in Subnetze segmentieren und damit Azure Virtual Machines oder Cloud Services starten.  Das VNet kann zudem unter Verwendung einer der in Azure verfügbaren Verbindungsoptionen mit Ihrem lokalen Netzwerk verbunden werden, um Ihr Netzwerk in die Cloud zu erweitern, ohne die Kontrolle über Ressourcen abzugeben.

<ul class="panelContent cardsFTitle">
    <li>
        <a href="/azure/Virtual-Network/virtual-networks-overview">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/virtual-network.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Informationen zu Virtual Network</h3>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="https://azure.microsoft.com/documentation/videos/index/?services=virtual-network">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/video-library.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Videobibliothek zu Virtual Network</h3>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="/azure/Virtual-Network/virtual-networks-create-vnet-arm-pportal">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/portal.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Erste Schritte mit Virtual Network über das Azure-Portal</h3>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="/azure/Virtual-Network/virtual-networks-create-vnet-arm-ps">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/powershell.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Erste Schritte mit Virtual Network unter Verwendung von PowerShell</h3>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="/azure/Virtual-Network/virtual-networks-create-vnet-arm-cli">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="media/index/cli.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <h3>Erste Schritte mit Virtual Network unter Verwendung der Azure-Befehlszeilenschnittstelle</h3>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
</ul>

---

## <a name="reference"></a>Referenz

<ul class="panelContent cardsW">
    <li>
        <a href="/powershell/azureps-cmdlets-doc">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3>PowerShell</h3>
                        <p>Azure PowerShell-Referenz</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="/cli/azure/network/vnet">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3>Azure-Befehlszeilenschnittstelle</h3>
                        <p>Virtual Network – Azure-Befehlszeilenschnittstelle</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="/dotnet/api">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3>.NET</h3>
                        <p>.NET-API-Referenz</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>   
    <li>
        <a href="/java/api">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3>Java</h3>
                        <p>Java-API-Referenz</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="https://msdn.microsoft.com/library/azure/mt163658">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardText">
                        <h3>REST</h3>
                        <p>Virtual Network – REST-API-Referenz</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
</ul>


<div class="downloadHolder">
    <a href="https://opbuildstorageprod.blob.core.windows.net/output-pdf-files/en-us/Azure.azure-documents/live/virtual-network.pdf">
        <div class="img"></div>
        <div class="text">
Herunterladen der Dokumentation zu Virtual Network </div>
    </a>
</div>



<!--HONumber=Nov16_HO4-->


