...
## <a name="wire-up-deployment-domain"></a>Einrichten der Bereitstellungsdomäne
...
5. Konfigurieren Sie Draft so, dass Ihre Registrierung verwendet wird und für jedes erstellte Helm-Diagramm Unterdomänen erstellt werden. Für die Konfiguration von Draft benötigen Sie Folgendes:
  - Ihren Azure Container Registry-Namen (in diesem Beispiel `draftacs`)
  - Ihren Registrierungsschlüssel oder Ihr Kennwort aus `az acr credential show -n $acrname --output tsv --query "passwords[0].value"`
  - die Stammbereitstellungsdomäne, die Sie für die Zuordnung zur externen IP-Adresse des Kubernetes-Eingangs konfiguriert haben (in diesem Fall `13.64.108.240`)

  Mit diesen Werten können Sie den Base64-codierten Wert der JSON-Konfigurationszeichenfolge erstellen: `{"username":"<user>","password":"<secret>","email":"email@example.com"}`. Es folgt eine Möglichkeit zum Codieren des Werts (ersetzen Sie die Werte in diesem Beispiel jedoch durch Ihre Werte).
      ```bash
      acrname="draftacs"
  ...
  Sie können überprüfen, ob die JSON-Zeichenfolge richtig ist, indem Sie `echo $authtoken | base64 -D` eingeben, um das nicht codierte Ergebnis anzuzeigen.
  Initialisieren Sie Draft mit diesem Befehl und dem Konfigurationsargument für die Option `-set`:
      ```bash
      draft init --set registry.url=$acrname.azurecr.io,registry.org=$acrname,registry.authtoken=$authtoken,basedomain=squillace.io
      ```
      > [!NOTE]
      > Es kann leicht vergessen werden, dass der `basedomain`-Wert die Basisbereitstellungsdomäne ist, die Sie steuern und so konfiguriert haben, dass sie auf die externe IP des Eingangs verweist.
