...
Bei der Kopieraktivität werden verschiedene Dateiformate unterstützt, einschließlich Binär- (Bilder, Dokumente, Musik usw.), Text-, Avro-, ORC- und JSON-Formate für dateibasierte Speicher. Dabei kann jedoch nur der interne Inhalt strukturierter Dateien in Text-, Avro-, ORC- und JSON-Dateien gelesen werden. Sie können mit der Kopieraktivität Daten von einem Format in ein anderes konvertieren. Beispiel: Text (CSV) in Avro. Bei unstrukturierten Daten können Sie die **Structure**-Eigenschaft in der JSON-Definition des [Datasets](data-factory-create-datasets.md) weglassen.
