...
2. Mithilfe von Azure Machine Learning eine Bestätigung, Optimierung, Beschleunigung und Skalierung unserer Ergebnisse herbeizuführen 

Wie viele andere Unternehmen durchläuft auch unsere Organisation einen monatlichen Prozess zur Erlösprognose. Unser kleines Analystenteam wurde beauftragt, diesen Prozess mithilfe von Azure Machine Learning zu unterstützen und die Prognosegenauigkeit zu verbessern. Das Team sammelte im Verlauf einiger Monate Daten aus mehreren Quellen und ermittelte in dieser Zeit mit statistischen Hilfsmitteln die wichtigsten Attribute für Vertriebsprognosen im Bereich Services. Im nächsten Schritt wurden verschiedene Prototypen für die Regressionsmodelle in Excel erstellt und mit den vorhandenen Daten getestet. Innerhalb einiger Wochen hatten wir ein Regressionsmodell in Excel erstellt, das besser war als unsere derzeitigen Prognoseverfahren aus dem Finanzbereich. Dieses Modell wurde unser Prognose-Benchmark. 

Weiter ging es mit dem Versuch, unsere Vorhersageanalyse in Azure Machine Learning zu implementieren, um herauszufinden, wie Machine Learning die Vorhersageleistung weiter verbessern würde.

## <a name="achieving-predictive-performance-parity"></a>Erreichen eines Leistungsgleichstands
Unsere oberste Priorität bestand darin, ein zu unserem Excel-Regressionsmodell gleichwertiges Modell in Machine Learning zu schaffen. Wir wollten mit denselben Daten und derselben Aufteilung von Trainings- und Testdaten einen Gleichstand der Vorhersageleistung bei Excel und Machine Learning herstellen. Zunächst scheiterten wir dabei. Das Excel-Modell übertraf das Machine Learning-Modell. Grund für das Scheitern waren unsere fehlenden Kenntnisse über die Basiseinstellung in Machine Learning. Nachdem wir uns mit dem Machine Learning-Produktteam kurzgeschlossen hatten, erhielten wir ein besseres Verständnis der erforderlichen Basiseinstellung für unsere Datasets und erzielten schließlich einen Leistungsgleichstand der beiden Modelle. 
...
Wir berechneten den *Mean Absolute % Error* und verwendeten diesen Wert als Leistungsmaß für unser Modell. Die Entwicklung des funktionierenden Excel-Modells dauerte 3 Monate. Viele Aspekte des Lernmodells konnten wir für das Machine Learning-Experiment übernehmen, was letztlich vorteilhaft für unser Verständnis der Anforderungen war.

### <a name="create-comparable-experiment-in-azure-machine-learning"></a>Erstellen eines vergleichbaren Experiments in Azure Machine Learning
Das Erstellen unseres Experiments in Machine Learning Studio verlief in folgenden Schritten: 

1. Hochladen des Datasets als CSV-Datei in Machine Learning Studio (sehr kleine Datei)
...
Zunächst war das Excel-Modell dem Machine Learning Studio-Modell deutlich überlegen. 

|  | Excel | Studio |
| --- |:---:|:---:|
| Leistung | | |
| <ul style="list-style-type: none;"><li>Adjusted R Square</li></ul> |0,96 |N/V |
| <ul style="list-style-type: none;"><li>Koeffizient von <br />Ermittlung</li></ul> |N/V |0,78<br />(geringe Genauigkeit) |
...
Als wir die Empfehlungen umgesetzt hatten, konnten wir in Machine Learning Studio denselben Benchmark erzielen wie mit Excel. 
...
Wir wollten unser Modell auch mithilfe der in Machine Learning Studio verfügbaren Optionen und Algorithmen optimieren.
