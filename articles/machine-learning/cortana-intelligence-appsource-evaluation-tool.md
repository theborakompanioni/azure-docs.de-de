...
Sie können das Lösungsauswertungstool für Cortana Intelligence dafür verwenden, Ihre erweiterten Analyseanwendungen in Bezug auf die Konformität zu den von Microsoft empfohlenen bewährten Methoden zu bewerten. Wir freuen uns darauf, mit den Microsoft-Partner (ISVs/SIs) zusammenzuarbeiten, um qualitativ hochwertige Lösungen für Kunden, Händler und Implementierungen bieten zu können. In diesem Handbuch wird schrittweise erläutert, wie das Lösungsauswertungstool in Ihrer Anwendung verwendet wird. Darüber hinaus werden die spezifischen zu prüfenden bewährten Methoden beschrieben.

## <a name="getting-started"></a>Erste Schritte
Bitte laden Sie das Lösungsauswertungstool für Cortana Intelligence [herunter](https://aka.ms/aa-evalution-tool-download), und installieren Sie es.
...
Geben Sie die Identitätsinformationen zu Ihrer Anwendung ein.
...
Nachdem die Ressourcengruppe geladen ist, wählen Sie bitte die in Ihrer App enthaltenen Ressourcen, und geben Sie an, wie auf die Datenressourcen zugegriffen werden kann:
- Erfassung
- Nutzung
- Intern

Wir verwenden diese Informationen, um besser zu verstehen, wie Ihre App unterschiedliche Komponenten nutzt, und um sicherzustellen, dass die Benutzerkomponenten mit den bewährten Methoden übereinstimmen.

### <a name="ingestion"></a>Erfassung
In diesem Fall ist unter „Erfassung“ jegliche Datenquelle zu verstehen, die zum Pullen von Daten von außerhalb der App oder von beliebigen Services außerhalb der App zum Übermittelns von Daten per Push in die App verwendet wird.

### <a name="consumption"></a>Nutzung
In diesem Fall ist unter „Veröffentlichen“ jegliches Dataset zu verstehen, das zum direkten oder indirekten Übermitteln per Push von Daten an die Endbenutzer verwendet wird. Beispiel:
...
Das Lösungstool führt eine Sammlung automatisierter Tests für Ihre Anwendung aus.

![Set-Test-Ausführung](./media/cortana-intelligence-appsource-evaluation-tool/5-set-test-execution.png)

Nach Abschluss der Tests werden Sie dazu aufgefordert, eine Erklärung oder Begründung dafür zu geben, weshalb Ihre App die Anforderung nicht erfüllt.
...
Wenn Ihre App z.B. an Azure SQL DW veröffentlicht, verlangt der Auswertungstest von Ihnen, ebenfalls an Azure Analysis Services zu veröffentlichen. 

Ihre App könnte beispielsweise virtuelle Iaas-Computer verwenden und SQL Server Analysis Services anstatt von Azure Analysis Services verwenden. Dies wäre ein akzeptabler Grund für das Versagen des Tests.
...
Bei allen Ressourcen von Azure SQL und Azure SQL DW in der App sollte die Azure Active Directory-Authentifizierung (AAD) aktiviert werden. AAD bietet die Verwaltung aller Identitäten und Rollen an einer zentralen Stelle.
...
Um in AppSource ein konsistentes Sicherheitsniveau bieten zu können, können nur zertifizierte und im Azure Marketplace veröffentlichte virtuelle Computer als Teil der Cortana Intelligence-App bereitgestellt werden.
...
### <a name="cortana-intelligence-apps-should-include-a-scalable-big-data-platform"></a>Cortana Intelligence-Apps müssen eine skalierbare Big-Data-Plattform umfassen
Cortana Intelligence-Apps müssen für sehr große Datengrößen skalierbar sein. In Azure bedeutet dies, dass eine der beiden Datenplattformen des Petabytebereichs enthalten sein muss:
- Azure Data Lake Store
- Azure SQL Data Warehouse

Wenn Ihre App keinen Support für diese Datengrößen benötigt oder Sie eine alternative Datenplattform verwenden, erläutern Sie dies bitte in der Testfallbegründung.
### <a name="cortana-intelligence-apps-should-include-dedicated-ingestion-data-environments"></a>Cortana Intelligence-Apps müssen spezifische Datenumgebungen für die Erfassung beinhalten
Cortana Intelligence-Apps sollten es im Allgemeinen vermeiden, Daten direkt in relationale Datenquellen einzufügen. Stattdessen sollten die Daten in einer unstrukturierten Umgebung gespeichert werden. Hierfür sollten in Azure Data Factory idempotente Einfügungen/Aktualisierungen in beliebigen relationalen Speichern verwendet werden.
