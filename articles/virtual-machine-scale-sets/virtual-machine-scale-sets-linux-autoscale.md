...
[Installieren Sie die Azure-Befehlszeilenschnittstelle](../cli-install-nodejs.md), bevor Sie mit den Schritten in diesem Tutorial beginnen.
...
1. **Anmelden bei Microsoft Azure**  
Wechseln Sie an Ihrer Befehlszeilenschnittstelle (Bash, Terminal, Eingabeaufforderung) in den Resource Manager-Modus, und [melden Sie sich mit Ihrer Geschäfts-, Schul- oder Uni-ID](../xplat-cli-connect.md#scenario-1-azure-login-with-interactive-login) an. Befolgen Sie die Anweisungen für eine interaktive Anmeldung bei Ihrem Azure-Konto.
...
3. **Bereitstellen eines Speicherkontos in der neuen Ressourcengruppen**  
In diesem Speicherkonto wird die Vorlage gespeichert. Erstellen Sie ein Speicherkonto mit dem Namen **vmsstestsa**.
...
11. Fügen Sie die autoscaleSettings-Ressource hinzu, mit der definiert wird, wie die Skalierungsgruppe basierend auf der Prozessornutzung auf den Computern in der Gruppe angepasst wird.
    ...
    * **metricName**  
    Dieser Wert entspricht dem Leistungsindikator, den wir in der Variablen „wadperfcounter“ definiert haben. Anhand dieser Variablen führt die Diagnoseerweiterung die Erfassung für den Indikator **Processor\PercentProcessorTime** durch.
    ...
    * **threshold**  
    Dieser Wert löst die Skalierungsaktion aus. In dieser Vorlage werden die Computer der Skalierungsgruppe hinzugefügt, wenn die durchschnittliche CPU-Nutzung für die Computer der Gruppe über 50 % liegt.
...
* Beispiele für Überwachungsfeatures von Azure Monitor finden Sie unter [Azure Monitor –Schnellstartbeispiele für die plattformübergreifende Befehlszeilenschnittstelle](../monitoring-and-diagnostics/insights-cli-samples.md).
...
* Sehen Sie sich die Vorlage [Autoscale demo app on Ubuntu 16.04](https://github.com/Azure/azure-quickstart-templates/tree/master/201-vmss-bottle-autoscale) (Demonstrations-App zum automatischen Skalieren unter Ubuntu 16.04) an. Diese richtet eine Python/bottle-App ein, um die automatische Skalierungsfunktion von VM-Skalierungsgruppen auszuführen.
