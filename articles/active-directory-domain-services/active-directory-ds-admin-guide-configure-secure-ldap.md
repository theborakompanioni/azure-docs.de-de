<properties
	pageTitle="Konfigurieren von sicherem LDAP (LDAPS) in den Azure AD Domain Services | Microsoft Azure"
	description="Konfigurieren von sicherem LDAP (LDAPS) für eine durch die Azure AD Domain Services verwaltete Domäne"
...
# Konfigurieren von sicherem LDAP (LDAPS) für eine durch Azure AD Domain Services verwaltete Domäne
...
3. **Antragstellername**: Als Name des Antragstellers muss im Zertifikat für die verwaltete Domäne ein Platzhalter angegeben sein. Wenn Ihre Domäne z. B. „contoso100.com“ heißt, muss als Antragstellername im Zertifikat „*.contoso100.com“ angegeben sein. Legen Sie als DNS-Name (bzw. alternativen Antragstellernamen) diesen Platzhalternamen fest.
...
## Aufgabe 1: Erwerben eines Zertifikats für sicheres LDAP
...
### Option A (empfohlen): Erwerben eines Zertifikats für sicheres LDAP von einer Zertifizierungsstelle
...
> [AZURE.NOTE] Die Clientcomputer, die eine Verbindung mit der verwalteten Domäne über sicheres LDAP herstellen sollen, müssen dem Aussteller des Zertifikats für sicheres LDAP (LDAPS) vertrauen.
...
12. Wählen Sie auf der Seite **Privaten Schlüssel exportieren** die Option **Ja, privaten Schlüssel exportieren** aus, und klicken Sie auf **Weiter**.
    ...
    > [AZURE.WARNING] Sie MÜSSEN den privaten Schlüssel gemeinsam mit dem Zertifikat exportieren. Wenn Sie eine PFX-Datei bereitstellen, die nicht den privaten Schlüssel für das Zertifikat enthält, kommt es beim Aktivieren von sicherem LDAP für Ihre verwaltete Domäne zu Fehlern.
...
5. Scrollen Sie nach unten bis zum Abschnitt **Domänendienste**. Dort finden Sie eine Option mit der Bezeichnung **Sicheres LDAP (LDAPS)**, wie im folgenden Screenshot gezeigt:
    ...
    ![Konfigurationsabschnitt für die Domänendienste](./media/active-directory-domain-services-admin-guide/secure-ldap-start.png)
...
8. Auf der Registerkarte **Konfigurieren** wird der Abschnitt **Domänendienste** abgeblendet dargestellt. Er befindet sich für einige Minuten im Status **Ausstehend**. Während dieses Zeitraums wird das LDAPS-Zertifikat auf Richtigkeit überprüft und sicheres LDAP für die verwaltete Domäne konfiguriert.

    ![Sicheres LDAP – Status „Ausstehend“](./media/active-directory-domain-services-admin-guide/secure-ldap-pending-state.png)

    > [AZURE.NOTE] Es dauert ungefähr 10 bis 15 Minuten, bis sicheres LDAP für die verwaltete Domäne aktiviert ist. Wenn das bereitgestellte Zertifikat für sicheres LDAP nicht den erforderlichen Kriterien entspricht, wird LDAPS nicht für Ihr Verzeichnis aktiviert, und Sie sehen einen Fehler. Mögliche Fehlerursachen sind z.B. ein falscher Domänenname, ein abgelaufenes oder bald ablaufendes Zertifikat usw.
...
1. Auf der Seite **Konfigurieren** wird im Abschnitt **Domänendienste** eine Option **SICHEREN LDAP-ZUGRIFF ÜBER DAS INTERNET AKTIVIEREN** angezeigt. Diese Option ist standardmäßig auf **NEIN** festgelegt, weil der Internetzugriff auf die verwaltete Domäne über sicheres LDAP in der Standardeinstellung deaktiviert ist.
...
3. Auf der Registerkarte **Konfigurieren** wird der Abschnitt **Domänendienste** abgeblendet dargestellt. Er befindet sich für einige Minuten im Status **Ausstehend**. Während dieser Zeit wird der Internetzugriff auf Ihre verwaltete Domäne über sicheres LDAP aktiviert.
