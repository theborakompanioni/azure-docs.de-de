...
* **[WordPress-Export][export]** – Dient zum Exportieren des Inhalts Ihres Blogs, der anschließend in eine neue WordPress-Website in Azure App Service mithilfe des Plug-Ins [WordPress Importer][import] importiert werden kann.
  ...
  > [!NOTE]
  > Dieser Prozess ermöglicht lediglich das Migrieren Ihrer Inhalte und nicht von Plug-Ins, Designs oder anderer Anpassungen. Diese müssen manuell erneut installiert werden.
  > 
  > 
...
7. Starten Sie, nachdem Sie alle Schritte abgeschlossen haben, Ihre Website auf dem Blatt der Web-App im [Azure-Portal][mgmtportal] neu.
...
| **Aktivieren von dauerhaften Datenbankverbindungen** <p>Standardmäßig verwendet WordPress keine dauerhaften Datenbankverbindungen. Daher kann es vorkommen, dass Ihre Verbindung zur Datenbank nach mehreren Verbindungen gedrosselt wird.</p> |<ol><li><p>Bearbeiten der <strong>wp-include/wp-db.php</strong> Datei.</p></li><li><p>Suchen Sie die folgende Zeile.</p><code>$Dies-Bhd > = Mysql\_connect ($dies Dbhost $dies -> -> Dbuser $dies-Dbpassword, $ Link zu neues, $Client\_flags >);</code></li><li><p>Ersetzen Sie die vorherige Zeile durch Folgendes.</p><code>$Dies-Bhd > = Mysql\_pconnect ($dies Dbhost $dies Dbuser $dies -> -> -> Dbpassword $Client\_flags); <br/>Wenn (false! == $Error\_reporting) {/br/ > & Nbsp; & Nbsp; Error\_reporting ($Error\_reporting); <br/>} </code></li><li><p>Suchen Sie die folgende Zeile.</p><code>$Dies-Bhd > = @mysql\_connect ($dies Dbhost $dies -> -> Dbuser $dies -> Dbpassword, $ Link zu neues, $Client\_flags); </code></li><li><p>Ersetzen Sie die obige Zeile durch Folgendes.</p><code>$Dies-Bhd > = @mysql\_pconnect ($dies Dbhost $dies Dbuser $dies -> -> -> Dbpassword $Client\_flags); </code></li><li><p>Speichern Sie die Datei <strong>wp-include/wp-db.php</strong> Datei und erneute Bereitstellung die Website.</p></li></ol><div class="wa-note"><span class="wa-icon-bulb"></span><p>Diese Änderungen können überschrieben werden, wenn WordPress aktualisiert wird.</p><p>WordPress-Standardeinstellungen für automatische Updates, die durch Bearbeiten deaktiviert werden können die <strong>wp-config.php</strong> Datei- und Hinzufügen von <code>definieren ("WP\_AUTO\_UPDATE\_CORE", false);</code></p><p>Adressierung Updates eine weitere Möglichkeit wäre, eine WebJob zu verwenden, die überwacht die <strong>wp-db.php</strong> Datei und führt die oben genannten Änderungen jedes Mal die Datei wird aktualisiert. Weitere Informationen finden Sie unter [Einführung in WebJobs](http://www.hanselman.com/blog/IntroducingWindowsAzureWebJobs.aspx).</p></div> |
| **Verbessern der Leistung** |<ul><li><p>[Deaktivieren des ARR-Cookies](http://ppe.blogs.msdn.com/b/windowsazure/archive/2013/11/18/disabling-arr-s-instance-affinity-in-windows-azure-web-sites.aspx) – Dies kann die Leistung verbessern, wenn WordPress auf mehreren Web-App-Instanzen ausgeführt wird.</p></li><li><p>Aktivieren von Caching. [Redis Cache](http://msdn.microsoft.com/library/azure/dn690470.aspx) kann mit dem [Redis Object Cache-Plug-In von WordPress](https://wordpress.org/plugins/redis-object-cache/) oder mit einem anderen Cacheangebot aus dem [Azure Store](/gallery/store/) verwendet werden.</p></li><li><p>[Beschleunigung von WordPress mit Wincache](http://ruslany.net/2010/03/make-wordpress-faster-on-iis-with-wincache-1-1/) – Wincache ist standardmäßig für Web-Apps aktiviert</p></li><li><p>[Skalieren einer Web-App in Azure App Service](web-sites-scale.md) und [Verwenden von ClearDB-Hochverfügbarkeitsrouting](http://www.cleardb.com/developers/cdbr/introduction) oder [MySQL Cluster CGE](http://www.mysql.com/products/cluster/)</p></li></ul> |
| **Verwenden von Blobs für die Speicherung** |<ol><li><p>[Erstellen eines Azure-Speicherkontos](../storage/storage-create-storage-account.md)</p></li><li><p>Informationen zur [Verwendung des Content Distribution Network (CDN)][cdn] für die geografische Verteilung der in Blobs gespeicherten Daten.</p></li><li><p>Installieren und Konfigurieren des [Azure-Speicher-Plug-Ins für WordPress](https://wordpress.org/plugins/windows-azure-storage/).</p><p>Detaillierte Einrichtungs- und Konfigurationsinformationen für das Plug-In finden Sie im [Benutzerhandbuch](http://plugins.svn.wordpress.org/windows-azure-storage/trunk/UserGuide.docx).</p></li></ol> |
| **Aktivieren von E-Mail** |<ol><li><p>[Aktivieren von SendGrid über den Azure-Store](/gallery/store/sendgrid/sendgrid-azure/)</p></li><li><p>[Installieren des SendGrid-Plug-Ins für WordPress](http://wordpress.org/plugins/sendgrid-email-delivery-simplified/)</p></li></ol> |
