---
title: "Erstellen einer Web-App in Azure App Service mithilfe des Azure SDK für Java"
...
---
# <a name="create-a-web-app-in-azure-app-service-using-the-azure-sdk-for-java"></a>Erstellen einer Web-App in Azure App Service mithilfe des Azure SDK für Java
<!-- Azure Active Directory workflow is not yet available on the Azure Portal -->

## <a name="overview"></a>Übersicht
In dieser exemplarischen Vorgehensweise wird gezeigt, wie Sie eine Azure SDK für Java-Anwendung erstellen, mit der eine Web-App in [Azure App Service][Azure App Service] erstellt wird. Anschließend stellen Sie eine Anwendung in der Web-App bereit. Die Vorgehensweise besteht aus zwei Teilen:
...
### <a name="create-an-active-directory-ad-in-azure"></a>Erstellen eines Active Directory-Verzeichnisses in Azure
Wenn Sie noch kein Active Directory-Verzeichnis in Ihrem Azure-Abonnement eingerichtet haben, melden Sie sich mit Ihrem Microsoft-Konto beim [klassischen Azure-Portal][Azure classic portal] an. Wenn Sie mehrere Abonnements besitzen, klicken Sie auf **Abonnements** , und wählen Sie das Standardverzeichnis für das Abonnement, das Sie für dieses Projekt verwenden möchten. Klicken Sie dann auf **Anwenden** , um dieses Abonnement anzuzeigen.

1. Wählen Sie im Menü links **Active Directory** aus. Klicken Sie auf **Neu > Verzeichnis > Benutzerdefiniert erstellen**.
2. Wählen Sie in **Verzeichnis hinzufügen** die Option **Neues Verzeichnis erstellen** aus.
...
Weitere Informationen zu Active Directory (AD) finden Sie unter [Was ist ein Azure AD-Verzeichnis?][What is an Azure AD directory].

### <a name="create-a-management-certificate-for-azure"></a>Erstellen eines Verwaltungszertifikats für Azure
...
Der Code in dieser Vorgehensweise verwendet ein selbstsigniertes Zertifikat zur Authentifizierung bei Azure. Für diese Vorgehensweise müssen Sie ein Zertifikat erstellen und dieses vorab in das [klassische Azure-Portal][Azure classic portal] hochladen. Dieser Vorgang umfasst die folgenden Schritte:
...
Wenn Sie diese Schritte ausgeführt haben, liegt das CER-Zertifikat in Ihrem Azure-Abonnement vor, und das JKS-Zertifikat befindet sich auf Ihrem lokalen Laufwerk. Weitere Informationen zu Verwaltungszertifikaten finden Sie unter [Erstellen und Hochladen eines Verwaltungszertifikats für Azure][Create and Upload a Management Certificate for Azure].
...
Weitere Informationen finden Sie unter [Erstellen und Hochladen eines Verwaltungszertifikats für Azure][Create and Upload a Management Certificate for Azure].
...
#### <a name="convert-the-pfx-file-into-jks"></a>Konvertieren der PFX-Datei in eine JKS-Datei
...
## <a name="build-a-web-app-creation-application"></a>Erstellen einer Anwendung für die Web-App-Erstellung
### <a name="create-the-eclipse-workspace-and-maven-project"></a>Erstellen des Eclipse-Arbeitsbereichs und des Maven-Projekts
...
6. Klicken Sie in **Abhängigkeiten** auf **Hinzufügen**. Geben Sie in **Gruppen-ID eingeben...** den Wert `azure-management` ein. Wählen Sie die Pakete für die grundlegende Verwaltung und die Verwaltung von App Service-Web-Apps.
   ...
   > **Hinweis:** Wenn Sie die Abhängigkeiten nach dem Veröffentlichen einer neuen Version aktualisieren, müssen Sie jede der Abhängigkeiten in dieser Liste neu hinzufügen.
   > Nachdem Sie auf **Hinzufügen** geklickt und jede Abhängigkeit ausgewählt haben, werden diese mit der neuen Versionsnummer in der Liste **Abhängigkeiten** angezeigt.
   > 
   > 

Klicken Sie auf **OK**. Das Azure-Paket wird in der Liste **Abhängigkeiten** angezeigt.

### <a name="writing-java-code-to-create-a-web-app-by-calling-the-azure-sdk"></a>Schreiben von Java-Code zum Erstellen einer Web-App durch Aufrufen des Azure SDK
...
#### <a name="define-the-main-entry-point-class"></a>Definieren der Hauptklasse für den Einstiegspunkt
...
* `webSpaceName` sollte einer der Werte sein, die in der Klasse [WebSpaceNames][WebSpaceNames] definiert werden.
...
#### <a name="define-the-web-creation-method"></a>Definieren der Methode für die Web-App-Erstellung
Als Nächstes definieren Sie eine Methode zum Erstellen der Web-App. Diese Methode, `createWebApp`, gibt die Parameter für Web-App und Webspace an. Es wird außerdem der Verwaltungsclient für die App Service-Web-Apps erstellt und konfiguriert, der durch das [WebSiteManagementClient][WebSiteManagementClient]-Objekt definiert wird. Der Verwaltungsclient ist entscheidend für das Erstellen von Web-Apps. Er stellt RESTful-Webdienste bereit, die Anwendungen das Verwalten von Web-Apps ermöglichen (Ausführen von Vorgängen wie Erstellung, Aktualisierung und Löschung), indem die Dienstverwaltungs-API aufgerufen wird.
...
#### <a name="run-the-application-and-verify-web-app-creation"></a>Ausführen der Anwendung und Überprüfen der Web-App-Erstellung
...
## <a name="deploying-an-application-to-the-web-app"></a>Bereitstellen einer Anwendung für die Web-App
Nachdem Sie "AzureWebDemo" ausgeführt und die neue Web-App erstellt haben, melden Sie sich beim klassischen Portal an, klicken Sie auf **Web-Apps**, und wählen Sie **WebDemoWebApp** in the **Web-Apps** angezeigt. Klicken Sie im Web-App-Dashboard auf **Durchsuchen** (oder klicken Sie auf die URL, `webdemowebapp.azurewebsites.net`), um zur Web-App zu navigieren. Sie sehen eine leere Platzhalterseite, da in der Web-App noch keine Inhalte bereitgestellt wurden.
...
#### <a name="run-the-hello-world-application-in-localhost"></a>Ausführen der Hello World-Anwendung in localhost
...
2. Wählen Sie im Dialogfeld **Eigenschaften** die Option **Java-Buildpfad**aus. Aktivieren Sie auf der Registerkarte **Reihenfolge und Export** die Option **JRE-Systembibliothek**, und klicken Sie dann auf **Nach oben**, um den Eintrag an den Anfang der Liste zu verschieben.
   
    ![][4]
3. Ebenfalls im Dialogfeld **Eigenschaften**: Wählen Sie **Ziellaufzeiten** aus, und klicken Sie auf **Neu**.
...
#### <a name="export-the-application-as-a-war"></a>Exportieren der Anwendung als WAR-Datei
...
2. Klicken Sie im Dialogfeld **Export auswählen** auf **Web > WAR-Datei**, und klicken Sie dann auf **Weiter**.
...
### <a name="deploying-the-hello-world-application-using-ftp"></a>Bereitstellen der Hello World-Anwendung mithilfe von FTP
...
#### <a name="set-up-deployment-credentials"></a>Einrichten der Anmeldeinformationen für die Bereitstellung
...
So rufen Sie FTP-Verbindungsinformationen auf dem Web-App-Blatt im [Azure-Portal][Azure Portal] ab
...
#### <a name="publish-your-application-using-kudu"></a>Veröffentlichen der Anwendung mit Kudu
...
#### <a name="publish-your-application-using-filezilla-optional"></a>Veröffentlichen der Anwendung mit FileZilla (optional)
...
1. Klicken Sie in FileZilla auf **Datei > Site Manager**.
2. Klicken Sie im **Site Manager** auf **Neue Website**. Unter **Eintrag auswählen** wird ein neuer, leerer FTP-Server angezeigt, für den Sie einen Namen eingeben müssen. Verwenden Sie in dieser Vorgehensweise `AzureWebDemo-FTP`.
   ...
   * **Kennwort:** Geben Sie das Kennwort ein, das Sie beim Festlegen der Anmeldeinformationen für die Bereitstellung angegeben haben.
     ...
     Wählen Sie auf der Registerkarte **Übertragungs-Einstellungen** die Option **Passiv** aus.
...
#### <a name="run-the-hello-world-application-on-the-web-app"></a>Ausführen der Hello World-Anwendung in der Web-App
