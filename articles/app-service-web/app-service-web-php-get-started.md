<properties
	pageTitle="Erstellen, Konfigurieren und Bereitstellen einer PHP-Web-App in Azure"
...
# Erstellen, Konfigurieren und Bereitstellen einer PHP-Web-App in Azure
...
## Erstellen einer Azure-Web-App und Einrichten der Git-Bereitstellung

>[AZURE.NOTE] „Einen Augenblick! Wie gehe ich vor, wenn ich die Bereitstellung per FTP durchführen möchte?“ Für diese Vorgehensweise ist ein [FTP-Tutorial](web-sites-php-mysql-deploy-use-ftp.md) vorhanden.
...
4. Führen Sie den Befehl zum Erstellen der Azure-Web-App mit der Git-Bereitstellung aus. Geben Sie die Zahl der gewünschten Region ein, wenn Sie dazu aufgefordert werden.
...
4. Legen Sie die PHP-Version fest, die für die Laravel-App erforderlich ist.

        azure site set --php-version 5.6

    Sie haben das Festlegen der PHP-Version hiermit abgeschlossen.
    
4. Generieren Sie ein neues `APP_KEY`-Element für Ihre Azure-Web-App, und legen Sie es als App-Einstellung für die Azure-Web-App fest.

        php artisan key:generate --show
        azure site appsetting add APP_KEY="<output_of_php_artisan_key:generate_--show>"

4. Aktivieren Sie auch das Laravel-Debuggen, um dem Anzeigen der kryptischen Seite `Whoops, looks like something went wrong.` vorzubeugen.

        azure site appsetting add APP_DEBUG=true

    Sie haben das Festlegen von Umgebungsvariablen abgeschlossen.
    
    >[AZURE.NOTE] Wir gehen aber erst einmal langsam vor und beschreiben, welche Abläufe hier in Laravel und in Azure stattfinden. Laravel verwendet die Datei `.env` im Stammverzeichnis, um Umgebungsvariablen für die App bereitzustellen. Darin ist die Zeile `APP_DEBUG=true` enthalten (und die Zeile `APP_KEY=...`). Auf diese Variable wird in `config/app.php` mit dem Code `'debug' => env('APP_DEBUG', false),` zugegriffen. [env()](https://laravel.com/docs/5.2/helpers#method-env) ist eine Laravel-Hilfsmethode, die das PHP-Element [getenv()](http://php.net/manual/en/function.getenv.php) im Hintergrund einsetzt.
    >
...
4. Beginnen Sie im linken Menü, und klicken Sie auf **App Services** > **&lt;App-Name>** > **Tools**.
    ...
    >[AZURE.TIP] Wenn Sie anstelle von **Tools** auf **Einstellungen** klicken, können Sie auf das Blatt **Anwendungseinstellungen** zugreifen. Darin können Sie – wie eben durchgeführt – PHP-Versionen, App-Einstellungen und virtuelle Verzeichnisse festlegen.
    
4. Klicken Sie auf **Erweiterungen** > **Hinzufügen**, um eine Erweiterung hinzuzufügen.

4. Wählen Sie im [Blatt](../azure-portal-overview.md) **Erweiterung wählen** die Option **Composer**. (Ein *Blatt* ist eine Portalseite, die horizontal geöffnet wird.)

4. Klicken Sie im Blatt **Rechtliche Bedingungen akzeptieren** auf **OK**.

5. Klicken Sie im Blatt **Erweiterung hinzufügen** auf **OK**.

    Nachdem die Erweiterung in Azure hinzugefügt wurde, wird in der Ecke eine benutzerfreundliche Popupmeldung angezeigt, und im Blatt **Erweiterungen** ist der Eintrag **Composer** enthalten.
...
4. Klicken Sie im Blatt Ihrer Web-App auf **Einstellungen** > **Anwendungseinstellungen**.
...
4. Scrollen Sie zum unteren Rand des Blatts, und ändern Sie das virtuelle Stammverzeichnis so, dass es nicht auf **site\\wwwroot**, sondern auf **site\\wwwroot\\public** zeigt.
...
4. Klicken Sie oben auf dem Blatt auf **Speichern**.

    Sie haben das Festlegen des virtuellen Verzeichnisses abgeschlossen.

## Bereitstellen der Web-App mit Git (und Festlegen von Umgebungsvariablen)
...
4. Übernehmen Sie alle Änderungen, und stellen Sie Ihren Code für die Azure-Web-App so bereit, wie Sie dies auch für jedes Git-Repository durchführen würden:
...
5. Wir verwenden für die Ausführung im Browser den folgenden Befehl:
...
<a name="clierror"></a>
### Azure-Befehlszeilenschnittstelle zeigt „‚site‘ is not an azure command“ an
...
<a name="http403"></a>
### Web-App zeigt HTTP 403-Fehler an
...
<a name="whoops"></a>
### Web-App zeigt „Whoops, looks like something went wrong.“ an
...
<a name="encryptor"></a>
### Web-App zeigt „Keine unterstützte Verschlüsselung gefunden.“ an
