...
1. Nennen Sie das Projekt für die Zwecke dieses Tutorials **MyHelloWorld**. Ihr Bildschirm sieht dann in etwa wie folgt aus:
   ![][02]
1. Klicken Sie auf **Finish** (Fertig stellen).
1. Erweitern Sie in der Projektexplorer-Ansicht von Eclipse die Option **MyHelloWorld**. Klicken Sie mit der rechten Maustaste auf **WebContent**, und klicken Sie dann auf **New** (Neu) sowie auf **JSP File** (JSP-Datei).
1. Geben Sie der Datei im Dialogfeld **New JSP File** (Neue JSP-Datei) den Namen **index.jsp**. Nennen Sie den übergeordneten Ordner **MyHelloWorld/WebContent**.
1. Wählen Sie im Dialogfeld **Select JSP Template** (JSP-Vorlage auswählen) im Rahmen dieses Tutorials **New JSP File (html)** (Neue JSP-Datei [HTML]), und klicken Sie dann auf **Finish** (Fertig stellen).
...
1. Wählen Sie im Kontextmenü **Azure**, und klicken Sie dann auf **Publish as Azure Web App...** (Veröffentlichen als Azure-Web-App...).

   ![][03]
   
   Alternativ können Sie auch das Webanwendungsprojekt im Projekt-Explorer auswählen und auf der Symbolleiste auf die Dropdown-Schaltfläche **Publish** (Veröffentlichen) klicken. Anschließend wählen Sie **Publish as Azure Web App** (Veröffentlichen als Azure-Web-App) aus:
...
1. Wenn Sie zuvor keinen Azure-Web-App-Container erstellt haben, oder Sie Ihre Anwendung in einem neuen Container veröffentlichen möchten, führen Sie die folgenden Schritte aus. Wählen Sie andernfalls einen vorhandenen Web-App-Container, und fahren Sie mit Schritt 7 fort.
  ...
  1. Geben Sie eine **DNS Label** (DNS-Bezeichnung) für Ihren Web-App-Container ein; dies ist die Blatt-DNS-Bezeichnung der Host-URL für Ihre Webanwendung in Azure. Hinweis: Der Name muss verfügbar sein und Azure-Web-App-Namenskonventionen entsprechen.
...
1. Als Nächstes sollten Sie die Ansicht **Azure Activity Log** (Azure-Aktivitätsprotokoll) sehen, das den Bereitstellungsstatus der Web-App anzeigt.

    ![][12]

    Die Bereitstellung Ihrer Web-App in Azure sollte nur einige Sekunden dauern. Wenn Ihre Anwendung bereit ist, wird in der Spalte **Status** ein Link **Published** (Veröffentlicht) angezeigt. Wenn Sie auf den Link klicken, gelangen Sie zur Startseite Ihrer bereitgestellten Web-App.
...
2. Wählen Sie im Kontextmenü **Azure** und dann **Publish as Azure Web App...** (Veröffentlichen als Azure-Web-App...).
...
Wenn die Ansicht **Azure Explorer** noch nicht geöffnet ist, können Sie sie öffnen, indem Sie in Eclipse auf das Menü **Windows** (Fenster) und dann auf **Show View** (Ansicht anzeigen), **Other...** (Andere...), **Azure** und **Azure Explorer** klicken. Wenn Sie sich nicht bereits angemeldet haben, werden sie dazu aufgefordert.
...
5. Wenn das Kontextmenü angezeigt wird, klicken Sie auf **Stop** (Beenden). 
    ![][13]
