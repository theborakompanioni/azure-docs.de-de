...
Wenn Sie versuchen, mithilfe des VPN-Client mit Microsoft Azure Virtual Network eine Verbindung herzustellen, erhalten Sie folgende Fehlermeldung:
...
Wenn Sie versuchen, mithilfe des VPN-Client mit dem virtuellen Netzwerk eine Verbindung herzustellen, erhalten Sie folgende Fehlermeldung:

**VPN-Clientfehler: Das Format der empfangenen Nachricht war unerwartet oder fehlerhaft. (Fehler 0x80090326)**

### <a name="cause"></a>Ursache

Dieses Problem tritt auf, wenn der öffentliche Schlüssel des Stammzertifikats nicht in VPN Gateway von Microsoft Azure hochgeladen oder der Schlüssel beschädigt oder abgelaufen ist.
...
Wenn Sie versuchen, mithilfe des VPN-Client mit dem virtuellen Netzwerk von Azure eine Verbindung herzustellen, erhalten Sie folgende Fehlermeldung:
...
## <a name="vpn-client-error-azure-vpn-custom-script-failed-8007026f"></a>VPN-Clientfehler: Azure-VPN benutzerdefinierter Skriptfehler (8007026f)

### <a name="symptom"></a>Symptom

Wenn Sie versuchen, mithilfe des VPN-Client mit dem virtuellen Netzwerk von Azure eine Verbindung herzustellen, erhalten Sie folgende Fehlermeldung:

**Benutzerdefinierter Skriptfehler (um die Routingtabelle zu aktualisieren) (8007026f).**
...
Extrahieren Sie das VPN-Clientkonfigurationspaket. Sie werden eine CER-Datei vorfinden. Installieren Sie das Zertifikat in den **vertrauenswürdigen Stammzertifizierungsstellen** des **Computerkontos**:

1. Öffnen Sie „mmc.exe“
2. Fügen Sie das **Zertifikate**-Snap-in hinzu.
3. Wählen Sie **Computer**-Konto für den lokalen Computer aus.
4. Klicken Sie mit der rechten Maustaste auf den Knoten **vertrauenswürdige Stammzertifizierungsstellen** > **All-Task**(Alle Aufgaben) > **Importieren**, und suchen Sie nach der CER-Datei, die Sie aus dem VPN-Clientkonfigurationspaket extrahiert haben.
5. Starten Sie den Computer neu. 
6. Versuchen Sie eine erneute Installation des VPN-Clients.

## <a name="azure-portal-error-failed-to-save-vpn-gateway-data-is-invalid"></a>Azure-Portal-Fehler: Fehler beim Speichern von VPN Gateway – Daten sind ungültig
...
**Fehler beim Speichern des Gateways für virtuelle Netzwerke &lt;Gatewayname&gt;. Fehler: Daten für Zertifikat &lt;Zertifikat-ID&gt; ist ungültig.**

### <a name="cause"></a>Ursache 

Dieses Problem tritt auf, wenn der öffentliche Schlüssel des Stammzertifikats, den Sie hochgeladen haben, ungültige Zeichen enthält, wie Leerzeichen.

### <a name="solution"></a>Lösung

Stellen Sie sicher, dass die Daten im Zertifikat keine ungültigen Zeichen enthalten, wie Zeilenumbrüche (Wagenrückläufe). Der gesamte Wert sollte in einer langen Zeile sein. Der folgende Text ist ein Beispiel für das Zertifikat:
...
## <a name="azure-portal-error-failed-to-save-vpn-gateway-resource-name-is-invalid"></a>Azure-Portal-Fehler: Fehler beim Speichern der VPN-Gateway – Ressourcenname ist ungültig

### <a name="symptom"></a>Symptom

Wenn Sie versuchen, die Änderungen für das VPN-Gateway im Azure-Portal zu speichern, erhalten Sie die folgende Fehlermeldung: 

**Fehler beim Speichern des Gateways für virtuelle Netzwerke &lt;Gatewayname&gt;. Fehler: Ressourcenname &lt;Zertifikatsname, das Sie versuchen hochzuladen&gt; ist ungültig**.

### <a name="cause"></a>Ursache

Dieses Problem tritt auf, da der Name des Zertifikats ungültige Zeichen enthält, z.B. ein Leerzeichen. 
...
## <a name="azure-vpn-gateway-upgrade-all-p2s-clients-are-unable-to-connect"></a>Azure VPN Gateway-Upgrade: Alle P2S-Clients können keine Verbindung herstellen
...
## <a name="point-to-site-vpn-incorrectly-adds-a-route-for-100008-to-route-table"></a>Punkt-zu-Standort-VPN fügt fälschlicherweise eine Route für 10.0.0.0/8 zur Routingtabelle hinzu

### <a name="symptom"></a>Symptom

Wenn Sie die VPN-Verbindung auf dem Client Punkt-zu-Standort wählen, wird erwartet, dass der VPN-Client eine Route zum Azure Virtual Network hinzufügt und der Dienst Iphelper eine Route für das Subnetz des VPN-Clients hinzufügt. 

Wenn der VPN-Clientbereich jedoch zu einem kleineren Subnetz von 10.0.0.0/8 gehört, z.B. 10.0.12.0/24, wird anstelle einer Route für 10.0.12.0/24 eine „falsche“ Route für 10.0.0.0/8 hinzugefügt, die eine höhere Priorität hat. 

Dies unterbricht die Verbindung mit anderen lokalen Netzwerken, die möglicherweise zu einem anderen Subnetz innerhalb des Bereichs von 10.0.0.0/8 gehören, z.B. 10.50.0.0/24, die keine bestimmte Route definiert haben. 

### <a name="cause"></a>Ursache

Dieses Verhalten ist für Windows-Clients beabsichtigt . Wenn das PPP IPCP-Protokoll verwendet wird, erhält der Client die IP-Adresse für die Tunnelschnittstelle vom Server (VPN Gateway in diesem Fall). Allerdings verfügt der Client aufgrund einer Einschränkung im Protokoll nicht über die Subnetzmaske. Da es keine andere Möglichkeit zum Abrufen gibt, versucht der Client, die Subnetzmaske basierend auf der Klasse der IP-Adresse der Tunnelschnittstelle zu erraten. 
...
Der VPN-Client wurde mit dem Azure-Netzwerk verbunden. Jedoch kann der Client nicht auf die Netzwerkfreigaben zugreifen.

### <a name="cause"></a>Ursache

Das SMB-Protokoll wird für den Zugriff auf Dateifreigaben verwendet. Der Fehler tritt auf, wenn die Anmeldeinformationen für die Sitzung vom VPN-Client hinzugefügt werden, wenn die Verbindung initiiert wird. Nachdem die Verbindung hergestellt wurde, wird der Client gezwungen, die Cache-Anmeldeinformationen für die Kerberos-Authentifizierung zu verwenden. Dadurch werden Abfragen an das Schlüsselverteilungscenter (ein Domänencontroller) initiiert, um ein Token abzurufen. Weil die Clients aus dem Internet eine Verbindung herstellen, können sie möglicherweise den Domänencontroller nicht erreichen. Aus diesem Grund können die Clients keinen Failover von Kerberos nach NTLM ausführen. 

Der Client wird nur dann nach Anmeldeinformation gefragt, wenn der Client ein gültiges Zertifikat (mit SAN=UPN) hat, das von der Domäne ausgestellt wurde, mit der der Client verbunden ist, und wenn der Client physisch mit dem Domänennetzwerk verbunden ist. In diesem Fall versucht der Client, das Zertifikat zu verwenden, und wendet sich an den Domänencontroller. Anschließend gibt das Schlüsselverteilungscenter einen Fehler „KDC_ERR_C_PRINCIPAL_UNKNOWN“ zurück.  Dies zwingt den Client ein Failover nach NTLM auszuführen. 
...
## <a name="cannot-find-the-point-to-site-vpn-connection-in-windows-after-reinstalling-vpn-client"></a>Kann die Punkt-zu-Standort-VPN-Verbindung in Windows nach der Neuinstallation des VPN-Clients nicht finden
