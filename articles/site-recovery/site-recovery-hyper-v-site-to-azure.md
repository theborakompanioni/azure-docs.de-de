---
title: Replizieren von virtuellen Hyper-V-Computern (ohne VMM) nach Azure per Azure Site Recovery über das Azure-Portal | Microsoft Docs
...
---
# <a name="replicate-hyper-v-virtual-machines-(without-vmm)-to-azure-using-azure-site-recovery-with-the-azure-portal-|-microsoft-azure"></a>Replizieren von virtuellen Hyper-V-Computern (ohne VMM) nach Azure per Azure Site Recovery über das Azure-Portal | Microsoft Azure
...
| **Voraussetzung** | **Details** |
| --- | --- |
| **Hyper-V** |Ein oder mehrere lokale Server, auf denen Windows Server 2012 R2 ausgeführt wird, mit den aktuellsten Updates und der Hyper-V-Rolle.<br/><br/>Der Hyper-V-Server muss mindestens einen virtuellen Computer enthalten.<br/><br/>Hyper-V-Server müssen entweder direkt oder über einen Proxy mit dem Internet verbunden sein.<br/><br/>Auf Hyper-V-Servern sollten die in [KB2961977](https://support.microsoft.com/en-us/kb/2961977 "KB2961977") erwähnten Fixes installiert sein. |
...
1. Klicken Sie nach Abschluss der Installation auf **Registrieren**, um den Server im Tresor zu registrieren.   
    ![Installationsspeicherort](./media/site-recovery-hyper-v-site-to-azure/provider2.png)
2. Nach der Registrierung ruft Azure Site Recovery Metadaten vom Hyper-V-Server ab, und der Server wird auf dem Blatt **Einstellungen** > **Site Recovery-Infrastruktur** > **Hyper-V-Hosts** angezeigt.
...
3. Führen Sie diesen Befehl aus, um die Komponenten zu installieren:
            ...
            C:\ASR> setupdr.exe /i
4. Führen Sie diese Befehle zum Registrieren des Servers im Tresor aus: CD C:\Programme\Microsoft Azure Site Recovery Provider\
   
            C:\Programme\Microsoft Azure Site Recovery Provider\> DRConfigurator.exe /r /FriendlyName <friendly name of the server> /Credentials <path of the credentials file> Dabei gilt:
