...
2. Führen Sie das Setup aus, und wählen Sie unter **Before you begin** (Vorbereitung) die Option **Add additional process servers to scale out deployment** (Weitere Prozessserver zum horizontalen Hochskalieren der Bereitstellung hinzufügen) aus.
...
1.	Installieren Sie das Minimalbetriebssystem CentOS 6.6 auf der Verwaltungsserver-VM. Behalten Sie das ISO-Image im DVD-Laufwerk, und starten Sie das System. Überspringen Sie den Medientest, wählen Sie als Sprache US-Englisch und anschließend **Basic Storage Devices** (Basisspeichergeräte) aus, und vergewissern Sie sich, dass sich auf der Festplatte keine wichtigen Daten befinden. Kicken Sie auf **Ja**, wodurch alle Daten gelöscht werden. Geben Sie den Hostnamen des Verwaltungsservers ein, und wählen Sie die Netzwerkkarte des Servers aus. Wählen Sie im Dialogfeld **Editing System** (System bearbeiten) die Option **Connect automatically** (Automatisch verbinden) aus, und fügen Sie eine statische IP-Adresse, ein Netzwerk und DNS-Einstellungen hinzu. Geben Sie eine Zeitzone und ein Stammkennwort zum Zugriff auf den Verwaltungsserver ein.
2.	Wenn Sie nach dem gewünschten Installationstyp gefragt werden, wählen Sie für die Partition **Create Custom Layout** (Benutzerdefiniertes Layout erstellen) aus. Klicken Sie anschließend auf **Weiter**, wählen Sie **Kostenlos** aus, und klicken Sie auf „Erstellen“. Erstellen Sie die Partitionen **/**, **/var/crash** und **/home** mit **FS Type:** **ext4**. Erstellen Sie die Partition „swap“ als **FS Type: swap**.
3.	Wenn bereits vorhandene Geräte gefunden werden, wird eine Warnung angezeigt. Klicken Sie auf **Format**, um das Laufwerk mit den Partitionseinstellungen zu formatieren. Klicken Sie auf **Write change to disk** (Änderung auf die Festplatte schreiben), um die Partitionsänderungen zu übernehmen.
4.	Wählen Sie **Install boot loader** (Startladeprogramm installieren) > **Next**, um das Startladeprogramm auf der Stammpartition zu installieren.
...
1.	Wählen Sie im Tresor unter den replizierten Elementen den virtuellen Computer aus, für den ein Failover ausgeführt wurde, und klicken Sie mit der rechten Maustaste auf **Re-Protect** (Erneut schützen). Sie können auch auf den Computer klicken und „reprotect“ (Erneut schützen) aus den Befehlsschaltflächen auswählen.
...
1.	Wählen Sie auf der Seite mit replizierten Elementen den virtuellen Computer aus, der den Rechtsklick auf **Ungeplantes Failover** enthält.
2.	Überprüfen Sie unter **Failover bestätigen** die Failoverrichtung (von Azure), und wählen Sie den Wiederherstellungspunkt aus, der für das Failover verwendet werden soll (den aktuellsten oder den aktuellsten anwendungskonsistenten Wiederherstellungspunkt). Der anwendungskonsistente Punkt würde vor dem aktuellsten Zeitpunkt liegen und einigen Datenverlust verursachen.
