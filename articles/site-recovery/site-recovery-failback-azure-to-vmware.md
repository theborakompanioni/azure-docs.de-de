...
3. Die festgelegten Werte der Richtlinie sind (RPO-Schwellenwert = 15 Min., Aufbewahrungszeitraum des Wiederherstellungspunkts = 24 Stunden, App-konsistente Momentaufnahmehäufigkeit = 60 Min.) ![](./media/site-recovery-failback-azure-to-vmware-new/failback-policy.png)
...
6. Wenn sie das *Resource Manager*-Azure-Netzwerk auswählen, müssen Sie zum Bereitstellen des Servers die folgenden Eingaben vornehmen.
...
7. Am Ende sollte der Prozessserver auf der Seite „Konfigurationsserver“ im Abschnitt „Zugeordnete Server“ auf der Registerkarte „Prozessserver“ aufgeführt sein. ![](./media/site-recovery-failback-azure-to-vmware-new/pslistingincs.png)
...
2. Führen Sie das Setup aus, und wählen Sie in **Before you begin** (Vorbereitung) **Add additional process servers to scale out deployment** (Weitere Prozessserver zum horizontalen Hochskalieren der Bereitstellung hinzufügen) aus.
...
1.	Installieren Sie das Minimalbetriebssystem CentOS 6.6 auf der Verwaltungsserver-VM. Behalten Sie das ISO-Image im DVD-Laufwerk, und starten Sie das System. Überspringen Sie den Medientest, wählen Sie als Sprache US-Englisch aus, wählen Sie **Basic Storage Devices** aus, vergewissern Sie sich, dass sich auf der Festplatte keine wichtigen Daten befinden, und klicken Sie auf **Yes, discard any data**. Geben Sie den Hostnamen des Verwaltungsservers ein, und wählen Sie die Netzwerkkarte des Servers aus. Wählen Sie im Dialogfeld **Editing System** **Connect automatically** aus, und fügen Sie eine statische IP-Adresse, Netzwerk- und DNS-Einstellungen hinzu. Geben Sie eine Zeitzone und ein Stammkennwort zum Zugriff auf den Verwaltungsserver ein. 
...
4.	Wählen Sie **Install boot loader** > **Next** aus, um das Startladeprogramm in der Stammpartition zu installieren.
...
2. Wählen Sie **Erweitert** > **General Item** (Allgemeines Element) aus, und klicken Sie auf **Configuration Parameters** (Konfigurationsparameter). Diese Option ist deaktiviert, wenn der virtuelle Computer ausgeführt wird. Um sie zu aktivieren, muss der virtuelle Computer heruntergefahren werden.
...
1.	Wählen Sie im Tresor unter „replicated items“ (Replizierte Elemente) die VM aus, für die ein Failover ausgeführt wurde, und klicken Sie mit der rechten Maustaste auf **Re-Protect** (Erneut schützen). Sie können auch auf den Computer klicken und „reprotect“ (Erneut schützen) aus den Befehlsschaltflächen auswählen.
...
5.	Das Aufbewahrungslaufwerk wird verwendet, um die Zeitpunkte festzuhalten, wenn von der VM zurück auf den lokalen Standort repliziert wird. Einige der Kriterien eines Aufbewahrungslaufwerks sind nachstehend beschrieben. Ohne diese wird das Laufwerk nicht für den Masterzielserver aufgelistet. a. Das Volume sollte nicht für andere Zwecke (Ziel für die Replikation usw.) verwendet werden. b. Das Volume sollte sich nicht im Sperrmodus befinden. c. Bei dem Volume sollte es sich nicht um ein Cachevolume handeln. (Die Masterzielserver-Installation sollte auf diesem Volume nicht vorhanden sein. Das Volume für die benutzerdefinierte Prozessserver- und Masterzielserver-Installation kann für das Aufbewahrungsvolume nicht ausgewählt werden. Das hier installierte Prozessserver- und Masterzielserver-Volume ist ein Cachevolume des Masterzielservers) d. Das Dateisystem des Volumes sollte nicht FAT und FAT32 sein. e. Die Volumekapazität sollte ungleich 0 sein. e. Das Standardaufbewahrungsvolume für Windows ist das R-Volume. f. Das Standardaufbewahrungsvolume für Linux ist „/mnt/retention“.

6.  Die Failbackrichtlinie wird automatisch ausgewählt.

7.	Nachdem Sie auf **OK** geklickt haben, um den erneuten Schutz zu starten, beginnt ein Auftrag mit der Replikation des virtuellen Computers von Azure auf den lokalen Standort. Sie können den Fortschritt auf der Registerkarte **Aufträge** nachverfolgen.

Wenn die Wiederherstellung an einem anderen Speicherort erfolgen soll, wählen Sie das Aufbewahrungslaufwerk und den Datenspeicher aus, wie sie für den Masterzielserver konfiguriert wurden. Beim Ausführen des Failbacks zum lokalen Standort verwenden die virtuellen VMware-Computer im Failback-Schutzplan den gleichen Datenspeicher wie der Masterzielserver. Wenn Sie den virtuellen Azure-Replikatcomputer auf dem gleichen lokalen virtuellen Computer wiederherstellen möchten, muss sich der lokale virtuelle Computer bereits im selben Datenspeicher befinden wie der Masterzielserver. Wenn lokal kein virtueller Computer vorhanden ist, wird bei der Ausführung des erneuten Schutzes ein neuer erstellt. ![](./media/site-recovery-failback-azure-to-vmware-new/reprotectinputs.png)
...
>[AZURE.NOTE] Eine Replikationsgruppe sollte mithilfe desselben Masterzielservers erneut geschützt werden. Replikationsgruppen können nicht gleichzeitig erneut geschützt werden, wenn Sie dafür verschiedene Masterzielserver verwenden.
...
1.	Wählen Sie auf der Seite „replicated items“ (Replizierte Elemente) den virtuellen Computer aus, der den Rechtsklick auf **Ungeplantes Failover** enthält.
2.	Überprüfen Sie unter **Failover bestätigen** die Failoverrichtung (von Azure), und wählen Sie den Wiederherstellungspunkt aus, der für das Failover verwendet werden soll (den aktuellsten oder den aktuellsten anwendungskonsistenten). Der anwendungskonsistente Punkt würde vor dem aktuellsten Zeitpunkt liegen und einigen Datenverlust verursachen.
...
1.	Wählen Sie im Tresor unter „Setting“ (Einstellung) > „Replicated items“ (Replizierte Elemente) die virtuellen Computer aus, für die das Failback durchgeführt wurde, und klicken Sie auf **Re-Protect** (Erneut schützen). 
