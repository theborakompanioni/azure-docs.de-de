---
title: "Replizieren virtueller Hyper-V-Computer in VMM-Clouds zu einem sekundären VMM-Standort mithilfe des Azure-Portals | Microsoft Docs"
...
Dieser Artikel beschreibt, wie Sie Site Recovery im Azure-Portal zur Replikation lokaler virtueller Hyper-V-Computer, die in System Center Virtual Machine Manager-Clouds (VMM) verwaltet werden, zu sekundären Standorten verwenden können.
...
| **Bereich** | **Details** |
| --- | --- |
| **Bereitstellungsszenario** |Replizieren von Hyper-V-VMs zu Hyper-V-Hosts zwischen VMM-Clouds|
| **Lokale Anforderungen** | Lokale VMM-Server, auf denen mindestens System Center 2012 SP1 mit den neuesten Updates ausgeführt wird<br/><br/> Wir empfehlen einen VMM-Server an jedem Standort, Sie können aber auch zwischen Clouds auf dem gleichen VMM-Server replizieren. [Weitere Informationen](site-recovery-single-vmm.md)<br/><br/> Ein VMM-Server, der über eine oder mehrere Clouds verfügt, die Hyper-V-Hosts enthalten<br/><br/> Hyper-V-Server mit mindestens Windows Server 2012 und den aktuellen Updates<br/><br/> VMM-Server und ein Hyper-V-Host mit Internetzugriff<br/><br/> [Erforderliche URLs](#on-premises-prerequisites) (direkt oder über einen Proxy)  |
| **Lokale Einschränkungen** | HTTPS-basierte Proxys werden nicht unterstützt.
| **Anforderungen für Azure** | Azure-Konto<br/><br/> Recovery Services-Tresor |
| **VM-Replikation** | Alle von Hyper-V unterstützten VMs |
| **Agents** | Azure Site Recovery-Anbieter, der auf VMM-Servern installiert ist<br/><br/> Kein Agent auf den Hyper-V-Hosts erforderlich |
| **Bereitstellungsschritte** | **(1)** Vorbereiten eines lokalen VMM-Servers -> **2)** Erstellen eines Recovery Services-Tresors -> **3)** Einrichten des VMM-Servers und Registrieren im Tresor -> **4)** Konfigurieren der Replikationseinstellungen-> **5)** Einrichten der Netzwerkzuordnung -> **6)** Aktivieren der Replikation -> **7)** Testen der Replikation und des Failover.


## <a name="site-recovery-in-the-azure-portal"></a>Site Recovery im Azure-Portal

Azure verfügt über zwei verschiedene [Bereitstellungsmodelle](../azure-resource-manager/resource-manager-deployment-model.md) für das Erstellen und Verwenden von Ressourcen – Azure Resource Manager und die klassische Bereitstellung. Azure verfügt zudem über zwei Portale – das klassische Azure-Portal und das Azure-Portal.
...
- Externer Schutz für geschäftliche Workloads, die auf Hyper-V-VMs ausgeführt werden
...
- Einfaches Failover und Failback zwischen lokalen Standorten
- Replikation und Failover mehrerer VMs.  Beim Failover können Sie mehrere Computer in Wiederherstellungsplänen sammeln, damit Failover für Workloads von Anwendungen mit mehreren Ebenen gemeinsam erfolgen.

## <a name="scenario-architecture"></a>Szenarioarchitektur

* **Primärer Standort**: Am primären Standort benötigen Sie einen oder mehrere VMM-Server. Diese Server verfügen über private VMM-Clouds, die Hyper-V-Hostserver enthalten. Diese Server verfügen über VMs, die Sie replizieren möchten.
* **Sekundärer Standort**: Am sekundären Standort benötigen Sie einen oder mehrere VMM-Server. Hyper-V-Hostserver in VMM-Clouds führen Ziel-VMs aus, zu denen Sie die primären VMs replizieren.
* **Anbieter**: Während der Site Recovery-Bereitstellung installieren Sie den Azure Site Recovery-Anbieter auf VMM-Servern und registrieren diese Server bei einem Recovery Services-Tresor. Der Anbieter kommuniziert mit Site Recovery über HTTPS 443, um die Orchestrierung zu replizieren. Die Datenreplikation erfolgt zwischen primären und sekundären Hyper-V-Hostservern. Replizierte Daten bleiben an lokalen Standorten und Netzwerken und werden nicht an Azure gesendet. Hier finden Sie weitere Informationen zum [Datenschutz](#privacy-information-for-site-recovery).
...
| --- | --- |
| **Azure** |Sie benötigen ein [Microsoft Azure](http://azure.microsoft.com/) -Konto. Für den Einstieg steht eine [kostenlose Testversion](https://azure.microsoft.com/pricing/free-trial/)zur Verfügung. [Preise für Site Recovery](https://azure.microsoft.com/pricing/details/site-recovery/) . |
...
* [Preise für Site Recovery](site-recovery-network-mapping.md) .

## <a name="prepare-for-deployment-with-a-single-vmm-server"></a>Vorbereiten der Bereitstellung mit einem einzelnen VMM-Server

Wenn Sie nur über einen einzelnen VMM-Server verfügen, können Sie VMs in Hyper-V-Hosts in der VMM-Cloud zu [Azure](site-recovery-vmm-to-azure.md) oder in eine sekundäre VMM-Cloud replizieren. Sie sollten die erste Option wählen, weil die Replikation zwischen Clouds nicht nahtlos erfolgt, aber wenn Sie dies trotzdem tun müssen, gehen Sie folgendermaßen vor:
...
7. Sie führen Failover und Wiederherstellung wie folgt aus:
   ...
   * Melden Sie sich nach der Wiederherstellung der primären VMM-VM im Azure-Portal beim Recovery Services-Tresor an, und führen Sie ein nicht geplantes Failover der virtuellen Computer vom sekundären zum primären Standort aus.
...
## <a name="step-1-choose-your-protection-goal"></a>Schritt 1: Auswählen der Schutzziele
...
3. Wählen Sie unter **Schutzziel** die Option **Auf Wiederherstellungsstandort** und dann **Yes, with Hyper-V** (Ja, mit Hyper-V) aus.
...
3. Vergewissern Sie sich unter **Server hinzufügen**, dass unter **Servertyp** der Eintrag **System Center-VMM-Server** angezeigt wird und dass der VMM-Server die [Voraussetzungen und URL-Anforderungen](#on-premises-prerequisites) erfüllt.
...
> [!NOTE]
> Sie müssen nicht explizit etwas auf Hyper-V-Hostservern installieren.
>
>
...
2. Das Setup führt einige Voraussetzungsüberprüfungen durch und fordert die Berechtigung zum Beenden des VMM-Diensts an. Der VMM-Dienst wird nach Abschluss des Setups automatisch neu gestartet. Bei der Installation auf einem VMM-Cluster werden Sie aufgefordert, die Clusterrolle zu beenden.
...
4. Ändern Sie unter **Installation** den Speicherort für die Anbieterinstallation, oder akzeptieren Sie den Standardspeicherort, und klicken Sie auf **Installieren**.
...
7. Geben Sie unter **Internetverbindung** an, wie sich der Anbieter auf dem VMM-Server mit dem Internet verbinden soll. Wählen Sie **Mit vorhandenen Proxyeinstellungen verbinden** aus, um die Standard-Internetverbindungseinstellungen auf dem Server zu verwenden.
   ...
   * Wenn Sie einen benutzerdefinierten Proxy verwenden oder Ihr Standardproxy eine Authentifizierung verlangt, müssen Sie die Proxydetails einschließlich der Proxyadresse und des Ports eingeben.
   - Stellen Sie sicher, dass die erforderlichen URLs vom VMM-Server und den Hyper-V-Hosts zugänglich sind.
...
4. Führen Sie diesen Befehl aus, um den Anbieter zu installieren:
        ...
        C:\ASR> setupdr.exe /i
5. Führen Sie anschließend die folgenden Befehle aus, um den Server im Tresor zu registrieren:
...
3. Wählen Sie unter **Hyper-V-Hostversion** aus, welches Betriebssystem auf dem Host ausgeführt wird.
   ...
   > [!NOTE]
   > Die VMM-Cloud kann Hyper-V-Hosts enthalten, auf denen verschiedene (unterstützte) Versionen von Windows Server ausgeführt werden, jedoch wird eine Replikationsrichtlinie auf Hosts angewendet, die das gleiche Betriebssystem ausführen. Wenn Sie über Hosts verfügen, auf denen mehrere Betriebssystemversionen ausgeführt werden, erstellen Sie separate Replikationsrichtlinien.
   >
   >
...
11. Wenn Sie eine neue Richtlinie erstellen, wird sie der VMM-Cloud automatisch zugeordnet. Klicken Sie unter **Replikationsrichtlinie** auf **OK**. Sie können dieser Replikationsrichtlinie unter **Einstellungen** > **Replikation** > „<Richtlinienname>“ > **VMM-Cloud zuordnen** zusätzliche VMM-Clouds (und die darin enthaltenen VMs) zuordnen.
...
* [kurze Übersicht](#prepare-for-network-mapping) über die Abläufe bei der Netzwerkzuordnung durch. Ausführlichere Informationen finden Sie [hier](site-recovery-network-mapping.md) .
...
Standardmäßig werden beim Replizieren einer Hyper-V-VM zu einem Hyper-V-Server die replizierten Daten am Standardspeicherort gespeichert, der für den Hyper-V-Zielhost in Hyper-V-Manager angegeben wurde. Um zu steuern, wo die Replikationsdaten gespeichert werden, können Sie eine Speicherzuordnung konfigurieren.<br/><br/> Zu diesem Zweck müssen Sie die Speicherklassifizierungen auf den Quell- und Ziel-VMM-Servern einrichten, bevor Sie mit der Bereitstellung beginnen.

Die Speicherzuordnung wird derzeit nicht im neuen Azure-Portal unterstützt. Sie kann aber über Powershell aktiviert werden. [detaillierte Kapazitätsplanung](site-recovery-vmm-to-vmm-powershell-resource-manager.md#step-7-configure-storage-mapping)
...
Site Recovery verfügt über eine Excel-basierte Kapazitätsplanung, die Sie dabei unterstützt, die richtigen Ressourcen für Ihre Quellumgebung, die Site Recovery-Komponenten, das Netzwerk und den Speicher zuzuordnen. Sie können den Planer im Schnellmodus für Schätzungen ausführen, die auf einer durchschnittlichen Anzahl von VMs, Datenträgern und Speicher basieren, oder im ausführlichen Modus, in dem Sie Zahlen auf Workloadebene eingeben. Führen Sie Folgendes aus, bevor Sie beginnen:
...
Sie können die Einstellungen für die Bandbreitengewichtung verwenden oder den Datenverkehr nach Bits pro Sekunde beschränken. Wenn Sie ein Cluster verwenden, müssen Sie dies auf allen Clusterknoten durchführen. Weitere Informationen finden Sie unter:

* Blog von Thomas Maurer: [Throttling Hyper-V Replica Traffic](http://www.thomasmaurer.ch/2013/12/throttling-hyper-v-replica-traffic/)
...
Nachdem Sie die Replikation aktiviert haben, können Sie unter **Einstellungen** > **Replizierte Elemente** die Eigenschaften für die VM anzeigen. Das Dashboard **Zusammenfassung** enthält Informationen zur Replikationsrichtlinie der VM und ihrem Status. Klicken Sie auf **Eigenschaften** , um weitere Details anzuzeigen.
...
Zum Testen Ihrer Bereitstellung können Sie ein Testfailover für einen einzelnen virtuellen Computer durchführen oder einen Wiederherstellungsplan erstellen, der mindestens einen virtuellen Computer enthält.
...
* Die Anweisungen in diesem Artikel beschreiben die Ausführung eines Testfailovers ohne Netzwerk. Diese Option testet, ob ein Failover für die VM durchgeführt wird, jedoch nicht die Netzwerkeinstellungen für die VM. [Preise für Site Recovery](site-recovery-failover.md#run-a-test-failover) .
...
3. Wählen Sie unter **Testfailover** die Option **Kein** aus. Mit dieser Option testen Sie, ob ein Failover der VM wie erwartet durchgeführt wird, aber die replizierte VM wird nicht mit einem Netzwerk verbunden.

    ![Testfailover](./media/site-recovery-vmm-to-vmm/test-failover1.png)
4. Klicken Sie auf **OK**, um den Failovervorgang zu starten. Sie können den Fortschritt verfolgen, indem Sie auf den virtuellen Computer klicken, um die Eigenschaften zu öffnen. Alternativ können Sie unter **Einstellungen** > **Aufträge** > **Site Recovery-Aufträge** auf den Auftrag **Testfailover** klicken.
5. Gehen Sie wie folgt vor, wenn der Failoverauftrag die Phase **Test abschließen** erreicht hat:
   ...
   * Klicken Sie auf **Notizen**, um alle Beobachtungen im Zusammenhang mit dem Test-Failover zu erfassen und zu speichern.
6. Der virtuelle Computer für den Test wird auf dem Host erstellt, auf dem auch der replizierte virtuelle Computer vorhanden ist. Er wird der gleichen Cloud hinzugefügt, in der sich der replizierte virtuelle Computer befindet.
7. Nachdem Sie sich vergewissert haben, dass die VM erfolgreich gestartet wurde, klicken Sie auf **Das Testfailover ist abgeschlossen**. An diesem Punkt werden alle Elemente gelöscht, die von Site Recovery während des Testfailovers automatisch erstellt wurden.  
...
### <a name="update-dns-with-the-replica-vm-ip-address"></a>DNS-Aktualisierung mit der Replikat-VM-IP-Adresse
