...
10. **Datenverkehr zum Dienst wiederhergestellt** – Traffic Manager sendet einen GET-Befehl und empfängt eine 200 OK-Meldung in weniger als zehn Sekunden. Anschließend wird wieder der DNS-Name des Cloud-Diensts an DNS-Server ausgegeben, sobald diese Aktualisierungen anfordern. Der Datenverkehr kehrt wieder zum Endpunkt zurück, wenn zwischengespeicherte DNS-Antworten anderer Endpunkte ablaufen und vorhandene Verbindungen mit anderen Endpunkten beendet werden.

>[AZURE.NOTE] Da der Traffic Manager auf DNS-Ebene verwendet wird, kann er vorhandene Verbindungen mit Endpunkten nicht beeinflussen. Während der Traffic Manager unter Umständen neue Verbindungen an den primären Endpunkt leitet, erhalten sekundäre Endpunkte während des Failbacks weiterhin Datenverkehr über vorhandene Verbindungen, bis diese Sitzungen beendet werden. Wenn ein schnelles Failback erforderlich ist, sollten Anwendungen die Sitzungsdauer auf sekundären Endpunkten begrenzen.
