...
---
# <a name="storage-and-predictable-performance-provisioning-in-azure-cosmos-db"></a>Speicherung und vorhersagbare Leistungsbereitstellung in Azure Cosmos DB
Azure Cosmos DB ist ein vollständig verwalteter, skalierbarer dokumentorientierter NoSQL-Datenbankdienst für JSON-Dokumente. Mit Azure Cosmos DB müssen Sie keine virtuellen Computer mieten, Software bereitstellen oder Datenbanken überwachen. Azure Cosmos DB wird von Microsoft-Entwicklern betrieben und ständig überwacht, um erstklassige Verfügbarkeit, Leistung und Datensicherheit zu gewährleisten.  

Machen Sie sich mit Azure Cosmos DB durch [Erstellen eines Datenbankkontos](documentdb-create-account.md) und anschließender Erstellung einer [Azure Cosmos DB-Sammlung und -Datenbank](documentdb-create-collection.md) über das [Azure-Portal](https://portal.azure.com/) vertraut. Azure Cosmos DB-Datenbanken werden in Einheiten aus SSD-gestütztem Speicher und Durchsatz angeboten. Diese Speichereinheiten werden durch Erstellen von Datenbanksammlungen bereitgestellt. Dabei gilt für jede Sammlung ein reservierter Durchsatz, der jederzeit zentral hoch- oder herunterskaliert werden kann, damit die Anforderungen Ihrer Anwendung erfüllt sind.
...
Als Azure-Abonnent können Sie ein oder mehrere Azure Cosmos DB-Datenbankkonten zur Verwaltung Ihrer Datenbankressourcen bereitstellen. Jedes Abonnement ist einem einzelnen Azure-Abonnement zugeordnet.

Azure Cosmos DB-Konten können über das [Azure-Portal](documentdb-create-account.md) oder mithilfe [einer ARM-Vorlage oder der Azure-CLI](documentdb-automation-resource-manager-cli.md) erstellt werden.

## <a name="databases"></a>Datenbanken
Eine einzelne Azure Cosmos DB-Datenbank kann eine nahezu unbegrenzte Menge an Dokumentspeicher enthalten, der anhand von Sammlungen gruppiert wird. Sammlungen ermöglichen eine Leistungsisolierung. Jede Sammlung kann mit einem Durchsatz bereitgestellt werden, der für andere Sammlungen in derselben Datenbank oder unter demselben Konto nicht freigegeben ist. Die Größe einer Azure Cosmos DB-Datenbank ist flexibel und reicht vom GB-Bereich bis hin zu SSD-gestütztem Dokumentspeicher im TB-Bereich mit bereitgestelltem Durchsatz. Im Gegensatz zu einer herkömmlichen RDBMS-Datenbank gilt der Bereich einer Datenbank in Azure Cosmos DB nicht nur für einen einzelnen Computer und kann mehrere Computer oder Cluster umfassen.  

Mit Azure Cosmos DB können Sie weitere Sammlungen oder Datenbanken bzw. beides erstellen, wenn Sie Ihre Anwendungen skalieren müssen. Datenbanken können über das [Azure-Portal](documentdb-create-database.md) oder eines der [-SDKs](documentdb-dotnet-samples.md) erstellt werden.   

## <a name="database-collections"></a>Datenbanksammlungen
Jede Azure Cosmos DB-Datenbank kann eine oder mehrere Sammlungen enthalten. Sammlungen dienen als hoch verfügbare Datenpartitionen für die Speicherung und Verarbeitung von Dokumenten. Jede Sammlung kann Dokumente mit heterogenem Schema speichern. Die automatischen Indizierungs- und Abfragefunktionen von Azure Cosmos DB ermöglichen Ihnen das einfache Filtern und Abrufen von Dokumenten. Eine Auflistung liefert den Umfang für Dokumentspeicher und Abfrageausführung. Eine Auflistung ist auch eine Transaktionsdomäne für ale Dokumente, die darin enthalten sind. Sammlungen werden dem Durchsatz ausgehend von dem im Azure-Portal oder über die SDKs festgelegten Wert zugeordnet.

Sammlungen werden von Azure Cosmos DB automatisch in einen oder mehrere physische Server partitioniert. Wenn Sie eine Sammlung erstellen, können Sie den bereitgestellten Durchsatz in Form von Anforderungseinheiten pro Sekunde und einer Partitionsschlüsseleigenschaft angeben. Der Wert dieser Eigenschaft wird von Azure Cosmos DB zum Verteilen von Dokumenten zwischen Partitionen und zum Weiterleiten von Anforderungen wie Abfragen verwendet. Der Partitionsschlüssel fungiert außerdem als die Transaktionsgrenze für gespeicherte Prozeduren und Trigger. Jede Sammlung hat eine reservierte Durchsatzmenge, die für diese Sammlung spezifisch ist und die nicht mit anderen Sammlungen im gleichen Konto gemeinsam genutzt wird. Daher können Sie Ihre Anwendung sowohl im Hinblick auf Speicher als auch Durchsatz horizontal hochskalieren.
...
Es gibt verschiedene Faktoren, die sich auf die Anforderungseinheiten auswirken, die für einen Vorgang in einem Azure Cosmos DB-Datenbankkonto genutzt werden. Zu diesen Faktoren zählen folgende:
...
Weitere Informationen finden Sie unter [Anforderungseinheiten in Azure Cosmos DB](documentdb-request-units.md).
...
Anleitungen zum Ändern Ihrer Konsistenzebene im Azure-Portal finden Sie unter [Verwalten eines Azure Cosmos DB-Kontos](documentdb-manage-account.md#consistency). Weitere Informationen zu Konsistenzebenen finden Sie unter [Verwenden von Konsistenzebenen](documentdb-consistency-levels.md).

## <a name="provisioned-document-storage-and-index-overhead"></a>Bereitgestellter Dokumentspeicher und Index-Overhead
Azure Cosmos DB unterstützt die Erstellung von einzelnen Partitionen und partitionierte Sammlungen. Für jede Partition in Azure Cosmos DB werden bis zu 10 GB an SSD-gestütztem Speicher unterstützt. Die 10 GB Dokumentspeicher umfassen die Dokumente plus den Speicher für den Index. Standardmäßig wird eine Azure Cosmos DB-Sammlung so konfiguriert, dass alle Dokumente automatisch indiziert werden, ohne dass explizit irgendwelche sekundären Indizes oder Schemas erforderlich wären. Basierend auf den Anwendungen, die Azure Cosmos DB nutzen, liegt der Mehraufwand für die Indizierung normalerweise zwischen 2 und 20 %. Durch die von Azure Cosmos DB verwendete Indizierungstechnologie wird sichergestellt, dass der Mehraufwand für die Indizierung, unabhängig von den Werten der Eigenschaften, bei den Standardeinstellungen 80 % der Größe der Dokumente nicht überschreitet.

Standardmäßig werden alle Dokumente von Azure Cosmos DB automatisch indiziert. Wenn Sie den Indizierungsaufwand jedoch optimieren möchten, können Sie beim Einfügen oder Ersetzen eines Dokuments bestimmte Dokumente von der Indizierung ausschließen (siehe [Indizierungsrichtlinien für Azure Cosmos DB](documentdb-indexing-policies.md)). Sie können eine Azure Cosmos DB-Sammlung so konfigurieren, dass alle Dokumente innerhalb der Sammlung von der Indizierung ausgeschlossen werden. Sie können eine Azure Cosmos DB-Sammlung auch so konfigurieren, dass nur bestimmte Eigenschaften oder Pfade mit Platzhaltern in Ihren JSON-Dokumenten selektiv indiziert werden (siehe [Konfigurieren der Indizierungsrichtlinie einer Sammlung](documentdb-indexing-policies.md#CustomizingIndexingPolicy)). Durch das Ausschließen von Eigenschaften oder Dokumenten wird auch der Schreibdurchsatz verbessert, was bedeutet, dass Sie weniger Anforderungseinheiten verbrauchen.   

## <a name="next-steps"></a>Nächste Schritte
Weitere Informationen zu Azure Cosmos DB finden Sie unter [Partitionieren und Skalieren von Daten in Azure Cosmos DB](documentdb-partition-data.md).

Anweisungen zur Überwachung der Leistungsebenen im Azure-Portal finden Sie unter [Überwachen eines Azure Cosmos DB-Kontos](documentdb-monitor-accounts.md). Weitere Informationen zum Auswählen von Leistungsebenen für Sammlungen finden Sie unter [Leistungsebenen in Azure Cosmos DB](documentdb-performance-levels.md).
