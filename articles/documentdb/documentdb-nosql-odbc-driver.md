---
title: Herstellen einer Azure Cosmos DB-Verbindung mit BI-Analysetools | Microsoft-Dokumentation
...
3. Öffnen Sie die App **ODBC-Datenquellenadministrator** auf Ihrem Computer, indem Sie beispielsweise im Windows-Suchfeld **ODBC-Datenquellen** eingeben. 
    ...
    ![ODBC-Datenquellenadministrator für Azure Cosmos DB](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver.png)

## <a id="connect"></a>Schritt 2: Herstellen einer Verbindung mit Ihrer Azure Cosmos DB-Datenbank

1. Klicken Sie nach dem [Installieren des Azure Cosmos DB ODBC-Treibers](#install) im Fenster **ODBC-Datenquellenadministrator** auf **Hinzufügen**. Sie können einen Benutzer- oder System-DSN erstellen. In diesem Beispiel erstellen wir einen Benutzer-DSN.
2. Wählen Sie im Fenster **Neue Datenquelle erstellen** die Option **Microsoft Azure Cosmos DB ODBC-Treiber**, und klicken Sie dann auf **Fertig stellen**.
3. Geben Sie im Fenster für **Azure Cosmos DB ODBC-Treiber – DSN-Setup** Folgendes ein: 

    ![Fenster „Azure Cosmos DB ODBC Driver DSN Setup“ (Azure Cosmos DB ODBC-Treiber – DSN-Setup)](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-dsn-setup.png)
    ...
    ![Blatt „Azure Cosmos DB Keys“ (Azure Cosmos DB-Schlüssel)](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-keys.png)
...
6. Nachdem Sie die Schritte im Fenster **Azure Cosmos DB ODBC Driver DSN Setup** (Azure Cosmos DB ODBC-Treiber – DSN-Setup) ausgeführt und das Fenster geschlossen haben, wird der neue Benutzer-DSN auf der Registerkarte „Benutzer-DSN“ hinzugefügt.

    ![Neuer Azure Cosmos DB ODBC-DSN auf der Registerkarte „Benutzer-DSN“](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-user-dsn.png)
...
1. Klicken Sie nach Abschluss der Schritte 1 bis 4 unter [Herstellen der Verbindung mit Ihrer Azure Cosmos DB-Datenbank](#connect) im Fenster **Azure Cosmos DB ODBC Driver DSN Setup** (Azure Cosmos DB ODBC-Treiber – DSN-Setup) auf **Schema-Editor**.
    ...
    ![Schaltfläche „Schema-Editor“ im Fenster „Azure Cosmos DB ODBC Driver DSN Setup“ (Azure Cosmos DB ODBC-Treiber – DSN-Setup)](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-schema-editor.png)
...
Im Rahmen des Samplingprozesses können Sie Ansichten definieren und erstellen. Diese Ansichten entsprechen SQL-Ansichten. Sie sind schreibgeschützt und auf die Auswahl und Projektionen der definierten Azure Cosmos DB-SQL beschränkt. 
...
2. Geben Sie im Fenster **Ansicht bearbeiten** eine Azure Cosmos DB-Abfrage ein. Hierbei muss es sich um eine Azure Cosmos DB-SQL-Abfrage handeln, z. B. `SELECT c.City, c.EmployeeName, c.Level, c.Age, c.Gender, c.Manager FROM c WHERE c.City = “Seattle”`. Klicken Sie anschließend auf **OK**.
...
Weitere Informationen zu Azure Cosmos DB finden Sie unter [Was ist Azure Cosmos DB?](documentdb-introduction.md).
