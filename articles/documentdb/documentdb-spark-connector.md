---
title: "Herstellen einer Verbindung für Apache Spark mit Azure DocumentDB | Microsoft-Dokumentation"
description: "In diesem Tutorial wird der Azure DocumentDB-Spark-Connector beschrieben, mit dem Sie für Apache Spark eine Verbindung mit Azure DocumentDB herstellen können, um verteilte Aggregationen und Data Science-Vorgänge für das global verteilte mehrinstanzenfähige Datenbanksystem für die Cloud durchzuführen."
...
# <a name="accelerate-real-time-big-data-analytics-with-the-spark-to-documentdb-connector"></a>Beschleunigen der Big Data-Echtzeitanalyse mit dem Spark-Connector für DocumentDB

Mit dem Spark-Connector für DocumentDB kann Azure DocumentDB als Eingabequelle oder Ausgabesenke für Apache Spark-Aufträge fungieren. Durch die Verbindung von [Spark](http://spark.apache.org/) mit [DocumentDB](https://azure.microsoft.com/services/documentdb/) haben Sie die Möglichkeit, plötzlich auftretende Data Science-Probleme schnell zu lösen, indem Daten schnell beibehalten und mit DocumentDB abgefragt werden können. Der Spark-Connector für DocumentDB nutzt die nativen verwalteten Indizes von DocumentDB auf effiziente Weise und ermöglicht den Einsatz von aktualisierbaren Spalten für Analysen und der Pushdown-Prädikatfilterung für sich rasant ändernde global verteilte Daten, z.B. in IoT-, Data Science- und Analyseszenarien. 

## <a name="download"></a>Download

Beginnen Sie, indem Sie den Spark-Connector für DocumentDB (Vorschau) aus dem GitHub-Repository [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark/) herunterladen.

## <a name="connector-components"></a>Connectorkomponenten

Der Connector verwendet die folgenden Komponenten:

* [DocumentDB](http://documentdb.com) ist das mehrinstanzenfähige [global verteilte Datenbanksystem](documentdb-distribute-data-globally.md) von Microsoft, das speziell für die Cloud konzipiert wurde. Mit DocumentDB können Kunden sowohl den Durchsatz als auch den Speicher in beliebig vielen geografischen Regionen bereitstellen und elastisch skalieren. Der Dienst verfügt über eine garantierte niedrige Wartezeit (99. Perzentil), eine garantierte hohe Verfügbarkeit von 99,99% und [mehrere gut definierte Konsistenzmodelle](documentdb-consistency-levels.md) für Entwickler.
...
Es gibt zwei Ansätze zum Verbinden von Apache Spark mit Azure DocumentDB:
...
Mit dem aktuellen [pyDocumentDB SDK](https://github.com/Azure/azure-documentdb-python) können wir Spark wie im folgenden Diagramm dargestellt mit DocumentDB verbinden:

![Datenfluss von Spark zu DocumentDB über pyDocumentDB](./media/documentdb-spark-connector/azure-documentdb-spark-pydocumentdb.png)
...
1. Es wird eine Verbindung vom Spark-Masterknoten zum DocumentDB-Gatewayknoten über pyDocumentDB hergestellt.  Beachten Sie, dass der Benutzer nur Spark- und DocumentDB-Verbindungen angibt. Die Tatsache, dass eine Verbindung mit den jeweiligen Master- und Gatewayknoten hergestellt wird, ist für den Benutzer transparent.
2. Die Abfrage wird für DocumentDB (über den Gatewayknoten) durchgeführt, wo die Abfrage dann für die Partitionen der Sammlung auf den Datenknoten erfolgt. Die Antwort auf diese Abfragen wird zurück an den Gatewayknoten gesendet, und das Resultset wird an den Spark-Masterknoten zurückgegeben.
3. Alle nachfolgenden Abfragen (z.B. für einen Spark DataFrame) werden zur Verarbeitung an die Spark-Workerknoten gesendet.

Wichtig ist hierbei, dass die Kommunikation zwischen Spark und DocumentDB auf den Spark-Masterknoten und die DocumentDB-Gatewayknoten beschränkt ist.  Wie schnell die Abfragen durchgeführt werden, hängt von der Transportschicht zwischen diesen beiden Knoten ab.
...
### <a name="connecting-spark-to-documentdb-via-pydocumentdb"></a>Herstellen einer Verbindung von Spark mit DocumentDB per pyDocumentDB 
Aufgrund der Einfachheit des Kommunikationstransports ist die Ausführung einer Abfrage von Spark zu DocumentDB per pyDocumentDB ebenfalls relativ einfach.
...
Eine Verbindung für Spark mit DocumentDB per pyDocumentDB wird normalerweise in den folgenden Szenarien hergestellt:
...
## <a name="spark-to-documentdb-connector"></a>Spark-Connector für DocumentDB

Der Spark-Connector für DocumentDB nutzt das [Azure DocumentDB Java SDK](https://github.com/Azure/azure-documentdb-java) und verschiebt Daten zwischen den Spark-Workerknoten und DocumentDB. Dies ist im folgenden Diagramm dargestellt:

![Datenfluss im Spark-Connector für DocumentDB](./media/documentdb-spark-connector/azure-documentdb-spark-connector.png)

Der Datenfluss läuft wie folgt ab:

1. Es wird eine Verbindung vom Spark-Masterknoten zum DocumentDB-Gatewayknoten hergestellt, um die Partitionszuordnung zu erhalten.  Beachten Sie, dass der Benutzer nur die Spark- und DocumentDB-Verbindungen angibt. Die Tatsache, dass eine Verbindung mit den jeweiligen Master- und Gatewayknoten hergestellt wird, ist für den Benutzer transparent.
2. Diese Informationen werden dann wieder für den Spark-Masterknoten bereitgestellt.  An diesem Punkt sollten Sie die Abfrage analysieren können, um zu ermitteln, auf welche Partitionen (und ihre Standorte) Sie in DocumentDB Zugriff haben müssen.
3. Diese Informationen werden an die Spark-Workerknoten übertragen ...
4. So können für die Spark-Workerknoten direkte Verbindungen mit den DocumentDB-Partitionen hergestellt werden, um die erforderlichen Daten zu extrahieren und sie dann wieder zurück auf die Spark-Partitionen der Spark-Workerknoten zu übertragen.

Hierbei ist der Hinweis wichtig, dass die Kommunikation zwischen Spark und DocumentDB erheblich schneller abläuft, weil die Datenverschiebung zwischen den Spark-Workerknoten und den DocumentDB-Datenknoten (Partitionen) erfolgt.

### <a name="building-the-spark-to-documentdb-connector"></a>Erstellen des Spark-Connectors für DocumentDB
...
### <a name="connecting-spark-to-documentdb-using-the-connector"></a>Herstellen einer Verbindung für Spark mit DocumentDB per Connector
Der Kommunikationstransport ist hierbei etwas komplizierter, aber das Durchführen einer Abfrage von Spark an DocumentDB ist mit dem Connector deutlich schneller.
...
- Importieren Sie einfach die erforderlichen Bibliotheken, und konfigurieren Sie Ihren masterKey und host, um den DocumentDB-Client zu erstellen.
...
### <a name="why-use-the-spark-to-documentdb-connector-implementation"></a>Gründe für die Implementierung des Spark-Connectors für DocumentDB

Eine Verbindung für Spark mit DocumentDB mit dem Connector wird normalerweise in den folgenden Szenarien hergestellt:
...
* Sie müssen eine größere Datenmenge zwischen Apache Spark und DocumentDB übertragen.
...
Dieser Abschnitt enthält einige Beispiele dafür, wie Sie verteilte Aggregationen und Analysen durchführen können, indem Sie Apache Spark und Azure DocumentDB gemeinsam verwenden.  Beachten Sie Folgendes: Azure DocumentDB verfügt bereits über Unterstützung für Aggregationen, wie im Blog [Planet scale aggregates with Azure DocumentDB](https://azure.microsoft.com/blog/planet-scale-aggregates-with-azure-documentdb/) (Weltweite Aggregate mit Azure DocumentDB) beschrieben. Hier wird erläutert, wie Sie mit Apache Spark die nächste Stufe erreichen.

Beachten Sie, dass sich diese Aggregationen auf das [Notebook für den Spark-Connector für DocumentDB](https://github.com/Azure/azure-documentdb-spark/blob/master/samples/notebooks/Spark-to-DocumentDB_Connector.ipynb) beziehen.

### <a name="connecting-to-flights-sample-data"></a>Herstellen einer Verbindung mit Beispieldaten für Flüge
Für diese Aggregationsbeispiele greifen wir auf einige Flugleistungsdaten zu, die in unserer **DoctorWho**-DocumentDB-Datenbank gespeichert sind.  Verwenden Sie den folgenden Codeausschnitt, um die Verbindung herzustellen:
...
Mit diesem Codeausschnitt führen wir außerdem eine grundlegende Abfrage durch, mit der die gewünschten gefilterten Daten von DocumentDB an Spark übertragen werden (hierbei können von Spark verteilte Aggregatvorgänge durchgeführt werden).  In diesem Fall suchen wir nach Flügen, die in Seattle (SEA) starten.
...
Im nächsten Satz können wir nun leicht **GROUP BY**-Abfragen für unsere DocumentDB-Datenbank durchführen:
...
Laden Sie – falls noch nicht geschehen – den Spark-Connector für DocumentDB aus dem GitHub-Repository [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark) herunter, und sehen Sie sich die zusätzlichen Ressourcen im Repository an:
