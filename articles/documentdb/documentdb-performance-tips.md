...
Azure Cosmos DB ist eine schnelle und flexible verteilte Datenbank mit nahtloser Skalierung, garantierter Latenz und garantiertem Durchsatz. Die Skalierung Ihrer Datenbank mit Cosmos DB erfordert keine aufwendigen Änderungen an der Architektur und keine Erstellung von komplexem Code. Zentrales Hoch- und Herunterskalieren ist ebenso problemlos möglich wie das Aufrufen einer einzelnen API oder [SDK-Methode](documentdb-set-throughput.md#set-throughput-sdk). Da der Zugriff auf Cosmos DB jedoch über Netzwerkaufrufe erfolgt, können Sie clientseitige Optimierungen vornehmen, um eine optimale Leistung zu erzielen.
...
2. **Verbindungsrichtlinie: Verwenden des TCP-Protokolls**
   ...
   * HTTPS
    ...
    ![Abbildung der Azure Cosmos DB-Verbindungsrichtlinie](./media/documentdb-performance-tips/azure-documentdb-connection-policy.png)
...
4. **Platzieren der Clients in der gleichen Azure-Region**
    ...
    ![Abbildung der Azure Cosmos DB-Verbindungsrichtlinie](./media/documentdb-performance-tips/azure-documentdb-same-region.png)
...
Eine Beispielanwendung zur Evaluierung von Cosmos DB für Hochleistungsszenarien auf einigen Clientcomputern finden Sie unter [Leistungs- und Skalierungstests mit Cosmos DB](documentdb-performance-testing.md).

Weitere Informationen zur Gestaltung einer auf Skalierung und hohe Leistung ausgelegten Anwendung finden Sie unter [Partitionieren und Skalieren von Daten in Cosmos DB](documentdb-partition-data.md).
