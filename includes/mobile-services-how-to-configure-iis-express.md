...
5. Öffnen Sie die Windows-Firewall-Konsole, und erstellen Sie eine neue Portregel, sodass Verbindungen mit dem Port hergestellt werden können. Weitere Informationen zum Erstellen einer neuen Windows-Firewall-Portregel finden Sie unter [Hinzufügen einer neuen Windows-Firewall-Portregel].
   ...
   > [!NOTE]
   > Wenn der Test-Computer einer Domäne angehört, werden Firewallausnahmen möglicherweise durch eine Domänenrichtlinie kontrolliert. In diesem Fall müssen Sie sich an Ihren Domänenadministrator wenden, um eine Ausnahme für den Port auf Ihrem Computer zu erhalten.
   > 
   > 
   ...
   > [!NOTE]
   > Wenn Sie fertig sind, den Dienst lokal zu testen, sollten Sie die Windows-Firewall-Regel, die sie erstellt haben, löschen.
   > 
   > 
