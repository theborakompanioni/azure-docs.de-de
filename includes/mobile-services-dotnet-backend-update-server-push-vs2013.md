...
4. Fügen Sie den folgenden Code ein, in dem die POST-Methode für den Controller implementiert ist:
   ...
   > [!NOTE]
   > Diese POST-Methode kann von jedem beliebigen Client aufgerufen werden, der den Anwendungsschlüssel kennt. Sie ist nicht sicher. Fügen Sie zur Absicherung des Endpunkts der Methode oder Klasse das Attribut `[AuthorizeLevel(AuthorizationLevel.User)]` hinzu, um die Authentifizierung zu erzwingen.
   > 
   > 
