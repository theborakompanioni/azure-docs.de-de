1. Klicken Sie im Azure-[Verwaltungsportal](http://manage.windowsazure.com) auf **Virtuelle Computer**, und wählen Sie anschließend den gerade von Ihnen erstellten virtuellen Computer aus (**testlinuxvm**).
...
## <a name="connect-to-the-virtual-machine-using-ssh-or-putty-and-complete-setup"></a>Herstellen einer Verbindung mit dem virtuellen Computer mithilfe von SSH oder PuTTY und Abschließen der Einrichtung
...
11. Fügen Sie das neue Laufwerk zu /etc/fstab hinzu:
    ...
    > [!NOTE]
    > blkid erfordert nicht in allen Fällen einen sudo-Zugang, ist aber auf manchen Distributionen leichter mit `sudo -i` auszuführen, wenn /sbin oder /usr/sbin sich nicht in Ihrem `$PATH` befinden.
    > 
    > 
    ...
    **Achtung:** Eine fehlerhafte Bearbeitung der Datei /etc/fstab kann dazu führen, dass das System sich nicht mehr starten lässt. Wenn Sie sich nicht sicher sind, finden Sie in der Dokumentation der Verteilung Informationen zur korrekten Bearbeitung dieser Datei. Außerdem wird empfohlen, ein Backup der Datei /etc/fstab zu erstellen, bevor Sie sie bearbeiten.
    ...
    Sie können nun testen, ob das Dateisystem richtig eingebunden wurde, indem Sie einfach die Einbindung aufheben und wiederherstellen. Verwenden Sie dafür den zuvor erstellten Beispielbereitstellungspunkt `/datadrive`: 
