[Azure App Service](http://go.microsoft.com/fwlink/?LinkId=529714) unterstützt die fortlaufende Bereitstellung von Quellcodeverwaltung und Repository-Tools wie GitHub, BitBucket und Visual Studio Team Services. Sie können diese Tools verwenden, um den Inhalt und den Code für Ihre App zu verwalten und schnell und einfach Änderungen an Ihre App Service-App mithilfe von Push zu übertragen.

In diesem Artikel erfahren Sie, wie Sie mithilfe von Git direkt von Ihrem lokalen Computer aus in Web-Apps veröffentlichen (in Azure wird diese Methode der Veröffentlichung **lokales Git** genannt). Außerdem wird erläutert, wie Sie die fortlaufende Bereitstellung von Repository-Websites aus ermöglichen, z.B. von BitBucket und GitHub. Informationen zur Verwendung von VSTS für die fortlaufende Bereitstellung finden Sie unter [Continuous Delivery für Azure mithilfe von Visual Studio Team Services].

> [!NOTE]
> Viele der in diesem Artikel beschriebenen Git-Befehle werden automatisch ausgeführt, wenn Sie eine Web-App mit den [Azure-Befehlszeilentools für Mac und Linux](/develop/nodejs/how-to-guides/command-line-tools/) erstellen
> 
> 
...
3. Stellen Sie mit der Befehlszeile sicher, dass Sie sich im Stammverzeichnis Ihres Git-Repositorys befinden. Verwenden Sie dann folgenden Befehl, um dem Repository die Datei **index.html** hinzuzufügen:
   ...
   > [!NOTE]
   > Sie können Hilfeinformationen für jeden Git-Befehl aufrufen, indem Sie nach dem Befehl "-help" oder "--help" eingeben. Geben Sie als Parameteroptionen für den Befehl "add" beispielsweise "git add -help" ein, um Befehlszeilenhilfe zu erhalten, oder "git add --help", um ausführlichere Hilfe anzuzeigen.
   > 
   > 
...
2. Stellen Sie unter Verwendung der Befehlszeile sicher, dass das Stammverzeichnis Ihres lokalen Git-Repositorys, das die vorher erstellte Datei "index.html" enthält, das aktuelle Verzeichnis ist.
3. Fügen Sie mit `git remote` die in Schritt 1 unter **Git-URL** aufgeführte Remotereferenz hinzu. Der Befehl sieht etwa wie folgt aus:
   ...
   > 
   > 
4. Verwenden Sie Folgendes an der Befehlszeile, um die aktuellen Repository-Inhalte aus dem lokalen Repository per Push in das "azure"-Remote-Repository zu übertragen:
   ...
   > [!NOTE]
   > Das für Ihre Web-App erstellte Repository erwartet Pushanforderungen für die <strong>master</strong>-Verzweigung des Repositorys, die dann als Inhalt der Web-App verwendet werden.
   > 
   > 
5. Kehren Sie zum Blatt Ihrer Web-App im Azure-Portal zurück. **No deployment found** sollte in **Aktive Bereitstellung** mit einen Protokolleintrag des letzten Pushvorgangs geändert werden.
   
    ![](./media/publishing-with-git/azure4-deployed.png)
6. Klicken Sie oben in der Web-App auf die **URL**, um zu überprüfen, ob **index.html** bereitgestellt wurde. Eine Seite mit "Hallo Git!" wird eingeblendet.
   
    ![Eine Webseite mit "Hallo Git!"][hello-git]
7. Ändern Sie die Datei **index.html** mit einem Text-Editor so, dass sie "Juhu!" enthält, und speichern Sie dann die Datei.
8. Verwenden Sie die folgenden Befehle an der Befehlszeile, um die Änderungen **hinzuzufügen** und zu **übernehmen**. Übertragen Sie die Änderungen dann per **Push** in das Remote-Repository:
...
3. Klicken Sie auf dem Blatt **Continous deployment** auf **Autorisierung** und dann auf **Autorisieren**. Das Azure-Portal leitet Sie an die Repository-Site weiter, um die Autorisierung abzuschließen.
...
5. Wählen Sie auf dem Blatt **Continuous Deployment** die Organisation, das Projekt und die Verzweigung für die Bereitstellung aus. Wenn Sie fertig sind, klicken Sie auf **OK**.
   ...
   > [!NOTE]
   > Wenn Sie die fortlaufende Bereitstellung mit GitHub oder BitBucket aktivieren, werden öffentliche und private Projekte angezeigt.
   > 
   > 
...
1. An diesem Punkt wurde Ihr Projekt aus dem Repository Ihrer Wahl in Ihrer Web-App bereitgestellt. Um zu überprüfen, ob die Web-App aktiv ist, klicken Sie oben im Portal auf die **URL**. Der Browser sollte zur Web-App wechseln.
...
* Fügen Sie dem Stammverzeichnis Ihres Repositorys eine `.gitignore`-Datei hinzu, und geben Sie alle Dateien und Ordner an, die Sie aus dem Repository ausschließen möchten, beispielsweise die Ordner `Obj`, `Bin` und `packages` (Formatinformationen finden Sie in der [gitignore-Dokumentation](http://git-scm.com/docs/gitignore)). Beispiel:
  ...
  > [!NOTE]
  > Bei Verwendung von GitHub können Sie beim Erstellen Ihres Repositorys optional eine Visual Studio-spezifische GITIGNORE-Datei generieren. Diese enthält alle allgemeinen temporären Dateien, Buildergebnisse usw. Anschließend können Sie sie Ihren Anforderungen entsprechend anpassen.
  > 
  > 
