...
In der folgenden Tabelle werden allgemeine Aufgaben für virtuelle Computer verglichen, die Sie mit Befehlen der Azure-Befehlszeilenschnittstelle im Dienstverwaltungsmodus und im Resource Manager-Modus ausführen können. Bei vielen Ressourcen-Manager-Befehlen müssen Sie den Namen einer vorhandenen Ressourcengruppe übergeben.
...
Erstellen eines virtuellen Computers von einem speziellen Datenträger | `azure  vm create [options]-d <custom-data-file> <dns-name> [userName] [password]` | `azure  vm create [options] –d <os-disk-vhd> <resource-group> <name> <location> <os-type>`
