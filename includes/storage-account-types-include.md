...
Ein allgemeines Speicherkonto bietet Ihnen Zugriff auf Azure Storage-Funktionen wie Tabellen, Warteschlangen, Dateien, Blobs und Datenträger von virtuellen Azure-Computern in einem einzelnen Konto. Diese Art von Speicherkonto umfasst zwei Leistungsstufen:

- Eine Standardleistungsstufe ermöglicht Ihnen das Speichern von Tabellen, Warteschlangen, Dateien, Blobs und Datenträgern von virtuellen Azure-Computern.
- Die Premium-Leistungsstufe unterstützt derzeit nur Datenträger von virtuellen Azure-Computern. Eine ausführliche Übersicht über Azure Premium Storage finden Sie unter [Premium-Speicher: Hochleistungsspeicher für Arbeitslasten auf virtuellen Azure-Computern](../articles/storage/storage-premium-storage.md).

### BLOB-Speicherkonten

BLOB-Speicherkonten sind spezielle Speicherkonten und dienen dazu, unstrukturierte Daten als Blobs (Objekte) in Azure Storage zu speichern. BLOB-Speicherkonten sind mit Ihren bereits vorhandenen allgemeinen Speicherkonten vergleichbar und besitzen die gleichen Haltbarkeits-, Verfügbarkeits-, Skalierbarkeits- und Leistungseigenschaften, die Sie schon heute verwenden – einschließlich vollständiger API-Konsistenz für Blockblobs und Anfügeblobs. BLOB-Speicherkonten verfügen über das Attribut **Zugriffsstufe**, das bei der Kontoerstellung festgelegt und später bei Bedarf geändert werden kann. Es gibt zwei Arten von Zugriffsstufen, die auf Grundlage Ihres Datenzugriffsmusters festgelegt werden können:
...
Bei Anwendungen, die nur Block- oder Anfügeblobspeicher benötigen, empfiehlt sich die Verwendung von BLOB-Speicherkonten. Weitere Informationen finden Sie unter [Azure Blob Storage: Cool and Hot tiers](../articles/storage/storage-blob-storage-tiers.md) Azure Blob Storage: Stufen für selten genutzte Daten (Cool Data) und häufig genutzte Daten (Hot Data)).
